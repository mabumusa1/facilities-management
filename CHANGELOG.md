...
- Facilities — admins can create, edit, view, and deactivate facilities from **Facilities → New Facility**: bilingual name (EN+AR), type, community, capacity, pricing mode (Free / Per Session / Per Hour with price and currency), booking constraints (advance booking window, cancellation deadline, min/max session duration), contract-required toggle, internal notes, and a 7-day availability grid (open time, close time, slot duration, max concurrent bookings per day); deactivating a facility with upcoming bookings requires an explicit confirmation step — the backend rejects unconfirmed deactivation attempts; facility detail page shows pricing, active availability rules, constraint summary, and booking count; all actions permission-gated (`facilities.CREATE` / `facilities.UPDATE`) ([#347](https://github.com/mabumusa1/facilities-management/pull/347))
