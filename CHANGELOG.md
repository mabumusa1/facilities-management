...
- Service Requests → Settings → Service Categories — admins can create and manage a two-level category tree (categories + nested subcategories) with bilingual EN/AR names, emoji icons, community visibility, SLA response and resolution time targets (hours), a default handler, and a completion-photo requirement flag; subcategories inherit parent SLA by default with optional per-subcategory overrides; categories and subcategories with active service requests are protected from deletion; permission-gated to admin role ([#349](https://github.com/mabumusa1/facilities-management/pull/349))
