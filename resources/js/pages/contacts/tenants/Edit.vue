...
const pageTitle = computed(() => t('app.contacts.tenants.editTitle', { name: `${props.resident.first_name} ${props.resident.last_name}` }));
...
<template>
    ...
    <div class="flex flex-col gap-6 p-4">
        <Heading variant="small" :title="pageTitle" :description="t('app.contacts.tenants.editDescription')" />
        <form @submit.prevent="submit" class="max-w-2xl space-y-6">
            ...
            <div class="flex items-center gap-4"><Button :disabled="form.processing">{{ t('app.contacts.tenants.updateButton') }}</Button></div>
        </form>
    </div>
</template>
