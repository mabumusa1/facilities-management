...
defineOptions({ layout: { breadcrumbs: [{ title: 'Dashboard', href: '/dashboard' }, { title: 'Professionals', href: '/professionals' }, { title: 'Edit', href: '#' }] } });
...
<template>
    <Head :title="`Edit ${professional.first_name} ${professional.last_name}`" />
    <div class="flex flex-col gap-6 p-4">
        <Heading variant="small" :title="`Edit ${professional.first_name} ${professional.last_name}`" description="Update professional details." />
        <form @submit.prevent="submit" class="max-w-2xl space-y-6">
            ...
            <div class="flex items-center gap-4"><Button :disabled="form.processing">Update Professional</Button></div>
        </form>
    </div>
</template>
