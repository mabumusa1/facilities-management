...
defineOptions({ layout: { breadcrumbs: [{ title: 'Dashboard', href: '/dashboard' }, { title: 'Residents', href: '/residents' }, { title: 'Details', href: '#' }] } });
...
<template>
    ...
    <div class="flex flex-col gap-6 p-4">
        ...
        <div class="grid gap-4 md:grid-cols-2">
            <Card>
                ...
                <CardContent class="space-y-2">
                    ...
                    <div class="flex justify-between"><span class="text-muted-foreground">Country Code</span><span>{{ resident.phone_country_code }}</span></div>
                </CardContent>
            </Card>
            <Card>
                ...
                <CardContent class="space-y-2">
                    ...
                    <div class="flex justify-between"><span class="text-muted-foreground">Units</span><span>{{ resident.units_count ?? 0 }}</span></div>
                    <div class="flex justify-between"><span class="text-muted-foreground">Leases</span><span>{{ resident.leases_count ?? 0 }}</span></div>
                </CardContent>
            </Card>
        </div>
        ...
        <Card v-if="resident.dependents && resident.dependents.length > 0">
            <CardHeader><CardTitle>Dependents</CardTitle></CardHeader>
            <CardContent>
                <div class="space-y-2">
                    <div v-for="dep in resident.dependents" :key="dep.id" class="flex items-center justify-between rounded-md border p-3">
                        ...
                        <span class="text-muted-foreground text-sm">{{ dep.relationship ?? '—' }}</span>
                    </div>
                </div>
            </CardContent>
        </Card>
