...
function getPriorityBadge(priority: string) {
    const priorityConfig: Record<string, { icon: React.ReactNode; className: string; label: string }> = {
        low: { icon: <Info className="h-3 w-3" />, className: 'bg-gray-100 text-gray-600', label: 'Low' },
        normal: { icon: <Info className="h-3 w-3" />, className: 'bg-blue-100 text-blue-600', label: 'Normal' },
        high: { icon: <AlertTriangle className="h-3 w-3" />, className: 'bg-orange-100 text-orange-600', label: 'High Priority' },
        urgent: { icon: <AlertTriangle className="h-3 w-3" />, className: 'bg-red-100 text-red-600', label: 'Urgent' },
    };
...
function AnnouncementCard({ announcement }: { announcement: Announcement }) {
    ...
    return (
        <Card className={`transition-shadow hover:shadow-md ${isUrgent ? 'border-l-4 border-l-orange-500' : ''}`}>
            ...
            <CardContent>
                <p className="text-sm text-muted-foreground whitespace-pre-wrap mb-4">{announcement.description}</p>
        ...
        <>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto p-4 md:p-6">
                <div>
                    ...
                    <p className="text-muted-foreground">Important announcements and notices for your community</p>
                </div>
