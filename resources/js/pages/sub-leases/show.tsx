...
export default function SubLeasesShow({ sublease }: Props) {
    ...
    return (
        <>
            <Head
                title={`Sub-Lease ${sublease.contract_number ?? `#${sublease.id}`}`}
            />
                        ...
                        <CardContent>
                            {sublease.parent_lease ? (
                                <div className="space-y-2">
                                    <Link
                                        href={`/leases/${sublease.parent_lease.id}`}
                                        className="text-blue-600 hover:underline font-medium"
                                    >
                                        {sublease.parent_lease
                                            .contract_number ??
                                            `#${sublease.parent_lease.id}`}
                                    </Link>
                                    {sublease.parent_lease.tenant && (
                                        <p className="text-sm text-muted-foreground">
                                            Tenant:{" "}
                                            {sublease.parent_lease.tenant.name}
                                        </p>
                                    )}
                                </div>
                            ) : (
                                <p className="text-sm text-muted-foreground">
                                    No parent lease linked
                                </p>
                            )}
                        </CardContent>
                    </Card>
