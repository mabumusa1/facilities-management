...
export default function SubLeasesCreate({
    parentLeases,
    communities,
    buildings,
    units,
    ...
    const toggleUnit = (unitId: number) => {
        ...
        setData(
            ...
            exists
                ...
                : [...data.units, { id: unitId }],
        );
    };

    const selectedParent = parentLeases.find(
        (p) => p.id === Number(data.parent_lease_id),
    );
        ...
        <>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-4 md:p-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground">
                            Link a new sub-lease to an existing active lease
                        </p>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6 max-w-3xl">
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle>Parent Lease</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <div className="space-y-2">
                                <Label htmlFor="parent_lease_id">
                                    Parent Lease *
                                </Label>
                                <Select
                                    value={data.parent_lease_id}
                                    onValueChange={(v) =>
                                        setData("parent_lease_id", v)
                                    }
                                >
                                    <SelectTrigger>
                                        <SelectValue placeholder="Select parent lease" />
                                    </SelectTrigger>
                                    <SelectContent>
                                        {parentLeases.map((l) => (
                                            <SelectItem
                                ...
                                {selectedParent && (
                                    <p className="text-xs text-muted-foreground">
                                        Parent period:{" "}
                                        {new Date(
                                            selectedParent.start_date,
                                        ).toLocaleDateString()}{" "}
                                        –{" "}
                                        {new Date(
                                            selectedParent.end_date,
                                        ).toLocaleDateString()}
                                    </p>
                                )}
                            </div>
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle>Sub-Tenant & Contract Details</CardTitle>
                        </CardHeader>
                        ...
                        <CardContent className="grid gap-4 md:grid-cols-2">
                            ...
                            <div className="space-y-2 md:col-span-2">
                                ...
                                <div className="grid grid-cols-3 gap-2 max-h-40 overflow-y-auto border rounded-md p-2">
                                    {units.map((u) => {
                                        const selected = data.units.some(
                                            (su) => su.id === u.id,
                                        );
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle>Financial & Duration</CardTitle>
                        </CardHeader>
                        <CardContent className="grid gap-4 md:grid-cols-2">
                            <div className="space-y-2">
