...
export default function LeaseEdit({ lease, communities, buildings, units, tenants, statuses }: LeaseEditProps) {
    const { data, setData, put, processing, errors } = useForm({
        ...
        security_deposit_due_date: lease.security_deposit_due_date?.split('T')[0] || '',
    ...
    const handleUnitToggle = (unitId: number) => {
        ...
        if (exists) {
            ...
            setData('units', [...data.units, { id: unitId, annual_rental_amount: '', net_area: '', meter_cost: '' }]);
        }
    };

    const handleUnitDataChange = (unitId: number, field: string, value: string) => {
        setData('units', data.units.map(u => {
            if (u.id === unitId) {
    ...
    return (
        <>
            <Head title={`Edit Lease ${lease.contract_number || `#${lease.id}`}`} />
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold tracking-tight">
                                Edit Lease {lease.contract_number || `#${lease.id}`}
                            </h1>
                                    ...
                                    <div className="grid gap-4 sm:grid-cols-2">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Select
                                                ...
                                                onValueChange={(value) => setData('tenant_type', value)}
                                            >
                                                <SelectTrigger>
                                            ...
                                            {errors.tenant_type && (
                                                <p className="text-sm text-destructive">{errors.tenant_type}</p>
                                            )}
                                        </div>
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            {errors.rental_type && (
                                                <p className="text-sm text-destructive">{errors.rental_type}</p>
                                            )}
                                        </div>
                                    </div>
                                </CardContent>
                                ...
                                <CardContent className="space-y-4">
                                    <div className="grid gap-4 sm:grid-cols-2">
                                        <div className="space-y-2">
                                            ...
                                            <Select
                                                value={String(data.community_id)}
                                                onValueChange={(value) => {
                                        ...
                                        <div className="rounded-md border">
                                            {filteredUnits.length === 0 ? (
                                                <p className="p-4 text-sm text-muted-foreground">
                                                    Select a building to see available units
                                                </p>
                                            ) : (
                                                                ...
                                                                {isSelected && (
                                                                    <div className="mt-3 grid gap-3 pl-7 sm:grid-cols-3">
                                                                        <div className="space-y-1">
                                                                            ...
                                                                            <Input
                                                                                type="number"
                                                                                value={selectedUnit?.annual_rental_amount || ''}
                                                                                onChange={(e) => handleUnitDataChange(unit.id, 'annual_rental_amount', e.target.value)}
                                                                                placeholder="0.00"
                                                                            />
                                                                        </div>
                                                                        <div className="space-y-1">
                                                                            <Label className="text-xs">Net Area (sqm)</Label>
                                                                            <Input
                                                                                type="number"
                                                                                value={selectedUnit?.net_area || ''}
                                                                                onChange={(e) => handleUnitDataChange(unit.id, 'net_area', e.target.value)}
                                                                                placeholder="0"
                                                                            />
                                                                        </div>
                                                                        <div className="space-y-1">
                                                                            <Label className="text-xs">Meter Cost</Label>
                                                                            <Input
                                                                                type="number"
                                                                                value={selectedUnit?.meter_cost || ''}
                                                                                onChange={(e) => handleUnitDataChange(unit.id, 'meter_cost', e.target.value)}
                                                                                placeholder="0.00"
                                                                            />
                                                                        </div>
                                                                    </div>
                                ...
                                <CardContent className="space-y-4">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <Checkbox
                                            ...
                                            onCheckedChange={(checked) => setData('is_terms', checked as boolean)}
                                        />
                                        <Label htmlFor="is_terms" className="cursor-pointer">
                                    ...
                                    <div className="grid grid-cols-3 gap-2">
                                        <div className="space-y-1">
                                            ...
                                            <Input
                                                ...
                                                onChange={(e) => setData('number_of_years', parseInt(e.target.value) || 0)}
                                            />
                                        </div>
                                        <div className="space-y-1">
                                            ...
                                            <Input
                                                ...
                                                onChange={(e) => setData('number_of_months', parseInt(e.target.value) || 0)}
                                            />
                                        </div>
                                        <div className="space-y-1">
                                            ...
                                            <Input
                                                ...
                                                onChange={(e) => setData('number_of_days', parseInt(e.target.value) || 0)}
                                            />
                                        </div>
                                    </div>

                                    <div className="space-y-2">
                                        <Label htmlFor="free_period">Free Period (days)</Label>
                                        <Input
                                            id="free_period"
                                            type="number"
                                            min="0"
                                            value={data.free_period}
                                            onChange={(e) => setData('free_period', parseInt(e.target.value) || 0)}
                                        />
                                    </div>
                                </CardContent>
                            </Card>
                                ...
                                <CardContent className="space-y-4">
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            ...
                                            onChange={(e) => setData('rental_total_amount', e.target.value)}
                                        />
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            ...
                                            onChange={(e) => setData('security_deposit_amount', e.target.value)}
                                        />
                                    </div>

                                    <div className="space-y-2">
                                        <Label htmlFor="security_deposit_due_date">Deposit Due Date</Label>
                                        <Input
                                            id="security_deposit_due_date"
                                            type="date"
                                            value={data.security_deposit_due_date}
                                            onChange={(e) => setData('security_deposit_due_date', e.target.value)}
                                        />
                                    </div>
                                </CardContent>
                            </Card>
