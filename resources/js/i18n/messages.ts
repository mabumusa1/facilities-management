import appArFallback from '@/i18n/appArFallback';
import appEnFallback from '@/i18n/appEnFallback';
import arBaseMessages from '@/locales/ar.json';
import enBaseMessages from '@/locales/en.json';

type MessageValue = string | number | boolean | null | MessageTree;

interface MessageTree {
    [key: string]: MessageValue;
}

function isObject(value: unknown): value is MessageTree {
    return typeof value === 'object' && value !== null && !Array.isArray(value);
}

function deepMerge(base: MessageTree, overrides: MessageTree): MessageTree {
    const merged: MessageTree = { ...base };

    for (const [key, value] of Object.entries(overrides)) {
        const baseValue = merged[key];

        if (isObject(baseValue) && isObject(value)) {
            merged[key] = deepMerge(baseValue, value);
            continue;
        }

        merged[key] = value;
    }

    return merged;
}

const appEn: MessageTree = {
    app: {
        language: {
            current: 'Language',
            english: 'English',
            arabic: 'Arabic',
        },
        common: {
            warning: 'Warning',
            somethingWentWrong: 'Something went wrong.',
            notAvailable: 'N/A',
            orReturnTo: 'Or, return to',
            orYouCan: 'or you can',
            yes: 'Yes',
            no: 'No',
            active: 'Active',
            inactive: 'Inactive',
            pageSummary: 'Page {{current}} of {{last}}',
            field: 'Field',
            optional: 'Optional',
            cancel: 'Cancel',
            save: 'Save',
            notes: 'Notes',
            noResults: 'No results.',
            search: 'Search',
            all: 'All',
            saving: 'Saving...',
            select: 'Select',
        },
        actions: {
            create: 'Create',
            save: 'Save',
            cancel: 'Cancel',
            back: 'Back',
            next: 'Next',
            close: 'Close',
            edit: 'Edit',
            delete: 'Delete',
            apply: 'Apply',
            reset: 'Reset',
            logout: 'Log out',
            markAllAsRead: 'Mark All as Read',
            markRead: 'Mark Read',
            continue: 'Continue',
            resendVerificationEmail: 'Resend verification email',
            enable2fa: 'Enable 2FA',
            disable2fa: 'Disable 2FA',
            continueSetup: 'Continue setup',
        },
        navigation: {
            modules: 'Modules',
            platform: 'Platform',
            dashboard: 'Dashboard',
            properties: 'Properties',
            communities: 'Communities',
            buildings: 'Buildings',
            units: 'Units',
            leasing: 'Leasing',
            leases: 'Leases',
            leads: 'Leads',
            requests: 'Requests',
            allRequests: 'All Requests',
            marketplace: 'Marketplace',
            overview: 'Overview',
            customers: 'Customers',
            listing: 'Listing',
            visits: 'Visits',
            facilities: 'Facilities',
            bookings: 'Bookings',
            accounting: 'Accounting',
            transactions: 'Transactions',
            communication: 'Communication',
            announcements: 'Announcements',
            contacts: 'Contacts',
            owners: 'Owners',
            tenants: 'Tenants',
            admins: 'Admins',
            professionals: 'Professionals',
            visitorAccess: 'Visitor Access',
            visitorAccessHistory: 'Visitor Access History',
            reports: 'Reports',
            systemReports: 'System Reports',
            appSettings: 'App Settings',
            settingsShell: 'Settings Shell',
            settingsFacilities: 'Settings Facilities',
            settingsForms: 'Settings Forms',
            requestCategories: 'Request Categories',
            facilityCategories: 'Facility Categories',
            invoiceSettings: 'Invoice Settings',
            companyProfile: 'Company Profile',
            generalSettings: 'General Settings',
            userManagement: 'User Management',
            accountSubscriptions: 'Account Subscriptions',
            documentTemplates: 'Document Templates',
            shared: 'Shared',
            notifications: 'Notifications',
            repository: 'Repository',
            documentation: 'Documentation',
            settings: 'Settings',
            profile: 'Profile',
            security: 'Security',
            appearance: 'Appearance',
            settingsDescription: 'Manage your profile and account settings',
            settingsAriaLabel: 'Settings',
        },
        auth: {
            login: {
                headTitle: 'Log in',
                layoutTitle: 'Log in to your account',
                layoutDescription: 'Enter your email and password below to log in',
                emailAddress: 'Email address',
                password: 'Password',
                forgotPassword: 'Forgot password?',
                rememberMe: 'Remember me',
                submit: 'Log in',
                noAccount: "Don't have an account?",
                signUp: 'Sign up',
                emailPlaceholder: 'email@example.com',
                invalidCredentials: 'These credentials do not match our records.',
                throttled: 'Too many login attempts. Please try again in {{seconds}} seconds.',
                showPassword: 'Show password',
                hidePassword: 'Hide password',
            },
            register: {
                headTitle: 'Register',
                layoutTitle: 'Create an account',
                layoutDescription: 'Enter your details below to create your account',
                name: 'Name',
                fullName: 'Full name',
                personalInformation: 'Personal information',
                firstName: 'First name',
                firstNamePlaceholder: 'First name',
                lastName: 'Last name',
                lastNamePlaceholder: 'Last name',
                emailAddress: 'Email address',
                phoneNumber: 'Phone number',
                phoneNumberPlaceholder: '+966500000000',
                tenantInformation: 'Account information',
                accountName: 'Account name',
                accountNamePlaceholder: 'Your account name',
                trialTitle: 'Trial details',
                trialDetails: 'Start with a 14-day free trial, then continue for 300 SAR per month.',
                acceptTerms: 'I agree to the terms and conditions.',
                password: 'Password',
                confirmPassword: 'Confirm password',
                submit: 'Create account',
                hasAccount: 'Already have an account?',
                logIn: 'Log in',
                emailPlaceholder: 'email@example.com',
            },
            registerInvite: {
                layoutTitle: 'Create your account',
                layoutDescription: 'Enter your invite code and set a password',
                inviteCode: 'Invite code',
                inviteCodePlaceholder: 'Enter your invite code',
                password: 'Password',
                confirmPassword: 'Confirm password',
                submit: 'Create account',
                hasAccount: 'Already have an account?',
                logIn: 'Log in',
            },
            forgotPassword: {
                headTitle: 'Forgot password',
                layoutTitle: 'Forgot password',
                layoutDescription: 'Enter your email to receive a password reset link',
                emailAddress: 'Email address',
                emailPlaceholder: 'email@example.com',
                submit: 'Email password reset link',
                logIn: 'log in',
            },
            resetPassword: {
                headTitle: 'Reset password',
                layoutTitle: 'Reset password',
                layoutDescription: 'Please enter your new password below',
                email: 'Email',
                password: 'Password',
                confirmPassword: 'Confirm password',
                resetPassword: 'Reset password',
            },
            verifyEmail: {
                headTitle: 'Verify email',
                layoutTitle: 'Verify your email',
                layoutDescription: 'We sent a verification link to {email}',
                instructions: 'Check your inbox and click the link to activate your account.',
                success: 'A new verification link has been sent.',
            },
            confirmPassword: {
                headTitle: 'Confirm password',
                layoutTitle: 'Confirm your password',
                layoutDescription: 'This is a secure area of the application. Please confirm your password before continuing.',
                password: 'Password',
                submit: 'Confirm password',
            },
            twoFactor: {
                headTitle: 'Two-factor authentication',
                recoveryCode: 'Recovery code',
                recoveryDescription: 'Please confirm access to your account by entering one of your emergency recovery codes.',
                authenticationCode: 'Authentication code',
                authenticationDescription: 'Enter the authentication code provided by your authenticator application.',
                useAuthenticationCode: 'log in using an authentication code',
                useRecoveryCode: 'log in using a recovery code',
                recoveryPlaceholder: 'Enter recovery code',
            },
            setPassword: {
                headTitle: 'Set your password',
                layoutTitle: 'Set your password',
                layoutDescription: 'Set a password to activate your account.',
                password: 'Password',
                confirmPassword: 'Confirm password',
                submit: 'Set Password & Sign In',
                expiredToken: 'This invitation link has expired or has already been used. Please contact your administrator to issue a new invitation.',
            },
        },
        admin: {
            users: {
                pageTitle: 'Manage Users',
                heading: 'Account user management',
                description: 'Create and manage users with role-based access for this account.',
                currentAccount: 'Current account',
                name: 'Name',
                email: 'Email',
                role: 'Role',
                password: 'Password',
                passwordConfirmation: 'Password confirmation',
                addUser: 'Add user',
                remove: 'Remove',
                actions: 'Actions',
                empty: 'No account users found.',
                showPageTitle: 'User Detail',
                backToUsers: '← Users',
                editUser: 'Edit user',
                tabRoles: 'Roles',
                tabOverview: 'Overview',
                tabActivity: 'Activity',
                assignRoleBtn: 'Assign Role',
                drawerTitle: 'Assign Role',
                roleLabel: 'Role',
                rolePlaceholder: 'Select a role…',
                communityLabel: 'Community',
                communityPlaceholder: 'Select communities…',
                buildingLabel: 'Building',
                buildingPlaceholder: 'Select buildings…',
                buildingOptionalHint: 'Optional — leave blank to apply to all buildings in the selected communities.',
                serviceTypeLabel: 'Service Type',
                serviceTypePlaceholder: 'Select service types…',
                globalScopeNote: 'This role applies globally. No scope selection needed.',
                assignBtn: 'Assign',
                cancelBtn: 'Cancel',
                cancelConfirm: 'Discard changes?',
                colRole: 'Role',
                colScope: 'Scope',
                colAssigned: 'Assigned',
                colActions: 'Actions',
                removeTooltip: 'Remove assignment',
                removePopoverTitle: 'Remove this role assignment?',
                removePopoverBody: 'This cannot be undone.',
                removePopoverConfirm: 'Remove',
                removePopoverCancel: 'Cancel',
                emptyRolesHeading: 'No roles assigned yet',
                emptyRolesBody: 'This user has no active role assignments.',
                toastAssigned: 'Role assigned successfully.',
                toastRemoved: 'Role assignment removed.',
                validationRoleRequired: 'Please select a role.',
                validationCommunityRequired: 'Please select at least one community for this role.',
                validationServiceTypeRequired: 'Please select at least one service type for this role.',
                status: 'Status',
                statusActive: 'Active',
                statusInvitationPending: 'Invitation pending',
                statusDeactivated: 'Deactivated',
                inviteTitle: 'Invite User',
                inviteUser: 'Invite User',
                firstName: 'First name',
                lastName: 'Last name',
                firstNamePlaceholder: 'e.g. Jana',
                lastNamePlaceholder: 'e.g. Al-Ali',
                emailPlaceholder: 'e.g. name@company.com',
                sendInvitation: 'Send Invitation',
                deactivate: 'Deactivate',
                reactivate: 'Reactivate',
                sendPasswordReset: 'Send password reset',
                resendInvitation: 'Resend invitation',
                revokeInvitation: 'Revoke invitation',
                viewDetails: 'View details',
                moreActions: 'More actions',
                cannotDeactivateSelf: 'You cannot deactivate your own account.',
                deactivatedBanner: 'This user\'s account is deactivated. They cannot log in until an administrator reactivates their account.',
                deactivateConfirmTitle: 'Deactivate account?',
                deactivateConfirmBody: 'This user will immediately lose access and cannot log in. You can reactivate them at any time.',
                deactivateConfirmAction: 'Deactivate',
            },
            roles: {
                pageTitle: 'Roles — Admin',
                heading: 'Roles',
                description: 'Manage roles for this account.',
                breadcrumb: 'Roles',
                search: 'Search roles…',
                typeLabel: 'Type',
                typeAll: 'All',
                typeUserRole: 'User Role',
                typeAdminRole: 'Admin Role',
                newRole: '+ New Role',
                colNameEn: 'Name (English)',
                colNameAr: 'Name (Arabic)',
                colType: 'Type',
                colUsers: 'Users',
                colActions: 'Actions',
                systemBadge: 'System',
                ariaEditRole: 'Edit role',
                ariaDeleteRole: 'Delete role',
                editTooltip: 'System roles cannot be modified.',
                deleteTooltip: 'System roles cannot be deleted.',
                drawerCreateTitle: 'New Role',
                drawerEditTitle: 'Edit Role',
                nameEnLabel: 'Role name (English)',
                nameArLabel: 'Role name (Arabic)',
                typeImmutableHint: 'Role type cannot be changed after creation.',
                cancel: 'Cancel',
                createRole: 'Create Role',
                saveChanges: 'Save Changes',
                saving: 'Saving…',
                deleteDialogTitle: 'Delete role?',
                deleteDialogBody: 'This will permanently remove "{name}". Users assigned to it will lose this role.',
                deleteDialogConfirm: 'Delete Role',
                deleteDialogCancel: 'Cancel',
                emptyHeading: 'No roles found.',
                emptyBody: 'Try adjusting your search or filter, or create your first custom role.',
                emptyCta: '+ New Role',
                toastCreated: 'Role created successfully.',
                toastUpdated: 'Role updated.',
                toastDeleted: 'Role deleted.',
                toastError: 'Something went wrong. Please try again.',
                managePermissions: 'Manage Permissions',
                permissions: {
                    pageTitle: 'Permissions — {{name}}',
                    tab: 'Permissions',
                    systemBanner: 'System roles cannot be modified. These permissions are managed by the platform.',
                    applyPreset: 'Apply preset',
                    presetPlaceholder: 'Select preset…',
                    presetTooltip: 'Presets load a standard permission set. You can adjust individual cells after applying.',
                    viewRequiredTooltip: 'View is required when any other action is enabled.',
                    allSubjects: 'All subjects',
                    all: 'All',
                    unsavedChanges: 'Unsaved changes',
                    savePermissions: 'Save permissions',
                    cancel: 'Cancel',
                    saving: 'Saving…',
                    permissionsSaved: 'Permissions saved',
                    saveError: 'Could not save permissions. Please try again.',
                    actions: {
                        VIEW: 'View',
                        CREATE: 'Create',
                        UPDATE: 'Update',
                        DELETE: 'Delete',
                        RESTORE: 'Restore',
                        FORCE_DELETE: 'Force Delete',
                    },
                    subjects: {
                        communities: 'Communities',
                        buildings: 'Buildings',
                        units: 'Units',
                        leases: 'Leases',
                        subLeases: 'Sub-Leases',
                        transactions: 'Transactions',
                        payments: 'Payments',
                        owners: 'Owners',
                        tenants: 'Tenants',
                        dependents: 'Dependents',
                        admins: 'Admins',
                        professionals: 'Professionals',
                        homeServices: 'Home Services',
                        neighbourhoodServices: 'Neighbourhood Services',
                        visitorAccess: 'Visitor Access',
                        facilityBookings: 'Facility Bookings',
                        managerRequests: 'Manager Requests',
                        facilities: 'Facilities',
                        announcements: 'Announcements',
                        directories: 'Directories',
                        suggestions: 'Suggestions',
                        complaints: 'Complaints',
                        marketPlaces: 'Marketplaces',
                        marketPlaceBookings: 'Marketplace Bookings',
                        marketPlaceVisits: 'Marketplace Visits',
                        offerRequests: 'Offer Requests',
                        reports: 'Reports',
                        settings: 'Settings',
                        companyProfile: 'Company Profile',
                        invoiceSettings: 'Invoice Settings',
                        leaseSettings: 'Lease Settings',
                    },
                    presets: {
                        admin: 'Admin',
                        manager: 'Manager',
                        accountingManager: 'Accounting Manager',
                        serviceManager: 'Service Manager',
                        owner: 'Owner',
                        tenant: 'Tenant',
                        professional: 'Professional',
                        dependent: 'Dependent',
                        empty: 'No permissions',
                    },
                },
            },
            subscriptions: {
                pageTitle: 'Manage Subscriptions',
                heading: 'Account subscriptions',
                description: 'Manage account subscriptions with the default monthly plan.',
                defaultPlan: 'Default plan',
                planName: 'Plan',
                price: 'Price',
                trial: 'Trial period',
                slug: 'Slug',
                account: 'Account',
                domain: 'Domain',
                status: 'Status',
                periodEndsAt: 'Period ends at',
                actions: 'Actions',
                activate: 'Activate',
                cancelAtEnd: 'Cancel at period end',
                cancelNow: 'Cancel now',
                notSubscribed: 'Not subscribed',
                active: 'Active',
                onTrial: 'On trial',
                canceled: 'Canceled',
                ended: 'Ended',
                inactive: 'Inactive',
                empty: 'No accounts found.',
            },
            documents: {
                pageTitle: 'Document Templates',
                heading: 'Document templates',
                description: 'Create and manage document templates with merge fields for lease contracts, invoices, receipts, and bookings.',
                create: 'Create template',
                edit: 'Edit template',
                nameEn: 'Name (English)',
                nameAr: 'Name (Arabic)',
                type: 'Type',
                format: 'Format',
                status: 'Status',
                version: 'Version',
                versions: 'Versions',
                currentVersion: 'Current version',
                bodyEn: 'Body (English)',
                bodyAr: 'Body (Arabic)',
                mergeFields: 'Merge fields',
                addMergeField: 'Add field',
                mergeFieldKey: 'Field key',
                mergeFieldLabelEn: 'Label (English)',
                mergeFieldLabelAr: 'Label (Arabic)',
                mergeFieldType: 'Type',
                mergeFieldSourcePath: 'Source path',
                removeMergeField: 'Remove',
                activate: 'Activate',
                archive: 'Archive',
                activated: 'Template activated.',
                archived: 'Template archived.',
                created: 'Template created successfully.',
                updated: 'Template updated to version :version.',
                statusDraft: 'Draft',
                statusActive: 'Active',
                statusArchived: 'Archived',
                formatWordUpload: 'Word upload',
                formatInPlatform: 'In-platform editor',
                empty: 'No document templates found. Create your first template to get started.',
            },
            featureFlags: {
                pageTitle: 'Feature Flags — {{tenant}}',
                tabOverview: 'Overview',
                tabFeatures: 'Features',
                tabUsers: 'Users',
                tabSubscription: 'Subscription',
                description: 'Configure which platform features are active for this tenant. Changes take effect immediately on the tenant\'s next page load.',
                colFeature: 'Feature',
                colStatus: 'Status',
                colInTier: 'Included in Plan',
                colToggle: 'Toggle',
                statusEnabled: 'Enabled',
                statusDisabled: 'Disabled',
                enableHeading: 'Enable {{feature}}?',
                enableBody: 'Tenant users will gain access immediately on next page load.',
                enableCta: 'Enable Feature',
                disableHeading: 'Disable {{feature}}?',
                disableImpactLabel: 'Immediate impact',
                disableImpactBody: 'Tenant users currently using {{feature}} will lose access on their next page load. Active sessions will see a \"Feature not available\" message.',
                disableReversible: 'This action is reversible. You can re-enable the feature at any time.',
                disableCta: 'Disable Feature',
                toastEnabled: '{{feature}} enabled',
                toastDisabled: '{{feature}} disabled',
                toggleError: 'Failed to update. Please try again.',
                tierIncluded: 'Enabled by default for the {{plan}} plan.',
                tierNotIncluded: 'Not included in the {{plan}} plan. Manually overridden by operator.',
                emptyHeading: 'No feature flags are defined yet.',
                emptyHeadingAr: 'لا توجد علامات ميزات محددة بعد.',
                emptyBody: 'Feature flags are defined in code and seeded automatically. Contact engineering if a flag is missing.',
                ariaEnable: 'Enable {{feature}}',
                ariaDisable: 'Disable {{feature}}',
            },
        },
        settings: {
            profile: {
                pageTitle: 'Profile settings',
                heading: 'Profile information',
                description: 'Update your name and email address',
                name: 'Name',
                fullName: 'Full name',
                emailAddress: 'Email address',
                emailPlaceholder: 'Email address',
                unverifiedNotice: 'Your email address is unverified.',
                resendVerification: 'Click here to resend the verification email.',
                verificationSent: 'A new verification link has been sent to your email address.',
            },
            security: {
                pageTitle: 'Security settings',
                updatePassword: 'Update password',
                passwordDescription: 'Ensure your account is using a long, random password to stay secure',
                currentPassword: 'Current password',
                currentPasswordPlaceholder: 'Current password',
                newPassword: 'New password',
                newPasswordPlaceholder: 'New password',
                confirmPassword: 'Confirm password',
                confirmPasswordPlaceholder: 'Confirm password',
                savePassword: 'Save password',
                twoFactorTitle: 'Two-factor authentication',
                twoFactorDescription: 'Manage your two-factor authentication settings',
                twoFactorEnableHelp: 'When you enable two-factor authentication, you will be prompted for a secure pin during login. This pin can be retrieved from a TOTP-supported application on your phone.',
                twoFactorEnabledHelp: 'You will be prompted for a secure, random pin during login, which you can retrieve from the TOTP-supported application on your phone.',
                sessions: {
                    title: 'Active sessions',
                    description: 'View and manage devices currently signed into your account.',
                    currentSession: 'Current session',
                    lastActive: 'Last active',
                    revoke: 'Revoke',
                    revokeAll: 'Log out all other sessions',
                    revokeAllSuccess: 'All other sessions have been logged out.',
                    revokeSingleSuccess: 'Session revoked successfully.',
                    onlyOne: 'Only one active session.',
                    driverNotConfigured: 'Session management requires additional configuration. Please contact your administrator.',
                    revokeConfirmTitle: 'Revoke this session?',
                    revokeConfirmBody: 'The device will be logged out and will need to sign in again.',
                    revokeAllConfirmTitle: 'Log out all other sessions?',
                    revokeAllConfirmBody: 'All devices except this one will be logged out immediately.',
                },
            },
            appearance: {
                pageTitle: 'Appearance settings',
                heading: 'Appearance settings',
                description: "Update your account's appearance settings",
                light: 'Light',
                dark: 'Dark',
                system: 'System',
            },
            deleteAccount: {
                heading: 'Delete account',
                description: 'Delete your account and all of its resources',
                warningDescription: 'Please proceed with caution, this cannot be undone.',
                button: 'Delete account',
                dialogTitle: 'Are you sure you want to delete your account?',
                dialogDescription: 'Once your account is deleted, all of its resources and data will also be permanently deleted. Please enter your password to confirm you would like to permanently delete your account.',
                password: 'Password',
                passwordPlaceholder: 'Password',
            },
        },
        dashboard: {
            pageTitle: 'Dashboard',
            communities: 'Communities',
            buildings: 'Buildings',
            units: 'Units',
            tenants: 'Tenants',
            activeLeases: 'Active Leases',
            openRequests: 'Open Requests',
            pendingTransactions: 'Pending Transactions',
            totalRevenue: 'Total Revenue',
            requiresAttention: 'Requires Attention',
            recentLeases: 'Recent Leases',
            recentServiceRequests: 'Recent Service Requests',
            contract: 'Contract',
            tenant: 'Tenant',
            status: 'Status',
            period: 'Period',
            amount: 'Amount',
            id: 'ID',
            category: 'Category',
            priority: 'Priority',
            date: 'Date',
            noLeasesYet: 'No leases yet.',
            noRequestsYet: 'No requests yet.',
        },
        notifications: {
            pageTitle: 'Notifications',
            heading: 'Notifications',
            description: 'Track system activity and mark items as read.',
            unread: 'Unread',
            recent: 'Recent Notifications',
            read: 'Read',
            noNotificationsYet: 'No notifications yet.',
            openCenter: 'Open Notifications Center',
            unreadCountEndpoint: 'Unread Count Endpoint',
        },
        properties: {
            communities: {
                pageTitle: 'Communities',
                heading: 'Communities',
                description: 'Manage your property communities and developments.',
                newCommunity: 'New Community',
                noCommunitiesFound: 'No communities found. Create your first community to get started.',
                table: {
                    name: 'Name',
                    country: 'Country',
                    city: 'City',
                    currency: 'Currency',
                    buildings: 'Buildings',
                    units: 'Units',
                    requests: 'Requests',
                    salesCommission: 'Sales %',
                    rentalCommission: 'Rental %',
                },
                create: {
                    pageTitle: 'New Community',
                    heading: 'Create Community',
                    description: 'Add a new community to your portfolio.',
                    communityName: 'Community Name',
                    communityNamePlaceholder: 'Enter community name',
                    country: 'Country',
                    selectCountry: 'Select country',
                    currency: 'Currency',
                    selectCurrency: 'Select currency',
                    city: 'City',
                    selectCity: 'Select city',
                    district: 'District',
                    selectDistrict: 'Select district',
                    salesCommission: 'Sales Commission (%)',
                    rentalCommission: 'Rental Commission (%)',
                    ratePlaceholder: '0.00',
                    createButton: 'Create Community',
                },
                edit: {
                    breadcrumb: 'Edit',
                    pageTitleWithName: 'Edit {{name}}',
                    description: 'Update community details.',
                    updateButton: 'Update Community',
                    amenities: {
                        sectionTitle: 'Amenities',
                        sectionDescription: 'Select the shared amenities available in this community.',
                        noAmenities: 'No amenities configured',
                        remove: 'Remove {{name}}',
                    },
                    workingDays: {
                        sectionTitle: 'Working Days',
                        sectionDescription: 'Days when community facilities and offices operate.',
                        noDays: 'Not configured',
                        sat: 'Sat',
                        sun: 'Sun',
                        mon: 'Mon',
                        tue: 'Tue',
                        wed: 'Wed',
                        thu: 'Thu',
                        fri: 'Fri',
                    },
                    coordinates: {
                        sectionTitle: 'Map Coordinates',
                        sectionDescription: "Set the community's geographic location.",
                        latitude: 'Latitude',
                        longitude: 'Longitude',
                        latitudePlaceholder: 'e.g. 24.7743',
                        longitudePlaceholder: 'e.g. 46.7386',
                        latitudeHint: 'Range: −90 to 90',
                        longitudeHint: 'Range: −180 to 180',
                        useMyLocation: 'Use my location',
                        permissionDenied: 'Location access was denied by your browser. Enter coordinates manually below.',
                        gettingLocation: 'Getting your location...',
                        locationFilled: 'Location filled',
                    },
                },
                show: {
                    breadcrumb: 'Details',
                    detailsDescription: 'Community details and statistics',
                    confirmDeletePrompt: 'Are you sure you want to delete this community?',
                    buildings: 'Buildings',
                    units: 'Units',
                    requests: 'Requests',
                    location: 'Location',
                    financials: 'Financials',
                    country: 'Country',
                    city: 'City',
                    district: 'District',
                    currency: 'Currency',
                    salesCommission: 'Sales Commission',
                    rentalCommission: 'Rental Commission',
                    marketplace: 'Marketplace',
                    relatedBuildings: 'Buildings',
                    buildingUnits: '{{count}} units',
                    facilities: 'Facilities',
                    amenities: 'Amenities',
                    noAmenities: 'No amenities configured',
                    workingDays: 'Working Days',
                    noDays: '—',
                    coordinates: 'Lat / Lng',
                    noCoordinates: '—',
                    sat: 'Sat',
                    sun: 'Sun',
                    mon: 'Mon',
                    tue: 'Tue',
                    wed: 'Wed',
                    thu: 'Thu',
                    fri: 'Fri',
                    aria: {
                        working_day: 'Working day',
                        non_working_day: 'Non-working day',
                    },
                },
            },
            buildings: {
                pageTitle: 'Buildings',
                heading: 'Buildings',
                description: 'Manage buildings within your communities.',
                newBuilding: 'New Building',
                noBuildingsFound: 'No buildings found.',
                table: {
                    name: 'Name',
                    community: 'Community',
                    city: 'City',
                    floors: 'Floors',
                    units: 'Units',
                    yearBuilt: 'Year Built',
                },
                create: {
                    pageTitle: 'New Building',
                    heading: 'Create Building',
                    description: 'Add a new building to a community.',
                    buildingName: 'Building Name',
                    buildingNamePlaceholder: 'Enter building name',
                    community: 'Community',
                    selectCommunity: 'Select community',
                    city: 'City',
                    selectCity: 'Select city',
                    district: 'District',
                    selectDistrict: 'Select district',
                    floors: 'Number of Floors',
                    floorsPlaceholder: '0',
                    yearBuilt: 'Year Built',
                    yearPlaceholder: '2024',
                    createButton: 'Create Building',
                },
                edit: {
                    breadcrumb: 'Edit',
                    pageTitleWithName: 'Edit {{name}}',
                    description: 'Update building details.',
                    updateButton: 'Update Building',
                },
                show: {
                    breadcrumb: 'Details',
                    confirmDeletePrompt: 'Are you sure you want to delete this building?',
                    units: 'Units',
                    floors: 'Floors',
                    yearBuilt: 'Year Built',
                    relatedUnits: 'Units',
                },
            },
            units: {
                pageTitle: 'Units',
                heading: 'Units',
                description: 'Manage individual units across your properties.',
                newUnit: 'New Unit',
                noUnitsFound: 'No units found.',
                table: {
                    name: 'Name',
                    community: 'Community',
                    building: 'Building',
                    category: 'Category',
                    status: 'Status',
                    floor: 'Floor',
                    area: 'Area (sqm)',
                    owner: 'Owner',
                    tenant: 'Tenant',
                },
                create: {
                    pageTitle: 'New Unit',
                    heading: 'Create Unit',
                    description: 'Add a new unit to your property.',
                    unitName: 'Unit Name',
                    unitNamePlaceholder: 'e.g. A-101',
                    community: 'Community',
                    selectCommunity: 'Select community',
                    building: 'Building',
                    selectBuilding: 'Select building',
                    status: 'Status',
                    selectStatus: 'Select status',
                    owner: 'Owner',
                    selectOwner: 'Select owner',
                    tenant: 'Tenant',
                    selectTenant: 'Select tenant',
                    city: 'City',
                    selectCity: 'Select city',
                    district: 'District',
                    selectDistrict: 'Select district',
                    category: 'Category',
                    selectCategory: 'Select category',
                    type: 'Type',
                    selectType: 'Select type',
                    netArea: 'Net Area (sqm)',
                    areaPlaceholder: '0.00',
                    floorNumber: 'Floor Number',
                    floorPlaceholder: '0',
                    yearBuilt: 'Year Built',
                    yearPlaceholder: '2024',
                    descriptionLabel: 'Description',
                    descriptionPlaceholder: 'Unit description...',
                    createButton: 'Create Unit',
                },
                edit: {
                    breadcrumb: 'Edit',
                    pageTitleWithName: 'Edit {{name}}',
                    description: 'Update unit details.',
                    updateButton: 'Update Unit',
                    specifications: {
                        sectionTitle: 'Specifications',
                        bedrooms: 'Bedrooms',
                        bathrooms: 'Bathrooms',
                        livingRooms: 'Living Rooms',
                        furnished: 'Furnished?',
                        furnishedYes: 'Yes',
                        furnishedNo: 'No',
                        parking: 'Parking',
                        parkingNone: 'None',
                        parkingOneBay: '1 Bay',
                        parkingTwoBays: '2 Bays',
                        view: 'View',
                        viewSea: 'Sea View',
                        viewCity: 'City View',
                        viewGarden: 'Garden View',
                        viewNone: 'No View',
                    },
                    amenities: {
                        sectionTitle: 'Amenities',
                        instruction: 'Select all that apply',
                        pool: 'Pool',
                        gym: 'Gym',
                        security: 'Security',
                        garden: 'Garden',
                        parking: 'Parking',
                        concierge: 'Concierge',
                        rooftop: 'Rooftop',
                        sauna: 'Sauna',
                        kidsPlay: 'Kids Play Area',
                    },
                    pricing: {
                        sectionTitle: 'Pricing Reference',
                        askingRent: 'Asking Rent',
                        selectCurrency: 'Currency',
                        periodYear: 'Year',
                        periodMonth: 'Month',
                        selectPeriod: 'Period',
                    },
                },
                show: {
                    breadcrumb: 'Details',
                    confirmDeletePrompt: 'Are you sure you want to delete this unit?',
                    noBuilding: 'No building',
                    category: 'Category',
                    type: 'Type',
                    status: 'Status',
                    area: 'Area',
                    areaValue: '{{value}} sqm',
                    details: 'Details',
                    floor: 'Floor',
                    yearBuilt: 'Year Built',
                    marketplace: 'Marketplace',
                    occupancy: 'Occupancy',
                    owner: 'Owner',
                    tenant: 'Tenant',
                    descriptionLabel: 'Description',
                    specifications: 'Specifications',
                    furnished: 'Furnished',
                    unfurnished: 'Unfurnished',
                    amenities: 'Amenities',
                    noAmenities: 'No amenities listed',
                    pricing: 'Pricing Reference',
                    askingRent: 'Asking Rent',
                    pricingNotSet: 'Not set',
                    rentPeriodYear: 'Year',
                    rentPeriodMonth: 'Month',
                },
                import: {
                    importUnits: 'Import Units',
                    modalTitle: 'Import Units from Excel',
                    step1: 'Upload',
                    step2: 'Map Columns',
                    step3: 'Review',
                    step4: 'Import',
                    downloadTemplate: 'Download template',
                    dropZoneText: 'Drag & drop your .xlsx file here or click to browse',
                    acceptedFormat: 'Accepted format: .xlsx · Max: 10MB',
                    rowsDetected: '{{n}} rows detected',
                    systemField: 'System Field',
                    uploadedColumn: 'Uploaded Column',
                    matched: 'matched',
                    unmatched: 'unmatched',
                    selectColumn: '-- Select --',
                    validRows: '{{n}} valid',
                    errorRows: '{{n}} with errors',
                    warningRows: '{{n}} warnings',
                    allRowsInvalid: 'All {{n}} rows contain errors. Please fix the file and re-upload.',
                    noDataRows: 'The uploaded file contains no data rows.',
                    importValidOnly: 'Import {{n}} valid rows',
                    cancelAndFix: 'Cancel and fix the file',
                    progressLabel: 'Importing {{n}} units...',
                    progressNote: 'Do not close this window.',
                    completionToast: '{{n}} units imported successfully. {{e}} rows skipped due to errors.',
                    viewReport: 'View report',
                    errorBuildingNotFound: 'Building "{{name}}" not found',
                    errorDuplicate: 'Duplicate unit number in this building',
                    errorInvalidStatus: 'Status "{{name}}" is not valid',
                    errorInvalidFile: 'File must be .xlsx format',
                    errorFileTooLarge: 'File exceeds 10MB limit. Please split your import.',
                    errorRow: 'Row',
                    errorField: 'Field',
                    errorMessage: 'Error',
                    filterAll: 'All rows',
                    filterErrors: 'Errors only',
                    queuedMessage: 'Import is processing in the background. You will be notified on completion.',
                    fieldName: 'Unit Name',
                    fieldCommunity: 'Community',
                    fieldBuilding: 'Building',
                    fieldNetArea: 'Area (sqm)',
                    fieldStatus: 'Status',
                },
            },
        },
        requests: {
            pageTitle: 'Requests',
            heading: 'Service Requests',
            description: 'Track and manage maintenance and service requests.',
            newRequest: 'New Request',
            search: 'Search',
            searchPlaceholder: 'Title, description, code, or ID',
            status: 'Status',
            allStatuses: 'All statuses',
            category: 'Category',
            allCategories: 'All categories',
            priority: 'Priority',
            allPriorities: 'All priorities',
            rowsPerPage: 'Rows Per Page',
            noRequestsFound: 'No requests found.',
            showingSummary: 'Showing {{from}} to {{to}} of {{total}} requests',
            table: {
                id: 'ID',
                category: 'Category',
                subcategory: 'Subcategory',
                status: 'Status',
                community: 'Community',
                created: 'Created',
            },
            create: {
                pageTitle: 'New Request',
                heading: 'Create Request',
                description: 'Submit a new service request.',
                selectCategory: 'Select category',
                selectSubcategory: 'Select subcategory',
                selectCommunity: 'Select community',
                selectUnit: 'Select unit',
                selectPriority: 'Select priority',
                low: 'Low',
                medium: 'Medium',
                high: 'High',
                urgent: 'Urgent',
                descriptionPlaceholder: 'Describe the request...',
                submitButton: 'Submit Request',
            },
            show: {
                breadcrumb: 'Details',
                pageTitle: 'Request #{{id}}',
                confirmDeletePrompt: 'Are you sure?',
                status: 'Status',
                priority: 'Priority',
                created: 'Created',
                location: 'Location',
                community: 'Community',
                unit: 'Unit',
                people: 'People',
                requester: 'Requester',
                assignee: 'Assignee',
                description: 'Description',
            },
        },
        facilities: {
            pageTitle: 'Facilities',
            heading: 'Facilities',
            description: 'Manage facilities and their details.',
            newFacility: 'New Facility',
            createTitle: 'Create Facility',
            createDescription: 'Add a new facility.',
            editTitle: 'Edit Facility',
            editDescription: 'Update facility details.',
            facilityName: 'Facility Name',
            name: 'Name',
            namePlaceholder: 'Enter facility name',
            category: 'Category',
            selectCategory: 'Select category',
            community: 'Community',
            selectCommunity: 'Select community',
            capacity: 'Capacity',
            capacityPlaceholder: 'Enter capacity',
            status: 'Status',
            details: 'Facility Details',
            descriptionLabel: 'Description',
            bookings: 'Bookings',
            createButton: 'Create Facility',
            updateButton: 'Update Facility',
            noFacilitiesFound: 'No facilities found.',
            resident: {
                pageTitle: 'Facilities',
                heading: 'Facilities',
                description: 'Book gym, pool, and more',
                free: 'Free',
                pricePerSession: 'SAR {{n}} / session',
                pricePerHour: 'SAR {{n}} / hour',
                contractRequired: 'Contract required',
                book: 'Book',
                capacityLabel: 'Capacity: {{n}}',
                morning: 'Morning',
                afternoon: 'Afternoon',
                evening: 'Evening',
                available: 'Available',
                availableTap: 'Available · Tap to book',
                booked: 'Booked',
                closed: 'Closed',
                confirmTitle: 'Confirm Booking',
                confirmDate: 'Date',
                confirmTime: 'Time',
                confirmDuration: 'Duration',
                confirmPrice: 'Price',
                confirmTotal: 'Total',
                confirmNote: 'Confirmation will be sent to your registered mobile number and email.',
                confirm: 'Confirm',
                cancel: 'Cancel',
                raceConditionTitle: 'This slot is no longer available',
                raceConditionDetail: 'Another resident just booked it. Please choose a different time.',
                pickAnotherSlot: 'Pick Another Slot',
                contractNotice: 'This facility requires a signed contract. Your booking will be pending until the contract is signed.',
                successToast: 'Booking confirmed! You will receive a confirmation shortly.',
                noFacilitiesAvailable: 'No facilities available in your community yet.',
                allSlotsFull: 'All slots are booked for this date.',
                joinWaitlist: 'Join Waitlist',
                closedOnDay: 'Facility closed on this day. Please select another date.',
                slotUnavailable: 'This slot is no longer available.',
                bookingConfirmed: 'Booking confirmed! You will receive a confirmation shortly.',
                previousWeek: 'Previous week',
                nextWeek: 'Next week',
                selectDate: 'Select date {{day}}, {{date}}',
                slotAriaLabel: '{{time}} - {{status}}',
                loadingFacilities: 'Loading facilities...',
                loadingSlots: 'Loading slots...',
                processing: 'Processing...',
                serverError: 'Something went wrong. Please try again.',
                allSlotsBookedJoinWaitlist: 'All slots are booked for this date.',
            },
            calendar: {
                pageTitle: 'Facility Calendar',
                heading: 'Facility Calendar',
                description: 'Manage all bookings across all facilities',
                allFacilities: 'All Facilities',
                previousWeek: 'Previous Week',
                nextWeek: 'Next Week',
                today: 'Today',
                filterAll: 'All',
                filterConfirmed: 'Confirmed',
                filterCheckedIn: 'Checked-in',
                filterCompleted: 'Completed',
                filterCancelled: 'Cancelled',
                adminReservation: 'Admin Reservation',
                pending: 'Pending (awaiting contract)',
                bookingDetail: 'Booking Detail',
                resident: 'Resident',
                facility: 'Facility',
                date: 'Date',
                time: 'Time',
                duration: 'Duration',
                status: 'Status',
                checkIn: 'Check In',
                cancelBooking: 'Cancel Booking',
                edit: 'Edit',
                createBooking: 'Create Booking',
                residentSearch: 'Search resident by name or unit...',
                start: 'Start',
                end: 'End',
                notes: 'Notes',
                cancel: 'Cancel',
                create: 'Create',
                overlapTitle: 'Overlap Detected',
                overlapMsg: 'This time slot conflicts with an existing booking.',
                overlapAction: 'Please adjust the start/end time.',
                noBookingsThisWeek: 'No bookings for this week.',
                loading: 'Loading...',
                bookingCreated: 'Booking created successfully.',
                legendConfirmed: 'Confirmed',
                legendCheckedIn: 'Checked-in',
                legendPending: 'Pending',
                legendCompleted: 'Completed',
                legendCancelled: 'Cancelled',
            },
        },
        leases: {
            pageTitle: 'Leases',
            heading: 'Leases',
            description: 'Manage rental agreements and contracts.',
            newLease: 'New Lease',
            search: 'Search',
            searchPlaceholder: 'Contract number or tenant name',
            status: 'Status',
            allStatuses: 'All statuses',
            tenant: 'Tenant',
            allTenants: 'All tenants',
            rowsPerPage: 'Rows Per Page',
            noLeasesFound: 'No leases found.',
            showingSummary: 'Showing {{from}} to {{to}} of {{total}} leases',
            table: {
                contract: 'Contract #',
                tenant: 'Tenant',
                status: 'Status',
                type: 'Type',
                startDate: 'Start Date',
                endDate: 'End Date',
                totalAmount: 'Total Amount',
            },
            create: {
                pageTitle: 'New Lease',
                heading: 'Create Lease',
                description: 'Create a new rental agreement.',
                contractNumber: 'Contract Number',
                contractPlaceholder: 'e.g. LC-2024-001',
                tenant: 'Tenant',
                selectTenant: 'Select tenant',
                status: 'Status',
                selectStatus: 'Select status',
                tenantType: 'Tenant Type',
                rentalType: 'Rental Type',
                selectType: 'Select type',
                individual: 'Individual',
                company: 'Company',
                total: 'Total',
                detailed: 'Detailed',
                unitCategory: 'Unit Category',
                selectCategory: 'Select category',
                contractType: 'Contract Type',
                selectContractType: 'Select contract type',
                paymentSchedule: 'Payment Schedule',
                selectSchedule: 'Select schedule',
                startDate: 'Start Date',
                endDate: 'End Date',
                handoverDate: 'Handover Date',
                totalAmount: 'Total Amount',
                amountPlaceholder: '0.00',
                securityDeposit: 'Security Deposit',
                termsAndConditions: 'Terms & Conditions',
                termsPlaceholder: 'Enter lease terms...',
                createButton: 'Create Lease',
            },
            edit: {
                breadcrumb: 'Edit',
                pageTitleWithContract: 'Edit Lease {{contract}}',
                description: 'Update lease details.',
                updateButton: 'Update Lease',
            },
            show: {
                breadcrumb: 'Details',
                pageTitle: 'Lease {{contract}}',
                confirmDeletePrompt: 'Are you sure?',
                createSublease: 'Create Sub-lease',
                parentLease: 'Parent Lease',
                status: 'Status',
                totalAmount: 'Total Amount',
                unpaid: 'Unpaid',
                type: 'Type',
                duration: 'Duration',
                start: 'Start',
                end: 'End',
                handover: 'Handover',
                financial: 'Financial',
                rentalType: 'Rental Type',
                securityDeposit: 'Security Deposit',
                subLease: 'Sub Lease',
                units: 'Units',
                additionalFees: 'Additional Fees',
                escalations: 'Escalations',
                subLeases: 'Sub-leases',
                createdBy: 'Created By',
                dealOwner: 'Deal Owner',
                tenantLabel: 'Tenant',
                unitLabel: 'Unit',
            },
            sublease: {
                pageTitle: 'Create Sub-lease',
                pageTitleWithContract: 'Create Sub-lease for {{contract}}',
                description: 'Inherits unit/category/contract settings from the parent lease.',
                contractNumber: 'Contract Number',
                contractPlaceholder: 'e.g. LC-SUB-2026-001',
                tenant: 'Tenant',
                selectTenant: 'Select tenant',
                status: 'Status',
                selectStatus: 'Select status',
                totalAmount: 'Total Amount',
                startDate: 'Start Date',
                endDate: 'End Date',
                handoverDate: 'Handover Date',
                securityDeposit: 'Security Deposit',
                legalRepresentative: 'Legal Representative',
                fitOutStatus: 'Fit-out Status',
                termsAndConditions: 'Terms & Conditions',
                termsPlaceholder: 'Enter sub-lease terms',
                createButton: 'Create Sub-lease',
            },
            approval: {
                queueTitle: 'Pending Lease Approvals',
                noPending: 'No applications pending review',
                pendingReview: 'This application is pending your review.',
                approve: 'Approve',
                reject: 'Reject',
                rejectTitle: 'Reject Lease Application?',
                rejectDesc: 'This will notify the submitting manager. The linked quote will remain accepted.',
                rejectReason: 'Reason for rejection',
                rejectReasonPlaceholder: 'Describe the reason for rejection...',
                confirmReject: 'Confirm Reject',
                timeline: 'Approval Timeline',
                approvedBy: 'Approved by',
                rejectedBy: 'Rejected by',
                unauthorized: 'You do not have approval rights for this community',
                approvedStatus: 'Approved',
                rejectedStatus: 'Rejected',
                cancel: 'Cancel',
                review: 'Review',
            },
            amend: {
                pageTitle: 'Amend Lease Terms',
                breadcrumb: 'Amend',
                intro: 'Amendable fields. Changes will be recorded in the amendment history.',
                currentLabel: 'Current',
                newLabel: 'New',
                unitNotAmendable: 'Use relocation process',
                diffSection: 'Diff Preview',
                changed: '(changed)',
                unchanged: '(unchanged)',
                reason: 'Reason for Amendment',
                reasonPlaceholder: 'Describe the reason for this amendment...',
                generateAddendum: 'Generate signed addendum after saving',
                save: 'Save Amendment',
                historyTitle: 'Amendment History',
                historyOriginal: 'Original Terms',
                historyDetail: 'Amendment #{{n}}',
                historyMadeBy: 'Made by: {{name}}',
                historyViewAddendum: 'View Addendum',
                historyNoAddendum: 'Not generated',
                historyGenerateNow: 'Generate Addendum',
                historyAddendumSigned: 'Signed',
            },
            notices: {
                title: 'Send Notice',
                breadcrumb: 'Notices',
                to: 'To: {{name}} ({{email}})',
                type: 'Notice Type',
                rentIncrease: 'Rent Increase',
                renewalOffer: 'Renewal Offer',
                moveOutReminder: 'Move-Out Reminder',
                freeForm: 'Free-form Notice',
                subjectEn: 'Subject (English)',
                subjectAr: 'Subject (Arabic)',
                bodyEn: 'Body (English)',
                bodyAr: 'Body (Arabic)',
                preview: 'Preview',
                send: 'Send Notice',
                sent: 'Notice sent to {{email}}',
                historyTitle: 'Notice History',
                historyViewBody: 'View Body',
                historyCount: '{{n}} notices sent',
                errorMissingEmail: 'Tenant email address is missing. Update the Resident contact before sending notices.',
                errorEditContact: 'Edit Contact',
            },
            renewal: {
                bannerTitle: 'Lease Renewal',
                bannerExpiring: 'Lease expires in {{days}} days ({{date}})',
                bannerWindow: 'This lease is within the renewal window.',
                generateOffer: 'Generate Renewal Offer',
                sentStatus: 'Renewal Offer: Sent',
                acceptedStatus: 'Renewal Offer: Accepted',
                expiredStatus: 'Renewal Offer: Expired',
                convertToLease: 'Convert to New Lease',
                viewOffer: 'View Offer',
                recordDecision: 'Record Decision',
                decisionTitle: 'Record Renewal Decision',
                decisionLabel: 'Decision',
                accepted: 'Accepted',
                declined: 'Declined',
                decisionHelp: 'If accepted, you can proceed to create the new lease using the renewal terms.',
                indexTitle: 'Renewal Offers',
                indexDescription: 'Track and manage all lease renewal offers.',
                searchPlaceholder: 'Search by contract number...',
                noOffersFound: 'No renewal offers found.',
                summaryCount: '{{count}} renewal offer(s) in pipeline',
                renewalAmount: 'SAR {{amount}}/{{freq}}',
                validUntil: 'Valid Until',
                createTitle: 'Generate Renewal Offer',
                sourceLease: 'Source Lease',
                currentTerm: 'Current Term',
                currentRent: 'Current Rent',
                currentRentValue: 'SAR {{amount}} / {{freq}}',
                renewalTerms: 'Renewal Terms',
                newStartDate: 'New Start Date',
                newStartDateHint: 'Auto-filled as the day after the lease end date.',
                durationMonths: 'Duration (months)',
                newRent: 'New Rent Amount (SAR)',
                rentDiffSuffix: 'vs. current rent',
                paymentFrequency: 'Payment Frequency',
                contractType: 'Contract Type',
                messageEn: 'Message to Tenant (English)',
                messageAr: 'Message to Tenant (Arabic)',
                saveDraft: 'Save as Draft',
            },
        },
        moveout: {
            initiate: {
                title: 'Initiate Move-Out',
                date: 'Move-Out Date',
                reason: 'Reason',
                reasonEndOfLease: 'End of lease term',
                reasonEarlyTenant: 'Early termination by tenant',
                reasonEarlyMgmt: 'Early termination by management',
                reasonOther: 'Other',
                warning: 'Unit status will change to Under Maintenance. Financial terms will remain locked.',
                button: 'Initiate Move-Out',
            },
            inspection: {
                title: 'Inspection',
                progress: '{{n}}/{{total}} rooms inspected',
                condition: 'Condition',
                conditionExcellent: 'Excellent',
                conditionGood: 'Good',
                conditionFair: 'Fair',
                conditionPoor: 'Poor',
                notes: 'Notes',
                photos: 'Photos',
                addPhoto: 'Add Photo',
                addRoom: 'Add Room',
                save: 'Save Inspection',
                proceedToDeductions: 'Proceed to Deductions',
            },
            deductions: {
                title: 'Deposit Deductions',
                securityDeposit: 'Security Deposit: SAR {{amount}}',
                securityDepositLabel: 'Security Deposit',
                add: 'Add Deduction',
                labelEn: 'Label (English)',
                labelAr: 'Label (Arabic)',
                amount: 'Amount (SAR)',
                reason: 'Reason',
                totalDeductions: 'Total Deductions',
                refundAmount: 'Refund Amount',
                outstandingCharge: 'Outstanding Charge',
                exceedWarning: 'Total deductions exceed the security deposit. Please verify before proceeding.',
                save: 'Save Deductions',
                proceedToSettlement: 'Proceed to Settlement',
            },
            show: {
                noMoveOut: 'No active move-out in progress.',
            },
            flash: {
                initiated: 'Move-out initiated.',
                inspection_saved: 'Inspection saved.',
                deductions_saved: 'Deductions saved.',
            },
            errors: {
                already_initiated: 'A move-out is already in progress for this lease.',
            },
        },
        quotes: {
            pageTitle: 'Lease Quotes',
            heading: 'Lease Quotes',
            description: 'Create and manage lease quotes for prospects.',
            newQuote: 'New Quote',
            search: 'Search',
            searchPlaceholder: 'Quote number or resident name',
            statusLabel: 'Status',
            allStatuses: 'All statuses',
            noQuotesFound: 'No quotes found.',
            showingSummary: 'Showing {{from}} to {{to}} of {{total}} quotes',
            table: {
                quoteNumber: 'Quote #',
                contact: 'Resident',
                unit: 'Unit',
                status: 'Status',
                rentAmount: 'Rent Amount',
                validUntil: 'Valid Until',
            },
            status: {
                draft: 'Draft',
                sent: 'Sent',
                viewed: 'Viewed',
                accepted: 'Accepted',
                rejected: 'Rejected',
                expired: 'Expired',
            },
            create: {
                pageTitle: 'New Quote',
                heading: 'Create Lease Quote',
                description: 'Fill in the details to generate a lease quote for a prospect.',
                unitLabel: 'Unit',
                selectUnit: 'Select unit',
                residentLabel: 'Resident Contact',
                selectContact: 'Select resident',
                contractType: 'Contract Type',
                selectContractType: 'Select contract type',
                duration: 'Duration (months)',
                durationPlaceholder: 'e.g. 12',
                financialTerms: 'Financial Terms',
                rentAmount: 'Rent Amount (SAR)',
                paymentFrequency: 'Payment Frequency',
                selectFrequency: 'Select frequency',
                securityDeposit: 'Security Deposit (SAR)',
                validUntil: 'Valid Until',
                startDate: 'Start Date',
                specialConditionsEn: 'Special Conditions (EN)',
                specialConditionsAr: 'الشروط الخاصة (AR)',
                additionalCharges: 'Additional Charges',
                addCharge: 'Add Charge',
                chargeLabelEn: 'Label (EN)',
                chargeLabelAr: 'Label (AR)',
                chargeLabelPlaceholder: 'e.g. Parking fee',
                chargeAmount: 'Amount (SAR)',
                saveDraft: 'Save as Draft',
                send: 'Send Quote',
            },
            show: {
                pageTitle: 'Quote {{number}}',
                breadcrumb: 'Quote Details',
                quoteDetails: 'Quote Details',
                confirmSend: 'Are you sure you want to send this quote to the prospect?',
                confirmReject: 'Are you sure you want to mark this quote as rejected?',
                confirmExpire: 'Are you sure you want to expire this quote?',
                previewLink: 'Preview',
                months: 'months',
                specialConditions: 'Special Conditions',
                revisionHistory: 'Revision History',
                latestVisible: 'Prospect sees v{{version}}',
                acceptedReadonly: 'This quote has been accepted and cannot be revised.',
                expiredReadonly: 'This quote has expired and cannot be revised.',
                rejectedReadonly: 'This quote has been rejected and cannot be revised.',
            },
            revise: {
                pageTitle: 'Revise Quote',
                heading: 'Revise Quote {{number}}',
                description: 'Update the quote fields. A new version will be created and sent to the prospect.',
                changedBadge: 'CHANGED',
                unchangedBadge: 'UNCHANGED',
                wasValue: 'was {{value}}',
                revisionNote: 'Revision Note',
                revisionNotePlaceholder: 'Describe what changed and why...',
                subjectPrefixLabel: 'Email Subject Prefix',
                subjectPrefixUpdatedQuote: 'Updated Quote',
                subjectPrefixRevisedOffer: 'Revised Offer',
                subjectPrefixCustom: 'Custom',
                saveRevision: 'Save Revision & Send',
                cancel: 'Cancel',
            },
            action: {
                revise: 'Revise Quote',
                markRejected: 'Mark as Rejected',
                expireQuote: 'Expire Quote',
            },
            preview: {
                pageTitle: 'Quote {{number}}',
                heading: 'Lease Quote',
                quoteNumber: 'Quote #{{number}}',
                footer: 'This quote was shared with you securely.',
            },
            convert: {
                pageTitle: 'Convert Quote to Lease Application',
                title: 'Convert Quote to Lease Application',
                source: 'Source: Quote #{{id}} (Accepted)',
                changesDetected: 'Changes detected from quote',
                saveDraft: 'Save as Draft',
                saveGoToKyc: 'Save & Go to KYC',
                contractInfo: 'Contract Info',
                financialTerms: 'Financial Terms (pre-filled from quote)',
                autoGenerateNumber: 'Auto-generate contract number',
            },
        },
        kyc: {
            title: 'KYC Documents',
            pageTitle: 'KYC Documents for {{number}}',
            completion: '{{n}}/{{total}} required',
            requiredSection: 'Required Documents',
            optionalSection: 'Optional Documents',
            upload: 'Upload',
            view: 'View',
            remove: 'Remove',
            submitBlocked: '{{n}} required document(s) missing: {{list}}',
            submitForApproval: 'Submit for Approval',
            noDocumentTypes: 'No document types configured.',
            uploadSuccess: 'Document uploaded.',
            removeSuccess: 'Document removed.',
        },
        transactions: {
            pageTitle: 'Transactions',
            heading: 'Transactions',
            description: 'View and manage financial transactions.',
            newTransaction: 'New Transaction',
            search: 'Search',
            searchPlaceholder: 'ID, lease number, unit, or notes',
            status: 'Status',
            allStatuses: 'All statuses',
            category: 'Category',
            allCategories: 'All categories',
            payment: 'Payment',
            allPayments: 'All',
            paid: 'Paid',
            unpaid: 'Unpaid',
            rowsPerPage: 'Rows Per Page',
            noTransactionsFound: 'No transactions found.',
            showingSummary: 'Showing {{from}} to {{to}} of {{total}} transactions',
            table: {
                id: 'ID',
                lease: 'Lease',
                unit: 'Unit',
                category: 'Category',
                type: 'Type',
                status: 'Status',
                amount: 'Amount',
                dueDate: 'Due Date',
                paid: 'Paid',
            },
            create: {
                breadcrumb: 'New Transaction',
                pageTitle: 'Record a Payment',
                heading: 'Record a Payment',
                description: 'Manually record an offline payment received from a resident or owner.',
                directionLabel: 'Money In',
                payer: 'Payer',
                selectPayer: 'Select payer…',
                residentsGroup: 'Residents',
                ownersGroup: 'Owners',
                paymentMethod: 'Payment Method',
                selectPaymentMethod: 'Select payment method…',
                cash: 'Cash',
                bankTransfer: 'Bank Transfer',
                cheque: 'Cheque',
                paymentDate: 'Payment Date',
                referenceNumber: 'Reference Number (optional)',
                leaseOptional: 'Lease (optional)',
                selectLease: 'Select lease (optional)',
                selectUnit: 'Select unit…',
                selectCategory: 'Select category…',
                saveWithReceipt: 'Save & Generate Receipt',
                saveWithoutReceipt: 'Save Without Receipt',
                cancel: 'Cancel',
                invoiceIncompleteTitle: 'Invoice settings are incomplete',
                invoiceIncompleteMessage: 'Receipt generation is unavailable until branding is configured.',
                configureSettings: 'Configure Settings →',
            },
            edit: {
                breadcrumb: 'Edit',
                pageTitle: 'Edit Transaction',
                heading: 'Edit Transaction',
                description: 'Update transaction details.',
                selectStatus: 'Select status',
                notes: 'Notes',
                notesPlaceholder: 'Additional notes...',
                updateButton: 'Update Transaction',
            },
            show: {
                breadcrumb: 'Details',
                pageTitle: 'Transaction #{{id}}',
                confirmDeletePrompt: 'Are you sure?',
                amount: 'Amount',
                tax: 'Tax',
                status: 'Status',
                due: 'Due',
                related: 'Related',
                lease: 'Lease',
                unit: 'Unit',
                payer: 'Payer',
                payerCard: 'Payer',
                payerName: 'Name',
                paymentMethod: 'Method',
                referenceNumber: 'Reference',
                dates: 'Dates',
                paid: 'Paid',
                created: 'Created',
                receipt: 'Receipt',
                receiptGenerated: 'Receipt Generated',
                settingsIncomplete: 'Settings Incomplete',
                downloadPdf: 'Download PDF',
                sendReceipt: 'Send Receipt',
                resendReceipt: 'Resend Receipt',
                lastSent: 'Last sent: {{date}}',
                invoiceIncompleteMessage: 'Invoice settings are incomplete — please configure branding before sending receipts.',
                configureSettings: 'Configure Settings →',
                sendReceiptTitle: 'Send Receipt',
                sendReceiptBody: 'Send the receipt for Transaction #{{id}} to {{name}} at {{email}}? The receipt PDF will be attached to the email.',
                sendReceiptConfirm: 'Send Receipt',
                sendReceiptCancel: 'Cancel',
                receiptSentSuccess: 'Receipt sent to {{email}}.',
                receiptSendFailed: 'Failed to send receipt. Please try again.',
            },
        },
        accountingSettings: {
            categories: {
                pageTitle: 'Transaction Categories',
                heading: 'Transaction Categories',
                description: 'Configure income and expense categories for transactions.',
                addCategory: 'Add Category',
                tabIncome: 'Income Categories',
                tabExpense: 'Expense Categories',
                colNameEn: 'Name (English)',
                colNameAr: 'Name (Arabic)',
                colStatus: 'Status',
                colActions: 'Actions',
                statusActive: 'Active',
                statusInactive: 'Inactive',
                defaultBadge: 'Default',
                editAction: 'Edit',
                deactivateAction: 'Deactivate',
                reactivateAction: 'Reactivate',
                deleteAction: 'Delete',
                noCategories: 'No categories configured yet. Add your first category.',
                sheetAddTitle: 'Add Category',
                sheetEditTitle: 'Edit Category',
                labelCategoryType: 'Category Type',
                labelNameEn: 'Name (English)',
                labelNameAr: 'Name (Arabic)',
                typeIncome: 'Income',
                typeExpense: 'Expense',
                saveButton: 'Save Category',
                saving: 'Saving...',
                cancelButton: 'Cancel',
                deactivateConfirmTitle: 'Deactivate category?',
                deactivateConfirmBody: 'Existing transactions will retain this category.',
                deactivateConfirmButton: 'Deactivate',
                keepActiveButton: 'Keep Active',
                deleteDefaultError: 'Default categories cannot be deleted; you may deactivate them instead.',
            },
        },
        serviceRequests: {
            pageTitle: 'My Service Requests',
            heading: 'My Service Requests',
            description: 'Track the status of your submitted service requests.',
            newRequest: 'New Request',
            noRequestsYet: "You haven't submitted any requests yet.",
            statusNotConfigured: 'Service request status is not configured. Please contact support.',
            createPageTitle: 'New Service Request',
            createHeading: 'Submit a Service Request',
            createDescription: 'Describe your issue so we can help you quickly.',
            whatNeedsFixing: 'What needs fixing?',
            whereIsTheProblem: 'Where is the problem?',
            howUrgent: 'How urgent is it?',
            describeTheIssue: 'Describe the issue',
            attachPhotos: 'Attach Photos (optional)',
            categoryLabel: 'Category',
            subcategoryLabel: 'Subcategory',
            communityLabel: 'Community',
            unitLabel: 'Unit',
            roomLabel: 'Room / Location',
            urgencyLabel: 'Urgency',
            urgencyNormal: 'Normal',
            urgencyUrgent: 'Urgent',
            descriptionLabel: 'Description',
            descriptionPlaceholder: 'Describe the issue in detail...',
            photoHint: 'JPG, PNG. Max 10 MB each.',
            addPhoto: '+ Add Photo',
            removePhoto: 'Remove',
            submitButton: 'Submit Request',
            submitting: 'Submitting...',
            cancelButton: 'Cancel',
            selectCategory: 'Select a category',
            selectSubcategory: 'Select a subcategory',
            subcategoryDisabled: 'Select a category first',
            selectCommunity: 'Select a community',
            selectUnit: 'Select a unit',
            selectRoom: 'Select room / location',
            confirmedHeading: 'Request Submitted Successfully',
            confirmedDescription: 'You will receive a notification when a technician is assigned and when the work is complete.',
            referenceLabel: 'Your reference number',
            copyButton: 'Copy',
            expectedResponseLabel: 'Expected response by',
            expectedResolutionLabel: 'Expected resolution by',
            viewMyRequests: 'View My Requests',
            submitAnother: 'Submit Another',
            urgencyNormalHint: 'Fix within SLA',
            urgencyUrgentHint: 'Safety / emergency',
            roomKitchen: 'Kitchen',
            roomBathroom: 'Bathroom',
            roomLivingRoom: 'Living Room',
            roomBedroom: 'Bedroom',
            roomBalcony: 'Balcony',
            roomOther: 'Other',
            // Admin triage
            triagePageTitle: 'Service Requests',
            triageHeading: 'Service Requests',
            triageDescription: 'Triage incoming requests, assign technicians and set priority.',
            tabAll: 'All Requests',
            tabUnassigned: 'Unassigned',
            tabOverdue: 'Overdue',
            tabSlaBreach: 'SLA Breach',
            searchPlaceholder: 'Search by ref or resident name',
            filterStatus: 'Status',
            filterCategory: 'Category',
            filterCommunity: 'Community',
            filterUrgency: 'Urgency',
            applyFilters: 'Apply',
            resetFilters: 'Reset',
            colRef: 'Ref#',
            colResident: 'Resident',
            colUnit: 'Unit',
            colCategory: 'Category',
            colUrgency: 'Urgency',
            colStatus: 'Status',
            colSla: 'SLA Response',
            colAssigned: 'Assigned To',
            slaOverdue: 'Overdue',
            slaDueSoon: 'Due soon',
            slaOnTime: 'On time',
            noRequests: 'No service requests yet.',
            noFilteredRequests: 'No requests match your filters.',
            clearFilters: 'Clear filters',
            allAssigned: 'All requests are assigned. Great work!',
            assignTechnicianLabel: 'Assign Technician',
            assignPlaceholder: 'Select technician...',
            priorityLabel: 'Priority',
            priorityLow: 'Low',
            priorityMedium: 'Medium',
            priorityHigh: 'High',
            priorityUrgent: 'Urgent',
            internalNotesLabel: 'Internal Notes',
            notePlaceholder: 'Add internal note (not visible to resident)...',
            addNoteButton: 'Add Note',
            notesHistoryLabel: 'Notes History',
            assignButton: 'Assign & Save',
            assignSuccess: 'Request assigned successfully.',
            noteSuccess: 'Note added successfully.',
            assignError: 'Failed to assign. Please try again.',
            viewButton: 'View',
            backToList: 'Back to Requests',
            detailResident: 'Resident',
            detailLocation: 'Location',
            detailCategory: 'Category',
            detailUrgency: 'Urgency',
            detailStatus: 'Status',
            detailSubmitted: 'Submitted',
            detailDescription: 'Description',
            detailPhotos: 'Attached Photos',
        },
        serviceCategories: {
            pageTitle: 'Service Categories',
            heading: 'Service Categories',
            description: 'Configure categories, subcategories, and SLA targets for service requests.',
            newCategory: '+ New Category',
            newSubcategory: '+ Add Subcategory',
            labelNameEn: 'Name (English)',
            labelNameAr: 'Name (Arabic)',
            labelIcon: 'Choose icon',
            labelResponseSla: 'Response SLA (hours)',
            labelResolutionSla: 'Resolution SLA (hours)',
            labelDefaultAssignee: 'Default handler',
            labelCommunities: 'Available in communities',
            labelRequirePhoto: 'Require completion photo',
            labelStatus: 'Status',
            sheetNewTitle: 'New Category',
            sheetEditTitle: 'Edit Category',
            sheetSubNewTitle: 'New Subcategory',
            sheetSubEditTitle: 'Edit Subcategory',
            saveButton: 'Save Category',
            saveSubButton: 'Save Subcategory',
            saving: 'Saving...',
            cancelButton: 'Cancel',
            inheritedSla: 'Inherited from parent',
            statusActive: 'Active',
            statusInactive: 'Inactive',
            colSubName: 'Name',
            colSubSla: 'SLA',
            colSubActions: 'Actions',
            editAction: 'Edit',
            disableAction: 'Disable',
            enableAction: 'Enable',
            deleteAction: 'Delete',
            noCategories: 'No categories configured. Create your first service category.',
            responseSlaLabel: 'Response {{hours}}h',
            resolutionSlaLabel: 'Resolution {{hours}}h',
            placeholderAssignee: 'Search user...',
            placeholderResponseHours: '4',
            placeholderResolutionHours: '24',
            slaConfigHeading: 'SLA Configuration',
        },
        leads: {
            pageTitle: 'Leads',
            heading: 'Leads',
            description: 'Manage and track your lead pipeline',
            addLead: '+ Add Lead',
            import: 'Import',
            importComingSoon: 'Lead import coming soon',
            noLeadsYet: 'No leads yet',
            noLeadsDescription: 'Start tracking your pipeline by adding your first lead or import an existing list from Excel.',
            importFromExcel: 'Import from Excel',
            downloadTemplate: 'Download Template',
            importLeads: 'Import Leads',
            showingSummary: 'Showing {{from}} to {{to}} of {{total}} leads',
            importSheet: {
                heading: 'Import Leads from Excel',
                fileHint: 'Accepted formats: .xlsx, .xls',
                maxSize: 'Max file size: 5MB',
                maxRows: 'Max rows: 500',
                templateLink: 'Download the import template',
                uploadCta: 'Upload & Preview',
                uploading: 'Uploading…',
                formatError: 'This file cannot be read. Please upload an Excel file (.xlsx or .xls) using the provided template.',
                cancel: 'Cancel',
            },
            review: {
                pageTitle: 'Review Import: {{filename}}',
                heading: 'Validation Results',
                description: 'Review the errors below. Valid rows can still be imported.',
                totalRows: 'Total rows',
                validRows: 'Valid rows',
                errorRows: 'Error rows',
                allValidAlert: 'All {{n}} rows passed validation. Ready to import.',
                allInvalidAlert: 'No valid rows found. Fix the errors and re-upload.',
                downloadErrorReport: 'Download Error Report',
                tableRowNum: 'Row',
                tableField: 'Field',
                tableError: 'Error',
                importCta: 'Import {{n}} valid rows',
                importAllCta: 'Import {{n}} leads',
                importDisabledTooltip: 'No valid rows to import',
                cancelLink: 'Cancel — return to Leads',
                confirmTitle: 'Import valid rows only?',
                confirmBody: 'You are about to import {{n}} valid leads. {{m}} rows with errors will be skipped and not saved. You can re-import them after fixing the errors.',
                confirmCta: 'Import {{n}} leads',
                successToast: '{{n}} leads imported successfully.',
            },
            filter: {
                search: 'Search',
                searchPlaceholder: 'Name or phone…',
                status: 'Status',
                allStatuses: 'All statuses',
                source: 'Source',
                allSources: 'All sources',
                rows: 'Rows',
                apply: 'Apply',
                reset: 'Reset',
            },
            table: {
                name: 'Lead Name',
                phone: 'Phone',
                email: 'Email',
                source: 'Source',
                status: 'Status',
                assignedTo: 'Assigned To',
                createdAt: 'Created At',
                unassigned: '—',
            },
            status: {
                new: 'New',
                contacted: 'Contacted',
                qualified: 'Qualified',
                converted: 'Converted',
                lost: 'Lost',
            },
            drawer: {
                title: 'Add Lead',
                nameEn: 'Lead Name (English)',
                nameAr: 'Lead Name (Arabic)',
                phone: 'Phone Number',
                email: 'Email Address',
                source: 'Source',
                selectSource: 'Select source',
                notes: 'Notes (optional)',
                save: 'Save Lead',
                cancel: 'Cancel',
            },
            toast: {
                added: 'Lead added',
                assigned: 'Lead assigned to {{name}}',
                statusUpdated: 'Status updated to {{status}}',
                noteAdded: 'Note added',
                deleted: 'Lead deleted',
                error: 'Failed to save — please try again',
            },
            detail: {
                pageTitle: '{{name}} — Lead Detail',
                tabDetails: 'Details',
                tabActivity: 'Activity',
                tabActivityCount: 'Activity ({{count}})',
                statusLabel: 'Status',
                statusPlaceholder: 'Select status…',
                lostReasonLabel: 'Lost reason (optional)',
                lostReasonPlaceholder: 'e.g. Budget constraints, not interested…',
                lostReasonCounter: '{{n}} / 500',
                unsavedChanges: 'Unsaved changes',
                discard: 'Discard',
                saveChanges: 'Save Changes',
                assignmentTitle: 'Assignment',
                assignedToLabel: 'Assigned To',
                unassigned: 'Unassigned',
                assignBtn: 'Assign',
                changeBtn: 'Change',
                unassignBtn: 'Unassign',
                sheetTitle: 'Assign Lead',
                sheetSearch: 'Search team members…',
                sheetEmptyDefault: 'Start typing to search team members',
                sheetEmptyNoResults: 'No team members match "{{query}}"',
                sheetCancel: 'Cancel',
                sheetConfirm: 'Assign',
                currentlyAssigned: 'Currently assigned',
                notesTitle: 'Notes',
                notesPlaceholder: 'No notes added',
                contactInfoTitle: 'Contact Info',
                phoneLabel: 'Phone',
                emailLabel: 'Email',
                sourceLabel: 'Source',
                createdLabel: 'Created',
                addNote: 'Add Note',
                composePlaceholder: 'Write a note…',
                composeCancel: 'Cancel',
                composeSave: 'Save Note',
                emptyActivityHeading: 'No activity yet',
                emptyActivityBody: 'Activity events such as status changes, assignment updates, and notes will appear here.',
                emptyActivityCta: 'Add First Note',
                activityAssigned: 'Assigned to {{name}}',
                activityUnassigned: 'Unassigned from {{name}}',
                activityStatusChange: 'Status changed: {{from}} → {{to}}',
                activityNote: 'Note',
                activityActor: 'by {{name}} · {{time}}',
                deleteTitle: 'Delete Lead',
                deleteBody: 'Are you sure you want to delete this lead? This action cannot be undone.',
                deleteConfirm: 'Delete Lead',
                moreActions: 'More actions',
                editLead: 'Edit lead',
                deleteLead: 'Delete lead',
                activityConverted: 'Lead converted to {{type}} contact',
            },
            conversion: {
                convertBtn: 'Convert to Contact',
                rowMenuConvert: 'Convert to Contact',
                drawerTitle: 'Convert to Contact',
                drawerSubtitle: 'Select a contact type and confirm to create the contact record.',
                typeLabel: 'Contact Type',
                typeOwner: 'Owner',
                typeOwnerHint: 'Will be added to Owners in Contacts',
                typeResident: 'Resident',
                typeResidentHint: 'Will be added to Residents in Contacts',
                dataPreviewTitle: 'Data to be transferred',
                fieldFirstName: 'First Name (EN)',
                fieldLastName: 'Last Name (EN)',
                fieldFirstNameAr: 'الاسم الأول (AR)',
                fieldLastNameAr: 'اسم العائلة (AR)',
                fieldEmail: 'Email',
                fieldPhone: 'Phone',
                convertCta: 'Convert',
                convertCtaLoading: 'Converting…',
                cancel: 'Cancel',
                dedupTitle: 'Possible Duplicate Found',
                dedupBody: 'A contact with a matching phone or email already exists.',
                dedupLinkOption: 'Link lead to this existing contact (recommended)',
                dedupCreateOption: 'Create a new contact record anyway',
                dedupCta: 'Continue',
                linkConfirmTitle: 'Link Lead to Contact',
                linkConfirmBody: 'No new contact record will be created.',
                linkConfirmCta: 'Confirm Link',
                createAnywayTitle: 'Create New Contact?',
                createAnywayWarning: 'Creating a new record may result in duplicate contacts in the system.',
                createAnywayCta: 'Create Anyway',
                successCreated: 'Lead converted — contact created.',
                successLinked: 'Lead linked to existing contact.',
                errorFailed: 'Conversion failed. Please try again.',
                errorDedupCheck: 'Could not check for duplicates. Please try again.',
                convertedCardTitle: 'Converted Contact',
                convertedDateLabel: 'Converted',
                convertedViewLink: 'View contact: {{name}} ({{type}})',
            },
        },
        pipeline: {
            title: 'Lease Pipeline',
            expiryWindow: 'Expiry Window',
            expiryFilter30: '30 days',
            expiryFilter60: '60 days',
            expiryFilter90: '90 days',
            allStatuses: 'All statuses',
            allCommunities: 'All communities',
            search: 'Search',
            searchPlaceholder: 'Contract number or tenant name…',
            apply: 'Apply',
            reset: 'Reset',
            groupExpiringSoon: 'Expiring Soon ({{n}})',
            groupActive: 'Active ({{n}})',
            groupExpired: 'Expired ({{n}})',
            groupTerminated: 'Terminated ({{n}})',
            groupPending: 'Pending ({{n}})',
            columnLease: 'Lease#',
            columnUnit: 'Unit',
            columnBuilding: 'Building',
            columnCommunity: 'Community',
            columnTenant: 'Tenant',
            columnStartDate: 'Start',
            columnEndDate: 'End',
            columnRent: 'Rent',
            columnDays: 'Days',
            daysRemaining: '{{n}}d',
            daysExpired: 'Expired',
            countTotal: 'Displaying {{shown}} of {{total}} leases',
            empty: 'No leases match the current filters.',
            clearFilters: 'Clear Filters',
            viewLease: 'View Lease',
            export: 'Export Excel',
            exportTitle: 'Export Lease Pipeline',
            exportMatching: 'Exporting {{n}} leases matching current filter',
            exportColumns: 'Columns included',
            exportFormat: 'Format',
            exportDownload: 'Download',
            exportCancel: 'Cancel',
            settingsLink: 'Settings',
            settings: {
                title: 'Lease Alert Settings',
                description: 'Configure when to receive in-app notifications and emails about upcoming lease expirations.',
                alertThresholds: 'Alert Thresholds',
                dayBefore: '{{n}} days before expiry',
                inApp: 'In-app notification',
                email: 'Email',
                addCustom: 'Add Custom Threshold',
                removeThreshold: 'Remove',
                customDays: 'Days',
                resetDefaults: 'Reset to Defaults',
                save: 'Save Settings',
                saved: 'Alert settings saved.',
                backToPipeline: '← Pipeline',
            },
        },
        contacts: {
            shared: {
                name: 'Name',
                firstName: 'First Name',
                lastName: 'Last Name',
                email: 'Email',
                phone: 'Phone',
                phoneNumber: 'Phone Number',
                phoneNumberPlaceholder: '5XXXXXXXX',
                countryCode: 'Country Code',
                countryCodePlaceholder: 'SA',
                nationalId: 'National ID',
                gender: 'Gender',
                dateOfBirth: 'Date of Birth',
                role: 'Role',
                status: 'Status',
                units: 'Units',
                leases: 'Leases',
                contactInfo: 'Contact Info',
                personalInfo: 'Personal Info',
                info: 'Info',
                lastLogin: 'Last Login',
                select: 'Select',
                selectGender: 'Select gender',
                male: 'Male',
                female: 'Female',
                confirmDeletePrompt: 'Are you sure?',
            },
            tenants: {
                pageTitle: 'Tenants',
                heading: 'Tenants',
                description: 'Manage tenants across your properties.',
                newTenant: 'New Tenant',
                createTitle: 'Create Tenant',
                createDescription: 'Add a new tenant.',
                editTitle: 'Edit {{name}}',
                editDescription: 'Update tenant details.',
                createButton: 'Create Tenant',
                updateButton: 'Update Tenant',
                emptyMessage: 'No tenants found.',
            },
            owners: {
                pageTitle: 'Owners',
                heading: 'Owners',
                description: 'Manage property owners.',
                newOwner: 'New Owner',
                createTitle: 'Create Owner',
                createDescription: 'Add a new property owner.',
                editTitle: 'Edit {{name}}',
                editDescription: 'Update owner details.',
                createButton: 'Create Owner',
                updateButton: 'Update Owner',
                emptyMessage: 'No owners found.',
            },
            admins: {
                pageTitle: 'Admins',
                heading: 'Admins',
                description: 'Manage administrators and managers.',
                newAdmin: 'New Admin',
                createTitle: 'Create Admin',
                createDescription: 'Add a new administrator.',
                editTitle: 'Edit {{name}}',
                editDescription: 'Update admin details.',
                createButton: 'Create Admin',
                updateButton: 'Update Admin',
                emptyMessage: 'No admins found.',
                roles: {
                    admin: 'Admin',
                    accountingManager: 'Accounting Manager',
                    serviceManager: 'Service Manager',
                    marketingManager: 'Marketing Manager',
                    salesLeasingManager: 'Sales & Leasing Manager',
                },
            },
            professionals: {
                pageTitle: 'Professionals',
                heading: 'Professionals',
                description: 'Manage service professionals and technicians.',
                newProfessional: 'New Professional',
                createTitle: 'Create Professional',
                createDescription: 'Add a new service professional.',
                editTitle: 'Edit {{name}}',
                editDescription: 'Update professional details.',
                createButton: 'Create Professional',
                updateButton: 'Update Professional',
                emptyMessage: 'No professionals found.',
            },
            residents: {
                pageTitle: 'Residents',
                heading: 'Residents',
                description: 'All resident contacts in your community.',
                newResident: 'New Resident',
                createTitle: 'New Resident',
                createDescription: 'Fill in the details below to create a new resident contact.',
                createButton: 'Create Resident',
                cancel: 'Cancel',
                emptyMessage: 'No residents found.',
                searchPlaceholder: 'Search by name or phone…',
                firstNameEn: 'First name (EN)',
                lastNameEn: 'Last name (EN)',
                firstNameAr: 'First name (AR)',
                lastNameAr: 'Last name (AR)',
                idType: 'ID type',
                selectIdType: 'Select ID type',
                idNumber: 'ID number',
                duplicateHeading: 'A resident with this phone number already exists.',
                duplicateGoToExisting: 'Go to existing record',
                duplicateConfirmCheckbox: 'I confirm this is a different person and want to create a new record anyway.',
                createdToast: 'Resident created successfully.',
                dependents: 'Dependents',
                idTypes: {
                    national_id: 'National ID',
                    passport: 'Passport',
                    iqama: 'Iqama',
                    emirates_id: 'Emirates ID',
                    other: 'Other',
                },
            },
        },
        appSettings: {
            common: {
                appSettings: 'App Settings',
                actions: 'Actions',
                status: 'Status',
                active: 'Active',
                inactive: 'Inactive',
                save: 'Save',
                cancel: 'Cancel',
                edit: 'Edit',
                delete: 'Delete',
            },
            shell: {
                pageTitle: 'Settings - {{title}}',
                heading: '{{title}}',
                description: 'Manage and update your workspace settings.',
                cardDescription: 'Review and update the selected settings section.',
                tabs: {
                    invoice: 'Invoice',
                    serviceRequest: 'Service Request',
                    visitorRequest: 'Visitor Request',
                    bankDetails: 'Bank Details',
                    visitsDetails: 'Visits Details',
                    salesDetails: 'Sales Details',
                },
                requestTypes: {
                    homeService: 'Home Service',
                    neighbourhoodService: 'Neighbourhood Service',
                },
                invoiceDescription: 'Manage invoice identity and billing information.',
                companyName: 'Company Name',
                companyNamePlaceholder: 'Enter company name',
                address: 'Address',
                addressPlaceholder: 'Enter address',
                vat: 'VAT %',
                vatNumber: 'VAT Number',
                vatNumberPlaceholder: 'Enter VAT number',
                crNumber: 'CR Number',
                crNumberPlaceholder: 'Enter CR number',
                paymentInstructions: 'Payment Instructions',
                paymentInstructionsPlaceholder: 'Enter payment instructions',
                notes: 'Notes',
                invoiceNotesPlaceholder: 'Enter invoice notes',
                saveInvoiceSettings: 'Save Invoice Settings',
                serviceRequestTitle: 'Service Request',
                serviceRequestTypes: 'Service Request Types',
                category: 'Category',
                status: 'Status',
                subcategories: 'Subcategories',
                navigation: 'Navigation',
                details: 'Details',
                openCategory: 'Open Category',
                noServiceRequestCategories: 'No service request categories found.',
                enabled: 'Enabled',
                requirePreApproval: 'Require Pre-Approval',
                maxVisitorsPerRequest: 'Max Visitors Per Request',
                allowedVisitDurationMinutes: 'Allowed Visit Duration (Minutes)',
                saveVisitorRequestSettings: 'Save Visitor Request Settings',
                allDayVisits: 'All Day Visits',
                availableDays: 'Available Days',
                startTime: 'Start Time',
                endTime: 'End Time',
                maxDailyVisits: 'Max Daily Visits',
                saveVisitsDetails: 'Save Visits Details',
                beneficiaryName: 'Beneficiary Name',
                bankName: 'Bank Name',
                accountNumber: 'Account Number',
                iban: 'IBAN',
                saveBankDetails: 'Save Bank Details',
                depositTimeLimitDays: 'Deposit Time Limit (Days)',
                cashContractSigningDays: 'Cash Contract Signing (Days)',
                bankContractSigningDays: 'Bank Contract Signing (Days)',
                saveSalesDetails: 'Save Sales Details',
                unsupportedSettingsTab: 'This settings tab is not configured yet.',
            },
            requestCategories: {
                pageTitle: 'Request Categories',
                heading: 'Request Categories',
                description: 'Manage service request categories and subcategories.',
                newCategory: 'New Category',
                createTitle: 'Create Request Category',
                createDescription: 'Add a new service request category.',
                editTitle: 'Edit {{name}}',
                editHeading: 'Edit: {{name}}',
                editDescription: 'Update category details and manage subcategories.',
                nameEn: 'Name (English)',
                nameAr: 'Name (Arabic)',
                status: 'Status',
                hasSubcategories: 'Has Subcategories',
                subcategories: 'Subcategories',
                subcategoriesCount: 'Subcategories',
                createButton: 'Create Category',
                updateButton: 'Update Category',
                addSubcategory: 'Add Subcategory',
                addSubcategoryButton: 'Add Subcategory',
                cancelSubcategory: 'Cancel',
                allDay: 'All Day',
                startTime: 'Start Time',
                endTime: 'End Time',
                addSubcategorySubmit: 'Add Subcategory',
                serviceSettingsTitle: 'Service Settings Permissions',
                serviceSettingsDescription: 'Configure request handling permissions for this category.',
                managerCloseRequest: 'Manager can close request',
                attachmentsRequired: 'Attachments required',
                allowProfessionalReschedule: 'Allow professional reschedule',
                skipRequestCodeForProfessional: 'Skip request code for professional',
                skipRequestPhotoForProfessional: 'Skip request photo upload for professional',
                saveServiceSettings: 'Save Service Settings',
                noSubcategories: 'No subcategories.',
                noCategories: 'No categories yet.',
                deleteCategoryConfirm: 'Are you sure you want to delete this category?',
                deleteSubcategoryConfirm: 'Delete this subcategory?',
                subcategoryNamePlaceholder: 'Subcategory name',
                subcategoryNameArPlaceholder: 'Subcategory Arabic name',
            },
            facilityCategories: {
                pageTitle: 'Facility Categories',
                heading: 'Facility Categories',
                description: 'Manage facility category types.',
                newCategory: 'New Category',
                createTitle: 'Create Facility Category',
                createDescription: 'Add a new facility category.',
                editTitle: 'Edit {{name}}',
                editHeading: 'Edit: {{name}}',
                editDescription: 'Update facility category.',
                nameEn: 'Name (English)',
                nameAr: 'Name (Arabic)',
                facilities: 'Facilities',
                createButton: 'Create Category',
                updateButton: 'Update Category',
                noCategories: 'No categories yet.',
                deleteCategoryConfirm: 'Are you sure you want to delete this category?',
            },
            facilities: {
                pageTitle: 'Facilities Settings',
                heading: 'Facilities Settings',
                description: 'Manage facilities and booking readiness.',
                addFacility: 'Add Facility',
                editFacility: 'Edit Facility',
                formDescription: 'Configure facility details and booking rules.',
                createFacility: 'Create Facility',
                updateFacility: 'Update Facility',
                name: 'Name',
                nameEn: 'Name (English)',
                nameAr: 'Name (Arabic)',
                category: 'Category',
                selectCategory: 'Select category',
                community: 'Community',
                selectCommunity: 'Select community',
                descriptionLabel: 'Description',
                capacity: 'Capacity',
                open: 'Open Time',
                close: 'Close Time',
                bookingFee: 'Booking Fee',
                requiresApproval: 'Requires Approval',
                status: 'Status',
                bookings: 'Bookings',
                actions: 'Actions',
                view: 'View',
                empty: 'No facilities found.',
                facilityTitle: 'Facility - {{name}}',
                detailsDescription: 'Review facility information and recent bookings.',
                details: 'Facility Details',
                approval: 'Approval',
                required: 'Required',
                notRequired: 'Not Required',
                recentBookings: 'Recent Bookings',
                bookingTitle: 'Booking #{{id}}',
                to: 'to',
                guests: 'Guests',
                notes: 'Notes',
                noBookings: 'No recent bookings.',
            },
            serviceRequestDetails: {
                pageTitle: 'Service Request - {{name}}',
                heading: 'Service Request - {{name}}',
                description: 'Review category subcategories and permission settings.',
                details: 'Details',
                type: 'Type',
                categoryCode: 'Category Code',
                backToSettings: 'Back to Settings',
                openCategoryEditor: 'Open Category Editor',
                subcategories: 'Subcategories',
                name: 'Name',
                status: 'Status',
                noSubcategories: 'No subcategories found.',
                permissionsTitle: 'Permissions',
                enabled: 'Enabled',
                disabled: 'Disabled',
                noPermissions: 'No permissions configured.',
            },
            general: {
                pageTitle: 'General Settings',
                heading: 'General Settings',
                description: 'Manage lookup tables for contract types, payment schedules, and transaction categories.',
                addSetting: 'Add Setting',
                cancelAdd: 'Cancel',
                nameEn: 'Name (English)',
                nameAr: 'Name (Arabic)',
                type: 'Type',
                selectType: 'Select type',
                settingNamePlaceholder: 'Setting name',
                settingNameArPlaceholder: 'Setting Arabic name',
                addSettingSubmit: 'Add Setting',
                noSettings: 'No settings configured yet. Add your first setting above.',
                rentalContractTypes: 'Rental Contract Types',
                paymentSchedules: 'Payment Schedules',
                transactionCategories: 'Transaction Categories',
                transactionTypes: 'Transaction Types',
                calculationBasis: 'Calculation Basis',
                fitOutStatus: 'Fit Out Status',
                paymentFrequency: 'Payment Frequency',
                unknownType: 'Other ({{type}})',
                deleteSettingConfirm: 'Delete this setting?',
            },
        },
        settingsForms: {
            pageTitle: 'Settings Forms',
            formTemplates: 'Form Templates',
            description: 'Create and manage simple request forms for your team.',
            createTemplate: 'Create Template',
            templates: 'Templates',
            name: 'Name',
            category: 'Category',
            communityBuilding: 'Community / Building',
            status: 'Status',
            actions: 'Actions',
            preview: 'Preview',
            empty: 'No form templates found.',
            editTemplate: 'Edit Template',
            createDescription: 'Set up this form in three quick steps: details, scope, and questions.',
            simpleFlowHint: 'Tip: keep it short. Add only the questions people really need to answer.',
            selectCommunity: 'Choose Community',
            selectBuilding: 'Choose Building',
            updateTemplate: 'Update Template',
            newTemplate: 'New Template',
            basicDetails: '1. Basic details',
            basicDetailsDescription: 'Give the form a clear name and a short description.',
            whereUsed: '2. Where this form is used (optional)',
            whereUsedDescription: 'Choose category, community, and building only if you want to limit where this form appears.',
            templateName: 'Form Name',
            descriptionLabel: 'Short Description',
            requestCategory: 'Request Category',
            selectCategory: 'Choose category (optional)',
            community: 'Community',
            building: 'Building',
            selectCommunityFirst: 'Choose a community first',
            schemaJson: 'Schema (JSON)',
            formBuilder: '3. Questions',
            formBuilderDescription: 'Add the questions people will fill in.',
            addField: 'Add Question',
            addFirstField: 'Add Your First Question',
            fieldPosition: 'Question {{number}}',
            fieldLabel: 'Question Text',
            fieldKey: 'Field Key',
            fieldType: 'Answer Type',
            fieldPlaceholder: 'Example Answer (Optional)',
            fieldKeyAutoGenerated: 'No technical setup needed. Field IDs are generated automatically.',
            moveFieldUp: 'Move Up',
            moveFieldDown: 'Move Down',
            removeField: 'Remove Question',
            fieldTypeText: 'Text',
            fieldTypeTextarea: 'Long Text',
            fieldTypeNumber: 'Number',
            fieldTypeEmail: 'Email',
            fieldTypeTel: 'Phone',
            fieldTypeDate: 'Date',
            activeTemplate: 'Enable This Form',
            schemaMustBeValidJson: 'Schema must be valid JSON.',
            templatePreviewTitle: 'Template Preview - {{name}}',
            previewTitle: 'Template Preview - {{name}}',
            previewDescription: 'Review template configuration and fields.',
            templateDetails: 'Template Details',
            fieldsPreview: 'Fields Preview',
            type: 'Type',
            required: 'Must Be Answered',
            fieldFallback: 'Field {{number}}',
            noFieldsConfigured: 'No questions added yet.',
            requiredFields: 'Required Fields',
            noRequiredFields: 'No required fields.',
            selectCommunityDescription: 'Choose a community to start creating a form for it.',
            communities: 'Communities',
            select: 'Select',
            noCommunities: 'No communities found.',
            selectBuildingDescription: 'Choose a community and building, then continue to form setup.',
            buildingScope: 'Building Scope',
            useInTemplate: 'Use This Building',
            noBuildingsForCommunity: 'No buildings found for this community.',
        },
        companyProfile: {
            pageTitle: 'Company Profile',
            heading: 'Company Profile',
            description: 'Configure your company identity, logo, and brand settings that appear on all contracts, invoices, and the login page.',
            sectionIdentity: 'Identity',
            sectionLogos: 'Logo & Brand',
            sectionRegional: 'Regional',
            sectionBrandColors: 'Brand Colors',
            nameEn: 'Company name (English)',
            nameEnPlaceholder: 'Enter company name in English',
            nameAr: 'Company name (Arabic)',
            nameArPlaceholder: 'Enter company name in Arabic',
            vatNumber: 'VAT registration number',
            vatNumberPlaceholder: 'Enter 15-digit VAT number',
            vatHelper: 'Appears on all generated invoices.',
            crNumber: 'CR number (optional)',
            crNumberPlaceholder: 'Enter CR number',
            primaryLogo: 'Primary logo',
            logoHint: 'This logo appears on all lease contracts and invoices.',
            uploadLogo: 'Upload logo',
            changeLogo: 'Change',
            removeLogo: 'Remove',
            uploadHelper: 'PNG or SVG, ≤ 2 MB',
            logoArVariant: 'Arabic logo variant (optional)',
            logoArHint: 'Used on Arabic-language documents. Falls back to the primary logo if not uploaded.',
            timezone: 'Timezone',
            timezoneHelper: 'Affects all timestamps: lease start dates, invoice due dates, and booking slots. Default is UTC.',
            timezoneSearch: 'Search timezones…',
            timezoneGroupGulf: 'Gulf',
            primaryColor: 'Primary brand color',
            colorHint: 'Applied to email notification headers and document template accents. Does not change the platform sidebar color.',
            hexLabel: 'Hex color',
            emptyState: 'Set up your company profile to get started. Your company name, logo, and VAT number will appear on all generated documents.',
            noLogo: 'No logo uploaded',
            unsavedChanges: 'Unsaved changes',
            discard: 'Discard',
            saveChanges: 'Save changes',
            saving: 'Saving…',
            toastSuccess: 'Company profile saved successfully.',
            toastError: 'Something went wrong. Your changes could not be saved.',
            retryMessage: 'Please try again or contact support if the problem persists.',
            validationNameEnRequired: 'Company name (English) is required.',
            validationNameArRequired: 'Company name (Arabic) is required.',
            validationVatFormat: 'VAT number must be 15 digits.',
            validationLogoType: 'The file must be a PNG or SVG image.',
            validationLogoSize: 'The file size must not exceed 2 MB.',
        },
    },
};

const appAr: MessageTree = {
    app: {
        language: {
            current: 'اللغة',
            english: 'الإنجليزية',
            arabic: 'العربية',
        },
        common: {
            warning: 'تنبيه',
            somethingWentWrong: 'حدث خطأ ما.',
            notAvailable: 'غير متوفر',
            orReturnTo: 'أو العودة إلى',
            orYouCan: 'أو يمكنك',
            yes: 'نعم',
            no: 'لا',
            active: 'نشط',
            inactive: 'غير نشط',
            pageSummary: 'الصفحة {{current}} من {{last}}',
            field: 'الحقل',
            optional: 'اختياري',
            cancel: 'إلغاء',
            save: 'حفظ',
            notes: 'ملاحظات',
            noResults: 'لا توجد نتائج.',
            search: 'بحث',
            all: 'الكل',
            saving: 'جارٍ الحفظ...',
            select: 'اختر',
        },
        actions: {
            create: 'إنشاء',
            save: 'حفظ',
            cancel: 'إلغاء',
            back: 'رجوع',
            next: 'التالي',
            close: 'إغلاق',
            edit: 'تعديل',
            delete: 'حذف',
            apply: 'تطبيق',
            reset: 'إعادة تعيين',
            logout: 'تسجيل الخروج',
            markAllAsRead: 'تحديد الكل كمقروء',
            markRead: 'تحديد كمقروء',
            continue: 'متابعة',
            resendVerificationEmail: 'إعادة إرسال بريد التحقق',
            enable2fa: 'تفعيل التحقق الثنائي',
            disable2fa: 'إيقاف التحقق الثنائي',
            continueSetup: 'متابعة الإعداد',
        },
        navigation: {
            modules: 'الوحدات',
            platform: 'المنصة',
            dashboard: 'لوحة التحكم',
            properties: 'العقارات',
            communities: 'المشاريع',
            buildings: 'المباني',
            units: 'الوحدات',
            leasing: 'التأجير',
            leases: 'العقود',
            leads: 'العملاء المحتملون',
            requests: 'الطلبات',
            allRequests: 'كل الطلبات',
            marketplace: 'السوق',
            overview: 'نظرة عامة',
            customers: 'العملاء',
            listing: 'القوائم',
            visits: 'الزيارات',
            facilities: 'المرافق',
            bookings: 'الحجوزات',
            accounting: 'المحاسبة',
            transactions: 'المعاملات',
            communication: 'التواصل',
            announcements: 'الإعلانات',
            contacts: 'جهات الاتصال',
            owners: 'الملاك',
            tenants: 'المستأجرون',
            admins: 'المديرون',
            professionals: 'المختصون',
            visitorAccess: 'دخول الزوار',
            visitorAccessHistory: 'سجل دخول الزوار',
            reports: 'التقارير',
            systemReports: 'تقارير النظام',
            appSettings: 'إعدادات التطبيق',
            settingsShell: 'واجهة الإعدادات',
            settingsFacilities: 'إعدادات المرافق',
            settingsForms: 'إعدادات النماذج',
            requestCategories: 'فئات الطلبات',
            facilityCategories: 'فئات المرافق',
            invoiceSettings: 'إعدادات الفواتير',
            companyProfile: 'ملف الشركة',
            generalSettings: 'الإعدادات العامة',
            userManagement: 'إدارة المستخدمين',
            accountSubscriptions: 'اشتراكات الحسابات',
            documentTemplates: 'قوالب المستندات',
            shared: 'المشترك',
            notifications: 'الإشعارات',
            repository: 'المستودع',
            documentation: 'التوثيق',
            settings: 'الإعدادات',
            profile: 'الملف الشخصي',
            security: 'الأمان',
            appearance: 'المظهر',
            settingsDescription: 'إدارة الملف الشخصي وإعدادات الحساب',
            settingsAriaLabel: 'الإعدادات',
        },
        auth: {
            login: {
                headTitle: 'تسجيل الدخول',
                layoutTitle: 'تسجيل الدخول إلى حسابك',
                layoutDescription: 'أدخل البريد الإلكتروني وكلمة المرور أدناه لتسجيل الدخول',
                emailAddress: 'البريد الإلكتروني',
                password: 'كلمة المرور',
                forgotPassword: 'نسيت كلمة المرور؟',
                rememberMe: 'تذكرني',
                submit: 'تسجيل الدخول',
                noAccount: 'ليس لديك حساب؟',
                signUp: 'إنشاء حساب',
                emailPlaceholder: 'email@example.com',
                invalidCredentials: 'بيانات الاعتماد هذه غير متطابقة مع سجلاتنا.',
                throttled: 'لقد تجاوزت الحد المسموح به من محاولات تسجيل الدخول. يرجى المحاولة مرة أخرى خلال {{seconds}} ثانية.',
                showPassword: 'إظهار كلمة المرور',
                hidePassword: 'إخفاء كلمة المرور',
            },
            register: {
                headTitle: 'إنشاء حساب',
                layoutTitle: 'إنشاء حساب جديد',
                layoutDescription: 'أدخل بياناتك أدناه لإنشاء حسابك',
                name: 'الاسم',
                fullName: 'الاسم الكامل',
                personalInformation: 'معلومات المستخدم',
                firstName: 'الاسم الأول',
                firstNamePlaceholder: 'الاسم الأول',
                lastName: 'اسم العائلة',
                lastNamePlaceholder: 'اسم العائلة',
                emailAddress: 'البريد الإلكتروني',
                phoneNumber: 'رقم الهاتف',
                phoneNumberPlaceholder: '+966500000000',
                tenantInformation: 'معلومات الحساب',
                accountName: 'اسم الحساب',
                accountNamePlaceholder: 'اسم الحساب',
                trialTitle: 'تفاصيل الفترة التجريبية',
                trialDetails: 'ابدأ بفترة تجريبية مجانية لمدة 14 يوماً، ثم استمر بسعر 300 ريال سعودي شهرياً.',
                acceptTerms: 'أوافق على الشروط والأحكام.',
                password: 'كلمة المرور',
                confirmPassword: 'تأكيد كلمة المرور',
                submit: 'إنشاء الحساب',
                hasAccount: 'لديك حساب بالفعل؟',
                logIn: 'تسجيل الدخول',
                emailPlaceholder: 'email@example.com',
            },
            registerInvite: {
                layoutTitle: 'إنشاء حسابك',
                layoutDescription: 'أدخل رمز الدعوة وحدد كلمة المرور',
                inviteCode: 'رمز الدعوة',
                inviteCodePlaceholder: 'أدخل رمز الدعوة',
                password: 'كلمة المرور',
                confirmPassword: 'تأكيد كلمة المرور',
                submit: 'إنشاء حساب',
                hasAccount: 'هل لديك حساب؟',
                logIn: 'تسجيل الدخول',
            },
            forgotPassword: {
                headTitle: 'نسيت كلمة المرور',
                layoutTitle: 'نسيت كلمة المرور',
                layoutDescription: 'أدخل بريدك الإلكتروني لاستلام رابط إعادة تعيين كلمة المرور',
                emailAddress: 'البريد الإلكتروني',
                emailPlaceholder: 'email@example.com',
                submit: 'إرسال رابط إعادة التعيين',
                logIn: 'تسجيل الدخول',
            },
            resetPassword: {
                headTitle: 'إعادة تعيين كلمة المرور',
                layoutTitle: 'إعادة تعيين كلمة المرور',
                layoutDescription: 'يرجى إدخال كلمة المرور الجديدة أدناه',
                email: 'البريد الإلكتروني',
                password: 'كلمة المرور',
                confirmPassword: 'تأكيد كلمة المرور',
                resetPassword: 'إعادة تعيين كلمة المرور',
            },
            verifyEmail: {
                headTitle: 'التحقق من البريد الإلكتروني',
                layoutTitle: 'تحقق من بريدك الإلكتروني',
                layoutDescription: 'أرسلنا رابط التحقق إلى {email}',
                instructions: 'تحقق من صندوق الوارد واضغط على الرابط لتفعيل حسابك.',
                success: 'تم إرسال رابط تحقق جديد.',
            },
            confirmPassword: {
                headTitle: 'تأكيد كلمة المرور',
                layoutTitle: 'تأكيد كلمة المرور',
                layoutDescription: 'هذه منطقة آمنة في التطبيق. يرجى تأكيد كلمة المرور قبل المتابعة.',
                password: 'كلمة المرور',
                submit: 'تأكيد كلمة المرور',
            },
            twoFactor: {
                headTitle: 'التحقق الثنائي',
                recoveryCode: 'رمز الاستعادة',
                recoveryDescription: 'يرجى تأكيد الوصول إلى حسابك بإدخال أحد رموز الاستعادة الاحتياطية.',
                authenticationCode: 'رمز التحقق',
                authenticationDescription: 'أدخل رمز التحقق من تطبيق المصادقة على هاتفك.',
                useAuthenticationCode: 'تسجيل الدخول باستخدام رمز التحقق',
                useRecoveryCode: 'تسجيل الدخول باستخدام رمز الاستعادة',
                recoveryPlaceholder: 'أدخل رمز الاستعادة',
            },
            setPassword: {
                headTitle: 'تعيين كلمة المرور',
                layoutTitle: 'تعيين كلمة المرور',
                layoutDescription: 'عيّن كلمة مرور لتفعيل حسابك.',
                password: 'كلمة المرور',
                confirmPassword: 'تأكيد كلمة المرور',
                submit: 'تعيين كلمة المرور وتسجيل الدخول',
                expiredToken: 'انتهت صلاحية رابط الدعوة أو تم استخدامه مسبقاً. تواصل مع المسؤول لإصدار دعوة جديدة.',
            },
        },
        admin: {
            users: {
                pageTitle: 'إدارة المستخدمين',
                heading: 'إدارة مستخدمي الحساب',
                description: 'إنشاء وإدارة المستخدمين بصلاحيات مبنية على الأدوار لهذا الحساب.',
                currentAccount: 'الحساب الحالي',
                name: 'الاسم',
                email: 'البريد الإلكتروني',
                role: 'الدور',
                password: 'كلمة المرور',
                passwordConfirmation: 'تأكيد كلمة المرور',
                addUser: 'إضافة مستخدم',
                remove: 'إزالة',
                actions: 'الإجراءات',
                empty: 'لا يوجد مستخدمون في الحساب.',
                showPageTitle: 'تفاصيل المستخدم',
                backToUsers: '→ المستخدمون',
                editUser: 'تعديل المستخدم',
                tabRoles: 'الأدوار',
                tabOverview: 'نظرة عامة',
                tabActivity: 'النشاط',
                assignRoleBtn: 'تعيين دور',
                drawerTitle: 'تعيين دور',
                roleLabel: 'الدور',
                rolePlaceholder: 'اختر دوراً…',
                communityLabel: 'المجتمع',
                communityPlaceholder: 'اختر المجتمعات…',
                buildingLabel: 'المبنى',
                buildingPlaceholder: 'اختر المباني…',
                buildingOptionalHint: 'اختياري — اتركه فارغاً للتطبيق على جميع المباني في المجتمعات المختارة.',
                serviceTypeLabel: 'نوع الخدمة',
                serviceTypePlaceholder: 'اختر أنواع الخدمات…',
                globalScopeNote: 'يُطبَّق هذا الدور على نطاق عام. لا يلزم تحديد نطاق.',
                assignBtn: 'تعيين',
                cancelBtn: 'إلغاء',
                cancelConfirm: 'إلغاء التغييرات؟',
                colRole: 'الدور',
                colScope: 'النطاق',
                colAssigned: 'تاريخ التعيين',
                colActions: 'الإجراءات',
                removeTooltip: 'إزالة التعيين',
                removePopoverTitle: 'إزالة هذا التعيين؟',
                removePopoverBody: 'لا يمكن التراجع عن هذا الإجراء.',
                removePopoverConfirm: 'إزالة',
                removePopoverCancel: 'إلغاء',
                emptyRolesHeading: 'لا توجد أدوار مُعيَّنة بعد',
                emptyRolesBody: 'ليس لدى هذا المستخدم أي تعيينات أدوار نشطة.',
                toastAssigned: 'تم تعيين الدور بنجاح.',
                toastRemoved: 'تمت إزالة التعيين بنجاح.',
                validationRoleRequired: 'الرجاء اختيار دور.',
                validationCommunityRequired: 'الرجاء اختيار مجتمع واحد على الأقل لهذا الدور.',
                validationServiceTypeRequired: 'الرجاء اختيار نوع خدمة واحد على الأقل لهذا الدور.',
                status: 'الحالة',
                statusActive: 'نشط',
                statusInvitationPending: 'في انتظار الدعوة',
                statusDeactivated: 'معطّل',
                inviteTitle: 'دعوة مستخدم',
                inviteUser: 'دعوة مستخدم',
                firstName: 'الاسم الأول',
                lastName: 'اسم العائلة',
                firstNamePlaceholder: 'مثال: جانا',
                lastNamePlaceholder: 'مثال: العلي',
                emailPlaceholder: 'مثال: name@company.com',
                sendInvitation: 'إرسال الدعوة',
                deactivate: 'إلغاء التنشيط',
                reactivate: 'إعادة التنشيط',
                sendPasswordReset: 'إرسال إعادة تعيين كلمة المرور',
                resendInvitation: 'إعادة الإرسال',
                revokeInvitation: 'إلغاء',
                viewDetails: 'عرض التفاصيل',
                moreActions: 'المزيد من الخيارات',
                cannotDeactivateSelf: 'لا يمكنك إلغاء تنشيط حسابك.',
                deactivatedBanner: 'حساب هذا المستخدم معطّل. لا يمكنه تسجيل الدخول حتى يُعيد أحد المسؤولين تنشيطه.',
                deactivateConfirmTitle: 'تعطيل الحساب؟',
                deactivateConfirmBody: 'سيفقد هذا المستخدم وصوله فوراً ولن يتمكن من تسجيل الدخول. يمكنك إعادة تنشيطه في أي وقت.',
                deactivateConfirmAction: 'تعطيل',
            },
            roles: {
                pageTitle: 'الأدوار — الإدارة',
                heading: 'الأدوار',
                description: 'إدارة الأدوار لهذا الحساب.',
                breadcrumb: 'الأدوار',
                search: 'البحث في الأدوار…',
                typeLabel: 'النوع',
                typeAll: 'الكل',
                typeUserRole: 'دور مستخدم',
                typeAdminRole: 'دور مسؤول',
                newRole: '+ دور جديد',
                colNameEn: 'الاسم (بالإنجليزية)',
                colNameAr: 'الاسم (بالعربية)',
                colType: 'النوع',
                colUsers: 'المستخدمون',
                colActions: 'الإجراءات',
                systemBadge: 'نظامي',
                ariaEditRole: 'تعديل الدور',
                ariaDeleteRole: 'حذف الدور',
                editTooltip: 'لا يمكن تعديل الأدوار النظامية.',
                deleteTooltip: 'لا يمكن حذف الأدوار النظامية.',
                drawerCreateTitle: 'دور جديد',
                drawerEditTitle: 'تعديل الدور',
                nameEnLabel: 'اسم الدور (بالإنجليزية)',
                nameArLabel: 'اسم الدور (بالعربية)',
                typeImmutableHint: 'لا يمكن تغيير نوع الدور بعد الإنشاء.',
                cancel: 'إلغاء',
                createRole: 'إنشاء الدور',
                saveChanges: 'حفظ التغييرات',
                saving: 'جارٍ الحفظ…',
                deleteDialogTitle: 'حذف الدور؟',
                deleteDialogBody: 'سيؤدي ذلك إلى إزالة "{name}" نهائيًا. سيفقد المستخدمون المعيّنون لهذا الدور صلاحياتهم.',
                deleteDialogConfirm: 'حذف الدور',
                deleteDialogCancel: 'إلغاء',
                emptyHeading: 'لم يتم العثور على أدوار.',
                emptyBody: 'حاول تعديل البحث أو الفلتر، أو أنشئ أول دور مخصص لك.',
                emptyCta: '+ دور جديد',
                toastCreated: 'تم إنشاء الدور بنجاح.',
                toastUpdated: 'تم تحديث الدور.',
                toastDeleted: 'تم حذف الدور.',
                toastError: 'حدث خطأ ما. يرجى المحاولة مجددًا.',
                managePermissions: 'إدارة الصلاحيات',
                permissions: {
                    pageTitle: 'الصلاحيات — {{name}}',
                    tab: 'الصلاحيات',
                    systemBanner: 'لا يمكن تعديل أدوار النظام. تُدار هذه الصلاحيات من قِبَل المنصة.',
                    applyPreset: 'تطبيق إعداد مسبق',
                    presetPlaceholder: 'اختر إعداداً مسبقاً…',
                    presetTooltip: 'تملأ الإعدادات المسبقة مجموعة صلاحيات قياسية. يمكنك تعديل الخلايا الفردية بعد التطبيق.',
                    viewRequiredTooltip: 'العرض مطلوب عند تفعيل أي إجراء آخر.',
                    allSubjects: 'جميع الموضوعات',
                    all: 'الكل',
                    unsavedChanges: 'تغييرات غير محفوظة',
                    savePermissions: 'حفظ الصلاحيات',
                    cancel: 'إلغاء',
                    saving: 'جارٍ الحفظ…',
                    permissionsSaved: 'تم حفظ الصلاحيات',
                    saveError: 'تعذّر حفظ الصلاحيات. يرجى المحاولة مجدداً.',
                    actions: {
                        VIEW: 'عرض',
                        CREATE: 'إنشاء',
                        UPDATE: 'تعديل',
                        DELETE: 'حذف',
                        RESTORE: 'استعادة',
                        FORCE_DELETE: 'حذف نهائي',
                    },
                    subjects: {
                        communities: 'المجتمعات',
                        buildings: 'المباني',
                        units: 'الوحدات',
                        leases: 'عقود الإيجار',
                        subLeases: 'عقود الإيجار الفرعية',
                        transactions: 'المعاملات',
                        payments: 'المدفوعات',
                        owners: 'الملاك',
                        tenants: 'المستأجرون',
                        dependents: 'المعالون',
                        admins: 'المسؤولون',
                        professionals: 'المهنيون',
                        homeServices: 'الخدمات المنزلية',
                        neighbourhoodServices: 'خدمات الحي',
                        visitorAccess: 'دخول الزوار',
                        facilityBookings: 'حجوزات المرافق',
                        managerRequests: 'طلبات المدير',
                        facilities: 'المرافق',
                        announcements: 'الإعلانات',
                        directories: 'الأدلة',
                        suggestions: 'الاقتراحات',
                        complaints: 'الشكاوى',
                        marketPlaces: 'الأسواق',
                        marketPlaceBookings: 'حجوزات الأسواق',
                        marketPlaceVisits: 'زيارات الأسواق',
                        offerRequests: 'طلبات العروض',
                        reports: 'التقارير',
                        settings: 'الإعدادات',
                        companyProfile: 'ملف الشركة',
                        invoiceSettings: 'إعدادات الفواتير',
                        leaseSettings: 'إعدادات الإيجار',
                    },
                    presets: {
                        admin: 'مسؤول',
                        manager: 'مدير',
                        accountingManager: 'مدير الحسابات',
                        serviceManager: 'مدير الخدمات',
                        owner: 'مالك',
                        tenant: 'مستأجر',
                        professional: 'مهني',
                        dependent: 'معال',
                        empty: 'بلا صلاحيات',
                    },
                },
            },
            subscriptions: {
                pageTitle: 'إدارة الاشتراكات',
                heading: 'اشتراكات الحسابات',
                description: 'إدارة اشتراكات الحسابات باستخدام الخطة الشهرية الافتراضية.',
                defaultPlan: 'الخطة الافتراضية',
                planName: 'الخطة',
                price: 'السعر',
                trial: 'الفترة التجريبية',
                slug: 'المعرف',
                account: 'الحساب',
                domain: 'النطاق',
                status: 'الحالة',
                periodEndsAt: 'ينتهي الاشتراك في',
                actions: 'الإجراءات',
                activate: 'تفعيل',
                cancelAtEnd: 'إلغاء بنهاية الفترة',
                cancelNow: 'إلغاء فوري',
                notSubscribed: 'غير مشترك',
                active: 'نشط',
                onTrial: 'في الفترة التجريبية',
                canceled: 'ملغى',
                ended: 'منتهي',
                inactive: 'غير نشط',
                empty: 'لا توجد حسابات.',
            },
            documents: {
                pageTitle: 'قوالب المستندات',
                heading: 'قوالب المستندات',
                description: 'إنشاء وإدارة قوالب المستندات مع حقول الدمج لعقود الإيجار والفواتير والإيصالات والحجوزات.',
                create: 'إنشاء قالب',
                edit: 'تعديل القالب',
                nameEn: 'الاسم (إنجليزي)',
                nameAr: 'الاسم (عربي)',
                type: 'النوع',
                format: 'التنسيق',
                status: 'الحالة',
                version: 'الإصدار',
                versions: 'الإصدارات',
                currentVersion: 'الإصدار الحالي',
                bodyEn: 'المحتوى (إنجليزي)',
                bodyAr: 'المحتوى (عربي)',
                mergeFields: 'حقول الدمج',
                addMergeField: 'إضافة حقل',
                mergeFieldKey: 'مفتاح الحقل',
                mergeFieldLabelEn: 'التسمية (إنجليزي)',
                mergeFieldLabelAr: 'التسمية (عربي)',
                mergeFieldType: 'النوع',
                mergeFieldSourcePath: 'مسار المصدر',
                removeMergeField: 'إزالة',
                activate: 'تفعيل',
                archive: 'أرشفة',
                activated: 'تم تفعيل القالب.',
                archived: 'تمت أرشفة القالب.',
                created: 'تم إنشاء القالب بنجاح.',
                updated: 'تم تحديث القالب إلى الإصدار :version.',
                statusDraft: 'مسودة',
                statusActive: 'نشط',
                statusArchived: 'مؤرشف',
                formatWordUpload: 'رفع ملف Word',
                formatInPlatform: 'محرر داخل المنصة',
                empty: 'لا توجد قوالب مستندات. أنشئ قالبك الأول للبدء.',
            },
            featureFlags: {
                pageTitle: 'علامات الميزات — {{tenant}}',
                tabOverview: 'نظرة عامة',
                tabFeatures: 'الميزات',
                tabUsers: 'المستخدمون',
                tabSubscription: 'الاشتراك',
                description: 'قم بتهيئة ميزات المنصة النشطة لهذا المستأجر. تسري التغييرات فورًا في تحميل الصفحة التالي.',
                colFeature: 'الميزة',
                colStatus: 'الحالة',
                colInTier: 'مضمّن في الخطة',
                colToggle: 'تبديل',
                statusEnabled: 'مفعّل',
                statusDisabled: 'معطّل',
                enableHeading: 'تفعيل {{feature}}؟',
                enableBody: 'سيحصل مستخدمو المستأجر على وصول فوري في تحميل الصفحة التالي.',
                enableCta: 'تفعيل الميزة',
                disableHeading: 'تعطيل {{feature}}؟',
                disableImpactLabel: 'تأثير فوري',
                disableImpactBody: 'سيفقد مستخدمو المستأجر الذين يستخدمون {{feature}} الوصول في تحميل الصفحة التالي. ستظهر رسالة "الميزة غير متاحة".',
                disableReversible: 'يمكن التراجع عن هذا الإجراء وإعادة تفعيل الميزة في أي وقت.',
                disableCta: 'تعطيل الميزة',
                toastEnabled: 'تم تفعيل {{feature}}',
                toastDisabled: 'تم تعطيل {{feature}}',
                toggleError: 'تعذّر التحديث. يرجى المحاولة مرة أخرى.',
                tierIncluded: 'مضمّن افتراضيًا في خطة {{plan}}.',
                tierNotIncluded: 'غير مضمّن في خطة {{plan}}. تم التفعيل يدويًا من قبل المشغّل.',
                emptyHeading: 'لا توجد علامات ميزات محددة بعد.',
                emptyHeadingAr: 'لا توجد علامات ميزات محددة بعد.',
                emptyBody: 'يتم تعريف علامات الميزات في الكود وزرعها تلقائيًا. تواصل مع الهندسة إذا كانت هناك علامة مفقودة.',
                ariaEnable: 'تفعيل {{feature}}',
                ariaDisable: 'تعطيل {{feature}}',
            },
        },
        settings: {
            profile: {
                pageTitle: 'إعدادات الملف الشخصي',
                heading: 'معلومات الملف الشخصي',
                description: 'حدّث الاسم والبريد الإلكتروني',
                name: 'الاسم',
                fullName: 'الاسم الكامل',
                emailAddress: 'البريد الإلكتروني',
                emailPlaceholder: 'البريد الإلكتروني',
                unverifiedNotice: 'بريدك الإلكتروني غير موثق.',
                resendVerification: 'اضغط هنا لإعادة إرسال رسالة التحقق.',
                verificationSent: 'تم إرسال رابط تحقق جديد إلى بريدك الإلكتروني.',
            },
            security: {
                pageTitle: 'إعدادات الأمان',
                updatePassword: 'تحديث كلمة المرور',
                passwordDescription: 'تأكد من استخدام كلمة مرور قوية وعشوائية لحماية حسابك',
                currentPassword: 'كلمة المرور الحالية',
                currentPasswordPlaceholder: 'كلمة المرور الحالية',
                newPassword: 'كلمة المرور الجديدة',
                newPasswordPlaceholder: 'كلمة المرور الجديدة',
                confirmPassword: 'تأكيد كلمة المرور',
                confirmPasswordPlaceholder: 'تأكيد كلمة المرور',
                savePassword: 'حفظ كلمة المرور',
                twoFactorTitle: 'التحقق الثنائي',
                twoFactorDescription: 'إدارة إعدادات التحقق الثنائي',
                twoFactorEnableHelp: 'عند تفعيل التحقق الثنائي، سيُطلب منك إدخال رمز آمن أثناء تسجيل الدخول. يمكن الحصول على الرمز من تطبيق يدعم TOTP على هاتفك.',
                twoFactorEnabledHelp: 'سيُطلب منك إدخال رمز آمن وعشوائي أثناء تسجيل الدخول، ويمكنك الحصول عليه من تطبيق يدعم TOTP على هاتفك.',
                sessions: {
                    title: 'الجلسات النشطة',
                    description: 'عرض وإدارة الأجهزة المسجلة حاليًا في حسابك.',
                    currentSession: 'الجلسة الحالية',
                    lastActive: 'آخر نشاط',
                    revoke: 'إنهاء',
                    revokeAll: 'تسجيل الخروج من جميع الجلسات الأخرى',
                    revokeAllSuccess: 'تم تسجيل الخروج من جميع الجلسات الأخرى.',
                    revokeSingleSuccess: 'تم إنهاء الجلسة بنجاح.',
                    onlyOne: 'جلسة نشطة واحدة فقط.',
                    driverNotConfigured: 'إدارة الجلسات تتطلب إعدادات إضافية. يرجى التواصل مع مدير النظام.',
                    revokeConfirmTitle: 'إنهاء هذه الجلسة؟',
                    revokeConfirmBody: 'سيتم تسجيل خروج الجهاز وسيحتاج إلى تسجيل الدخول مرة أخرى.',
                    revokeAllConfirmTitle: 'تسجيل الخروج من جميع الجلسات الأخرى؟',
                    revokeAllConfirmBody: 'سيتم تسجيل خروج جميع الأجهزة ما عدا هذا الجهاز فورًا.',
                },
            },
            appearance: {
                pageTitle: 'إعدادات المظهر',
                heading: 'إعدادات المظهر',
                description: 'حدّث إعدادات مظهر حسابك',
                light: 'فاتح',
                dark: 'داكن',
                system: 'النظام',
            },
            deleteAccount: {
                heading: 'حذف الحساب',
                description: 'حذف الحساب وجميع موارده',
                warningDescription: 'يرجى المتابعة بحذر، لا يمكن التراجع عن هذا الإجراء.',
                button: 'حذف الحساب',
                dialogTitle: 'هل أنت متأكد من رغبتك في حذف الحساب؟',
                dialogDescription: 'بعد حذف الحساب سيتم حذف جميع البيانات والموارد نهائيًا. أدخل كلمة المرور لتأكيد حذف الحساب بشكل دائم.',
                password: 'كلمة المرور',
                passwordPlaceholder: 'كلمة المرور',
            },
        },
        dashboard: {
            pageTitle: 'لوحة التحكم',
            communities: 'المشاريع',
            buildings: 'المباني',
            units: 'الوحدات',
            tenants: 'المستأجرون',
            activeLeases: 'العقود النشطة',
            openRequests: 'الطلبات المفتوحة',
            pendingTransactions: 'المعاملات المعلّقة',
            totalRevenue: 'إجمالي الإيرادات',
            requiresAttention: 'يتطلب الانتباه',
            recentLeases: 'أحدث العقود',
            recentServiceRequests: 'أحدث طلبات الخدمة',
            contract: 'العقد',
            tenant: 'المستأجر',
            status: 'الحالة',
            period: 'الفترة',
            amount: 'المبلغ',
            id: 'المعرّف',
            category: 'الفئة',
            priority: 'الأولوية',
            date: 'التاريخ',
            noLeasesYet: 'لا توجد عقود حتى الآن.',
            noRequestsYet: 'لا توجد طلبات حتى الآن.',
        },
        notifications: {
            pageTitle: 'الإشعارات',
            heading: 'الإشعارات',
            description: 'تابع نشاط النظام وحدد العناصر كمقروءة.',
            unread: 'غير المقروء',
            recent: 'أحدث الإشعارات',
            read: 'مقروء',
            noNotificationsYet: 'لا توجد إشعارات حتى الآن.',
            openCenter: 'فتح مركز الإشعارات',
            unreadCountEndpoint: 'واجهة عدد غير المقروء',
        },
        properties: {
            communities: {
                pageTitle: 'المشاريع',
                heading: 'المشاريع',
                description: 'إدارة المشاريع والتجمعات العقارية.',
                newCommunity: 'مشروع جديد',
                noCommunitiesFound: 'لا توجد مشاريع. أنشئ مشروعك الأول للبدء.',
                table: {
                    name: 'الاسم',
                    country: 'الدولة',
                    city: 'المدينة',
                    currency: 'العملة',
                    buildings: 'المباني',
                    units: 'الوحدات',
                    requests: 'الطلبات',
                    salesCommission: 'عمولة البيع %',
                    rentalCommission: 'عمولة الإيجار %',
                },
                create: {
                    pageTitle: 'مشروع جديد',
                    heading: 'إنشاء مشروع',
                    description: 'أضف مشروعًا جديدًا إلى محفظتك.',
                    communityName: 'اسم المشروع',
                    communityNamePlaceholder: 'أدخل اسم المشروع',
                    country: 'الدولة',
                    selectCountry: 'اختر الدولة',
                    currency: 'العملة',
                    selectCurrency: 'اختر العملة',
                    city: 'المدينة',
                    selectCity: 'اختر المدينة',
                    district: 'الحي',
                    selectDistrict: 'اختر الحي',
                    salesCommission: 'عمولة البيع (%)',
                    rentalCommission: 'عمولة الإيجار (%)',
                    ratePlaceholder: '0.00',
                    createButton: 'إنشاء مشروع',
                },
                edit: {
                    breadcrumb: 'تعديل',
                    pageTitleWithName: 'تعديل {{name}}',
                    description: 'تحديث تفاصيل المشروع.',
                    updateButton: 'تحديث المشروع',
                    amenities: {
                        sectionTitle: 'المرافق',
                        sectionDescription: 'حدد المرافق المشتركة المتاحة في هذا المجمع.',
                        noAmenities: 'لم يتم تحديد أي مرافق',
                        remove: 'إزالة {{name}}',
                    },
                    workingDays: {
                        sectionTitle: 'أيام العمل',
                        sectionDescription: 'الأيام التي تعمل فيها مرافق المجمع والمكاتب.',
                        noDays: 'غير محدد',
                        sat: 'السبت',
                        sun: 'الأحد',
                        mon: 'الاثنين',
                        tue: 'الثلاثاء',
                        wed: 'الأربعاء',
                        thu: 'الخميس',
                        fri: 'الجمعة',
                    },
                    coordinates: {
                        sectionTitle: 'إحداثيات الخريطة',
                        sectionDescription: 'حدد الموقع الجغرافي للمجمع.',
                        latitude: 'خط العرض',
                        longitude: 'خط الطول',
                        latitudePlaceholder: 'مثال: 24.7743',
                        longitudePlaceholder: 'مثال: 46.7386',
                        latitudeHint: 'النطاق: −90 إلى 90',
                        longitudeHint: 'النطاق: −180 إلى 180',
                        useMyLocation: 'استخدم موقعي',
                        permissionDenied: 'تم رفض الوصول إلى الموقع من قِبل متصفحك. أدخل الإحداثيات يدوياً أدناه.',
                        gettingLocation: 'جاري تحديد موقعك...',
                        locationFilled: 'تم ملء الموقع',
                    },
                },
                show: {
                    breadcrumb: 'التفاصيل',
                    detailsDescription: 'تفاصيل وإحصائيات المشروع',
                    confirmDeletePrompt: 'هل أنت متأكد من حذف هذا المشروع؟',
                    buildings: 'المباني',
                    units: 'الوحدات',
                    requests: 'الطلبات',
                    location: 'الموقع',
                    financials: 'البيانات المالية',
                    country: 'الدولة',
                    city: 'المدينة',
                    district: 'الحي',
                    currency: 'العملة',
                    salesCommission: 'عمولة البيع',
                    rentalCommission: 'عمولة الإيجار',
                    marketplace: 'السوق',
                    relatedBuildings: 'المباني',
                    buildingUnits: '{{count}} وحدة',
                    facilities: 'المرافق',
                    amenities: 'المرافق',
                    noAmenities: 'لم يتم تحديد أي مرافق',
                    workingDays: 'أيام العمل',
                    noDays: '—',
                    coordinates: 'خط العرض / الطول',
                    noCoordinates: '—',
                    sat: 'السبت',
                    sun: 'الأحد',
                    mon: 'الاثنين',
                    tue: 'الثلاثاء',
                    wed: 'الأربعاء',
                    thu: 'الخميس',
                    fri: 'الجمعة',
                    aria: {
                        working_day: 'يوم عمل',
                        non_working_day: 'يوم عطلة',
                    },
                },
            },
            buildings: {
                pageTitle: 'المباني',
                heading: 'المباني',
                description: 'إدارة المباني داخل مشاريعك.',
                newBuilding: 'مبنى جديد',
                noBuildingsFound: 'لا توجد مبانٍ.',
                table: {
                    name: 'الاسم',
                    community: 'المشروع',
                    city: 'المدينة',
                    floors: 'الطوابق',
                    units: 'الوحدات',
                    yearBuilt: 'سنة البناء',
                },
                create: {
                    pageTitle: 'مبنى جديد',
                    heading: 'إنشاء مبنى',
                    description: 'أضف مبنى جديدًا إلى مشروع.',
                    buildingName: 'اسم المبنى',
                    buildingNamePlaceholder: 'أدخل اسم المبنى',
                    community: 'المشروع',
                    selectCommunity: 'اختر المشروع',
                    city: 'المدينة',
                    selectCity: 'اختر المدينة',
                    district: 'الحي',
                    selectDistrict: 'اختر الحي',
                    floors: 'عدد الطوابق',
                    floorsPlaceholder: '0',
                    yearBuilt: 'سنة البناء',
                    yearPlaceholder: '2024',
                    createButton: 'إنشاء مبنى',
                },
                edit: {
                    breadcrumb: 'تعديل',
                    pageTitleWithName: 'تعديل {{name}}',
                    description: 'تحديث تفاصيل المبنى.',
                    updateButton: 'تحديث المبنى',
                },
                show: {
                    breadcrumb: 'التفاصيل',
                    confirmDeletePrompt: 'هل أنت متأكد من حذف هذا المبنى؟',
                    units: 'الوحدات',
                    floors: 'الطوابق',
                    yearBuilt: 'سنة البناء',
                    relatedUnits: 'الوحدات',
                },
            },
            units: {
                pageTitle: 'الوحدات',
                heading: 'الوحدات',
                description: 'إدارة الوحدات الفردية عبر عقاراتك.',
                newUnit: 'وحدة جديدة',
                noUnitsFound: 'لا توجد وحدات.',
                table: {
                    name: 'الاسم',
                    community: 'المشروع',
                    building: 'المبنى',
                    category: 'الفئة',
                    status: 'الحالة',
                    floor: 'الطابق',
                    area: 'المساحة (م²)',
                    owner: 'المالك',
                    tenant: 'المستأجر',
                },
                create: {
                    pageTitle: 'وحدة جديدة',
                    heading: 'إنشاء وحدة',
                    description: 'أضف وحدة جديدة إلى عقارك.',
                    unitName: 'اسم الوحدة',
                    unitNamePlaceholder: 'مثال: A-101',
                    community: 'المشروع',
                    selectCommunity: 'اختر المشروع',
                    building: 'المبنى',
                    selectBuilding: 'اختر المبنى',
                    status: 'الحالة',
                    selectStatus: 'اختر الحالة',
                    owner: 'المالك',
                    selectOwner: 'اختر المالك',
                    tenant: 'المستأجر',
                    selectTenant: 'اختر المستأجر',
                    city: 'المدينة',
                    selectCity: 'اختر المدينة',
                    district: 'الحي',
                    selectDistrict: 'اختر الحي',
                    category: 'الفئة',
                    selectCategory: 'اختر الفئة',
                    type: 'النوع',
                    selectType: 'اختر النوع',
                    netArea: 'المساحة الصافية (م²)',
                    areaPlaceholder: '0.00',
                    floorNumber: 'رقم الطابق',
                    floorPlaceholder: '0',
                    yearBuilt: 'سنة البناء',
                    yearPlaceholder: '2024',
                    descriptionLabel: 'الوصف',
                    descriptionPlaceholder: 'وصف الوحدة...',
                    createButton: 'إنشاء وحدة',
                },
                edit: {
                    breadcrumb: 'تعديل',
                    pageTitleWithName: 'تعديل {{name}}',
                    description: 'تحديث تفاصيل الوحدة.',
                    updateButton: 'تحديث الوحدة',
                    specifications: {
                        sectionTitle: 'المواصفات',
                        bedrooms: 'غرف النوم',
                        bathrooms: 'الحمامات',
                        livingRooms: 'غرف المعيشة',
                        furnished: 'مفروشة؟',
                        furnishedYes: 'نعم',
                        furnishedNo: 'لا',
                        parking: 'مواقف السيارات',
                        parkingNone: 'بدون',
                        parkingOneBay: 'موقف واحد',
                        parkingTwoBays: 'موقفان',
                        view: 'الإطلالة',
                        viewSea: 'إطلالة بحرية',
                        viewCity: 'إطلالة على المدينة',
                        viewGarden: 'إطلالة على الحديقة',
                        viewNone: 'بدون إطلالة',
                    },
                    amenities: {
                        sectionTitle: 'المرافق',
                        instruction: 'اختر كل ما ينطبق',
                        pool: 'مسبح',
                        gym: 'صالة رياضية',
                        security: 'أمن',
                        garden: 'حديقة',
                        parking: 'موقف سيارات',
                        concierge: 'كونسيرج',
                        rooftop: 'سطح',
                        sauna: 'ساونا',
                        kidsPlay: 'منطقة ألعاب أطفال',
                    },
                    pricing: {
                        sectionTitle: 'مرجع التسعير',
                        askingRent: 'الإيجار المطلوب',
                        selectCurrency: 'العملة',
                        periodYear: 'سنة',
                        periodMonth: 'شهر',
                        selectPeriod: 'الفترة',
                    },
                },
                show: {
                    breadcrumb: 'التفاصيل',
                    confirmDeletePrompt: 'هل أنت متأكد من حذف هذه الوحدة؟',
                    noBuilding: 'بدون مبنى',
                    category: 'الفئة',
                    type: 'النوع',
                    status: 'الحالة',
                    area: 'المساحة',
                    areaValue: '{{value}} م²',
                    details: 'التفاصيل',
                    floor: 'الطابق',
                    yearBuilt: 'سنة البناء',
                    marketplace: 'السوق',
                    occupancy: 'الإشغال',
                    owner: 'المالك',
                    tenant: 'المستأجر',
                    descriptionLabel: 'الوصف',
                    specifications: 'المواصفات',
                    furnished: 'مفروشة',
                    unfurnished: 'غير مفروشة',
                    amenities: 'المرافق',
                    noAmenities: 'لا توجد مرافق مدرجة',
                    pricing: 'مرجع التسعير',
                    askingRent: 'الإيجار المطلوب',
                    pricingNotSet: 'غير محدد',
                    rentPeriodYear: 'سنة',
                    rentPeriodMonth: 'شهر',
                },
                import: {
                    importUnits: 'استيراد وحدات',
                    modalTitle: 'استيراد وحدات من إكسل',
                    step1: 'رفع الملف',
                    step2: 'مطابقة الأعمدة',
                    step3: 'مراجعة',
                    step4: 'استيراد',
                    downloadTemplate: 'تحميل النموذج',
                    dropZoneText: 'اسحب وأفلت ملف الإكسل هنا أو انقر للتصفح',
                    acceptedFormat: 'الصيغة المقبولة: .xlsx · الحد الأقصى: 10MB',
                    rowsDetected: 'تم اكتشاف {{n}} صف',
                    systemField: 'الحقل في النظام',
                    uploadedColumn: 'العمود المرفوع',
                    matched: 'متطابق',
                    unmatched: 'غير متطابق',
                    selectColumn: '-- اختر --',
                    validRows: '{{n}} صحيح',
                    errorRows: '{{n}} بأخطاء',
                    warningRows: '{{n}} تحذيرات',
                    allRowsInvalid: 'جميع الصفوف {{n}} تحتوي على أخطاء. يرجى تصحيح الملف وإعادة الرفع.',
                    noDataRows: 'الملف المرفوع لا يحتوي على بيانات.',
                    importValidOnly: 'استيراد {{n}} صف صحيح',
                    cancelAndFix: 'إلغاء وتصحيح الملف',
                    progressLabel: 'جارٍ استيراد {{n}} وحدة...',
                    progressNote: 'لا تُغلق هذه النافذة.',
                    completionToast: 'تم استيراد {{n}} وحدة بنجاح. تم تخطي {{e}} صف بسبب أخطاء.',
                    viewReport: 'عرض التقرير',
                    errorBuildingNotFound: 'المبنى "{{name}}" غير موجود',
                    errorDuplicate: 'رقم وحدة مكرر في نفس المبنى',
                    errorInvalidStatus: 'الحالة "{{name}}" غير صالحة',
                    errorInvalidFile: 'يجب أن يكون الملف بصيغة .xlsx',
                    errorFileTooLarge: 'حجم الملف يتجاوز 10MB. يرجى تقسيم الملف.',
                    errorRow: 'الصف',
                    errorField: 'الحقل',
                    errorMessage: 'الخطأ',
                    filterAll: 'جميع الصفوف',
                    filterErrors: 'الأخطاء فقط',
                    queuedMessage: 'الاستيراد قيد المعالجة في الخلفية. ستُشعَر عند الانتهاء.',
                    fieldName: 'اسم الوحدة',
                    fieldCommunity: 'المشروع',
                    fieldBuilding: 'المبنى',
                    fieldNetArea: 'المساحة (م²)',
                    fieldStatus: 'الحالة',
                },
            },
        },
        requests: {
            pageTitle: 'الطلبات',
            heading: 'طلبات الخدمة',
            description: 'تابع وأدر طلبات الصيانة والخدمات.',
            newRequest: 'طلب جديد',
            search: 'بحث',
            searchPlaceholder: 'العنوان أو الوصف أو الكود أو الرقم',
            status: 'الحالة',
            allStatuses: 'كل الحالات',
            category: 'الفئة',
            allCategories: 'كل الفئات',
            priority: 'الأولوية',
            allPriorities: 'كل الأولويات',
            rowsPerPage: 'عدد الصفوف',
            noRequestsFound: 'لا توجد طلبات.',
            showingSummary: 'عرض {{from}} إلى {{to}} من {{total}} طلب',
            table: {
                id: 'المعرّف',
                category: 'الفئة',
                subcategory: 'الفئة الفرعية',
                status: 'الحالة',
                community: 'المشروع',
                created: 'تاريخ الإنشاء',
            },
            create: {
                pageTitle: 'طلب جديد',
                heading: 'إنشاء طلب',
                description: 'قدّم طلب خدمة جديد.',
                selectCategory: 'اختر الفئة',
                selectSubcategory: 'اختر الفئة الفرعية',
                selectCommunity: 'اختر المشروع',
                selectUnit: 'اختر الوحدة',
                selectPriority: 'اختر الأولوية',
                low: 'منخفض',
                medium: 'متوسط',
                high: 'مرتفع',
                urgent: 'عاجل',
                descriptionPlaceholder: 'اكتب تفاصيل الطلب...',
                submitButton: 'إرسال الطلب',
            },
            show: {
                breadcrumb: 'التفاصيل',
                pageTitle: 'الطلب #{{id}}',
                confirmDeletePrompt: 'هل أنت متأكد؟',
                status: 'الحالة',
                priority: 'الأولوية',
                created: 'تاريخ الإنشاء',
                location: 'الموقع',
                community: 'المشروع',
                unit: 'الوحدة',
                people: 'الأشخاص',
                requester: 'مقدم الطلب',
                assignee: 'المكلّف',
                description: 'الوصف',
            },
        },
        facilities: {
            pageTitle: 'المرافق',
            heading: 'المرافق',
            description: 'إدارة المرافق وتفاصيلها.',
            newFacility: 'مرفق جديد',
            createTitle: 'إنشاء مرفق',
            createDescription: 'إضافة مرفق جديد.',
            editTitle: 'تعديل المرفق',
            editDescription: 'تحديث تفاصيل المرفق.',
            facilityName: 'اسم المرفق',
            name: 'الاسم',
            namePlaceholder: 'أدخل اسم المرفق',
            category: 'الفئة',
            selectCategory: 'اختر الفئة',
            community: 'المشروع',
            selectCommunity: 'اختر المشروع',
            capacity: 'السعة',
            capacityPlaceholder: 'أدخل السعة',
            status: 'الحالة',
            details: 'تفاصيل المرفق',
            descriptionLabel: 'الوصف',
            bookings: 'الحجوزات',
            createButton: 'إنشاء المرفق',
            updateButton: 'تحديث المرفق',
            noFacilitiesFound: 'لا توجد مرافق.',
            resident: {
                pageTitle: 'المرافق',
                heading: 'المرافق',
                description: 'احجز الصالة الرياضية والمسبح وغيرها',
                free: 'مجاني',
                pricePerSession: '{{n}} ر.س / جلسة',
                pricePerHour: '{{n}} ر.س / ساعة',
                contractRequired: 'يتطلب عقداً',
                book: 'احجز',
                capacityLabel: 'السعة: {{n}}',
                morning: 'صباحاً',
                afternoon: 'بعد الظهر',
                evening: 'مساءً',
                available: 'متاح',
                availableTap: 'متاح · اضغط للحجز',
                booked: 'محجوز',
                closed: 'مغلق',
                confirmTitle: 'تأكيد الحجز',
                confirmDate: 'التاريخ',
                confirmTime: 'الوقت',
                confirmDuration: 'المدة',
                confirmPrice: 'السعر',
                confirmTotal: 'المجموع',
                confirmNote: 'سيتم إرسال التأكيد إلى رقم هاتفك المحمول وبريدك الإلكتروني المسجلين.',
                confirm: 'تأكيد',
                cancel: 'إلغاء',
                raceConditionTitle: 'هذه الفتحة لم تعد متاحة',
                raceConditionDetail: 'قام ساكن آخر بحجزها للتو. الرجاء اختيار وقت آخر.',
                pickAnotherSlot: 'اختر فتحة أخرى',
                contractNotice: 'هذا المرفق يتطلب عقداً موقعاً. سيظل حجزك معلقاً حتى توقيع العقد.',
                successToast: 'تم تأكيد الحجز! ستتلقى تأكيداً قريباً.',
                noFacilitiesAvailable: 'لا توجد مرافق متاحة في مجتمعك بعد.',
                allSlotsFull: 'جميع الفترات محجوزة لهذا التاريخ.',
                joinWaitlist: 'انضم لقائمة الانتظار',
                closedOnDay: 'المرفق مغلق في هذا اليوم. الرجاء اختيار تاريخ آخر.',
                slotUnavailable: 'هذه الفتحة لم تعد متاحة.',
                bookingConfirmed: 'تم تأكيد الحجز! ستتلقى تأكيداً قريباً.',
                previousWeek: 'الأسبوع السابق',
                nextWeek: 'الأسبوع التالي',
                selectDate: 'اختر تاريخ {{day}}، {{date}}',
                slotAriaLabel: '{{time}} - {{status}}',
                loadingFacilities: 'جارٍ تحميل المرافق...',
                loadingSlots: 'جارٍ تحميل الفترات...',
                processing: 'جارٍ المعالجة...',
                serverError: 'حدث خطأ ما. الرجاء المحاولة مرة أخرى.',
                allSlotsBookedJoinWaitlist: 'جميع الفترات محجوزة لهذا التاريخ.',
            },
            calendar: {
                pageTitle: 'تقويم المرافق',
                heading: 'تقويم المرافق',
                description: 'إدارة جميع الحجوزات عبر جميع المرافق',
                allFacilities: 'جميع المرافق',
                previousWeek: 'الأسبوع السابق',
                nextWeek: 'الأسبوع التالي',
                today: 'اليوم',
                filterAll: 'الكل',
                filterConfirmed: 'مؤكد',
                filterCheckedIn: 'تم الدخول',
                filterCompleted: 'مكتمل',
                filterCancelled: 'ملغي',
                adminReservation: 'حجز إداري',
                pending: 'معلق (بانتظار العقد)',
                bookingDetail: 'تفاصيل الحجز',
                resident: 'الساكن',
                facility: 'المرفق',
                date: 'التاريخ',
                time: 'الوقت',
                duration: 'المدة',
                status: 'الحالة',
                checkIn: 'تسجيل دخول',
                cancelBooking: 'إلغاء الحجز',
                edit: 'تعديل',
                createBooking: 'إنشاء حجز',
                residentSearch: 'ابحث عن ساكن بالاسم أو الوحدة...',
                start: 'البداية',
                end: 'النهاية',
                notes: 'ملاحظات',
                cancel: 'إلغاء',
                create: 'إنشاء',
                overlapTitle: 'تم اكتشاف تعارض',
                overlapMsg: 'تتعارض هذه الفتحة الزمنية مع حجز موجود.',
                overlapAction: 'يرجى تعديل وقت البداية/النهاية.',
                noBookingsThisWeek: 'لا توجد حجوزات لهذا الأسبوع.',
                loading: 'جارٍ التحميل...',
                bookingCreated: 'تم إنشاء الحجز بنجاح.',
                legendConfirmed: 'مؤكد',
                legendCheckedIn: 'تم الدخول',
                legendPending: 'معلق',
                legendCompleted: 'مكتمل',
                legendCancelled: 'ملغي',
            },
        },
        leases: {
            pageTitle: 'العقود',
            heading: 'العقود',
            description: 'إدارة عقود الإيجار والاتفاقيات.',
            newLease: 'عقد جديد',
            search: 'بحث',
            searchPlaceholder: 'رقم العقد أو اسم المستأجر',
            status: 'الحالة',
            allStatuses: 'كل الحالات',
            tenant: 'المستأجر',
            allTenants: 'كل المستأجرين',
            rowsPerPage: 'عدد الصفوف',
            noLeasesFound: 'لا توجد عقود.',
            showingSummary: 'عرض {{from}} إلى {{to}} من {{total}} عقد',
            table: {
                contract: 'رقم العقد',
                tenant: 'المستأجر',
                status: 'الحالة',
                type: 'النوع',
                startDate: 'تاريخ البداية',
                endDate: 'تاريخ النهاية',
                totalAmount: 'إجمالي المبلغ',
            },
            create: {
                pageTitle: 'عقد جديد',
                heading: 'إنشاء عقد',
                description: 'إنشاء اتفاقية إيجار جديدة.',
                contractNumber: 'رقم العقد',
                contractPlaceholder: 'مثال: LC-2024-001',
                tenant: 'المستأجر',
                selectTenant: 'اختر المستأجر',
                status: 'الحالة',
                selectStatus: 'اختر الحالة',
                tenantType: 'نوع المستأجر',
                rentalType: 'نوع الإيجار',
                selectType: 'اختر النوع',
                individual: 'فرد',
                company: 'شركة',
                total: 'إجمالي',
                detailed: 'تفصيلي',
                unitCategory: 'فئة الوحدة',
                selectCategory: 'اختر الفئة',
                contractType: 'نوع العقد',
                selectContractType: 'اختر نوع العقد',
                paymentSchedule: 'جدول الدفعات',
                selectSchedule: 'اختر الجدول',
                startDate: 'تاريخ البداية',
                endDate: 'تاريخ النهاية',
                handoverDate: 'تاريخ التسليم',
                totalAmount: 'إجمالي المبلغ',
                amountPlaceholder: '0.00',
                securityDeposit: 'التأمين',
                termsAndConditions: 'الشروط والأحكام',
                termsPlaceholder: 'أدخل شروط العقد...',
                createButton: 'إنشاء عقد',
            },
            edit: {
                breadcrumb: 'تعديل',
                pageTitleWithContract: 'تعديل عقد {{contract}}',
                description: 'تحديث تفاصيل العقد.',
                updateButton: 'تحديث العقد',
            },
            show: {
                breadcrumb: 'التفاصيل',
                pageTitle: 'عقد {{contract}}',
                confirmDeletePrompt: 'هل أنت متأكد؟',
                createSublease: 'إنشاء عقد فرعي',
                parentLease: 'العقد الرئيسي',
                status: 'الحالة',
                totalAmount: 'إجمالي المبلغ',
                unpaid: 'غير المدفوع',
                type: 'النوع',
                duration: 'المدة',
                start: 'البداية',
                end: 'النهاية',
                handover: 'التسليم',
                financial: 'البيانات المالية',
                rentalType: 'نوع الإيجار',
                securityDeposit: 'التأمين',
                subLease: 'عقد فرعي',
                units: 'الوحدات',
                additionalFees: 'الرسوم الإضافية',
                escalations: 'الزيادات',
                subLeases: 'العقود الفرعية',
                createdBy: 'تم الإنشاء بواسطة',
                dealOwner: 'مسؤول العقد',
                tenantLabel: 'المستأجر',
                unitLabel: 'الوحدة',
            },
            sublease: {
                pageTitle: 'إنشاء عقد فرعي',
                pageTitleWithContract: 'إنشاء عقد فرعي للعقد {{contract}}',
                description: 'يرث إعدادات الوحدات والتصنيفات والعقد من العقد الرئيسي.',
                contractNumber: 'رقم العقد',
                contractPlaceholder: 'مثال: LC-SUB-2026-001',
                tenant: 'المستأجر',
                selectTenant: 'اختر المستأجر',
                status: 'الحالة',
                selectStatus: 'اختر الحالة',
                totalAmount: 'إجمالي المبلغ',
                startDate: 'تاريخ البداية',
                endDate: 'تاريخ النهاية',
                handoverDate: 'تاريخ التسليم',
                securityDeposit: 'التأمين',
                legalRepresentative: 'الممثل القانوني',
                fitOutStatus: 'حالة التشطيب',
                termsAndConditions: 'الشروط والأحكام',
                termsPlaceholder: 'أدخل شروط العقد الفرعي',
                createButton: 'إنشاء عقد فرعي',
            },
            approval: {
                queueTitle: 'طلبات الإيجار المعلقة',
                noPending: 'لا توجد طلبات معلقة للمراجعة',
                pendingReview: 'هذا الطلب في انتظار مراجعتك.',
                approve: 'موافقة',
                reject: 'رفض',
                rejectTitle: 'رفض طلب الإيجار؟',
                rejectDesc: 'سيتم إشعار المدير المقدم للطلب. سيبقى الاقتباس المرتبط مقبولاً.',
                rejectReason: 'سبب الرفض',
                rejectReasonPlaceholder: 'اذكر سبب الرفض...',
                confirmReject: 'تأكيد الرفض',
                timeline: 'الجدول الزمني للموافقة',
                approvedBy: 'تمت الموافقة بواسطة',
                rejectedBy: 'تم الرفض بواسطة',
                unauthorized: 'ليس لديك صلاحية الموافقة على هذا المجتمع',
                approvedStatus: 'تمت الموافقة',
                rejectedStatus: 'مرفوض',
                cancel: 'إلغاء',
                review: 'مراجعة',
            },
            amend: {
                pageTitle: 'تعديل شروط العقد',
                breadcrumb: 'تعديل',
                intro: 'الحقول القابلة للتعديل. سيتم تسجيل التغييرات في سجل التعديلات.',
                currentLabel: 'الحالي',
                newLabel: 'الجديد',
                unitNotAmendable: 'استخدم عملية النقل',
                diffSection: 'معاينة الفروقات',
                changed: '(تم التغيير)',
                unchanged: '(بدون تغيير)',
                reason: 'سبب التعديل',
                reasonPlaceholder: 'اذكر سبب هذا التعديل...',
                generateAddendum: 'إنشاء ملحق موقع بعد الحفظ',
                save: 'حفظ التعديل',
                historyTitle: 'سجل التعديلات',
                historyOriginal: 'الشروط الأصلية',
                historyDetail: 'التعديل #{{n}}',
                historyMadeBy: 'تم بواسطة: {{name}}',
                historyViewAddendum: 'عرض الملحق',
                historyNoAddendum: 'لم يتم الإنشاء',
                historyGenerateNow: 'إنشاء ملحق',
                historyAddendumSigned: 'موقع',
            },
            notices: {
                title: 'إرسال إشعار',
                breadcrumb: 'الإشعارات',
                to: 'إلى: {{name}} ({{email}})',
                type: 'نوع الإشعار',
                rentIncrease: 'زيادة الإيجار',
                renewalOffer: 'عرض تجديد',
                moveOutReminder: 'تذكير بالإخلاء',
                freeForm: 'إشعار حر',
                subjectEn: 'الموضوع (إنجليزي)',
                subjectAr: 'الموضوع (العربية)',
                bodyEn: 'المحتوى (إنجليزي)',
                bodyAr: 'المحتوى (عربي)',
                preview: 'معاينة',
                send: 'إرسال الإشعار',
                sent: 'تم إرسال الإشعار إلى {{email}}',
                historyTitle: 'سجل الإشعارات',
                historyViewBody: 'عرض المحتوى',
                historyCount: '{{n}} إشعارات مرسلة',
                errorMissingEmail: 'البريد الإلكتروني للمستأجر مفقود. يرجى تحديث بيانات المقيم قبل إرسال الإشعارات.',
                errorEditContact: 'تعديل جهة الاتصال',
            },
            renewal: {
                bannerTitle: 'تجديد العقد',
                bannerExpiring: 'تنتهي صلاحية العقد خلال {{days}} يوم ({{date}})',
                bannerWindow: 'هذا العقد ضمن نافذة التجديد.',
                generateOffer: 'إنشاء عرض تجديد',
                sentStatus: 'عرض التجديد: مرسل',
                acceptedStatus: 'عرض التجديد: مقبول',
                expiredStatus: 'عرض التجديد: منتهي',
                convertToLease: 'تحويل إلى عقد جديد',
                viewOffer: 'عرض العرض',
                recordDecision: 'تسجيل القرار',
                decisionTitle: 'تسجيل قرار التجديد',
                decisionLabel: 'القرار',
                accepted: 'مقبول',
                declined: 'مرفوض',
                decisionHelp: 'في حالة القبول، يمكنك متابعة إنشاء العقد الجديد باستخدام شروط التجديد.',
                indexTitle: 'عروض التجديد',
                indexDescription: 'تتبع وإدارة جميع عروض تجديد العقود.',
                searchPlaceholder: 'البحث برقم العقد...',
                noOffersFound: 'لا توجد عروض تجديد.',
                summaryCount: '{{count}} عرض تجديد في خط الأنابيب',
                renewalAmount: '{{amount}} ر.س/{{freq}}',
                validUntil: 'صالح حتى',
                createTitle: 'إنشاء عرض تجديد',
                sourceLease: 'العقد المصدر',
                currentTerm: 'المدة الحالية',
                currentRent: 'الإيجار الحالي',
                currentRentValue: '{{amount}} ر.س / {{freq}}',
                renewalTerms: 'شروط التجديد',
                newStartDate: 'تاريخ البدء الجديد',
                newStartDateHint: 'يتم تعبئته تلقائياً ليوم بعد تاريخ انتهاء العقد.',
                durationMonths: 'المدة (بالأشهر)',
                newRent: 'قيمة الإيجار الجديدة (ر.س)',
                rentDiffSuffix: 'مقارنة بالإيجار الحالي',
                paymentFrequency: 'دورية الدفع',
                contractType: 'نوع العقد',
                messageEn: 'رسالة للمستأجر (إنجليزي)',
                messageAr: 'رسالة للمستأجر (عربي)',
                saveDraft: 'حفظ كمسودة',
            },
        },
        moveout: {
            initiate: {
                title: 'بدء الإخلاء',
                date: 'تاريخ الإخلاء',
                reason: 'السبب',
                reasonEndOfLease: 'نهاية مدة العقد',
                reasonEarlyTenant: 'إنهاية مبكر من المستأجر',
                reasonEarlyMgmt: 'إنهاء مبكر من الإدارة',
                reasonOther: 'آخر',
                warning: 'ستتغير حالة الوحدة إلى تحت الصيانة. ستبقى الشروط المالية مقفلة.',
                button: 'بدء الإخلاء',
            },
            inspection: {
                title: 'المعاينة',
                progress: '{{n}}/{{total}} غرف تمت معاينتها',
                condition: 'الحالة',
                conditionExcellent: 'ممتاز',
                conditionGood: 'جيد',
                conditionFair: 'مقبول',
                conditionPoor: 'سيئ',
                notes: 'ملاحظات',
                photos: 'صور',
                addPhoto: 'إضافة صورة',
                addRoom: 'إضافة غرفة',
                save: 'حفظ المعاينة',
                proceedToDeductions: 'الانتقال للخصومات',
            },
            deductions: {
                title: 'خصومات التأمين',
                securityDeposit: 'التأمين: {{amount}} ر.س',
                securityDepositLabel: 'التأمين',
                add: 'إضافة خصم',
                labelEn: 'الوصف (إنجليزي)',
                labelAr: 'الوصف (عربي)',
                amount: 'المبلغ (ر.س)',
                reason: 'السبب',
                totalDeductions: 'إجمالي الخصومات',
                refundAmount: 'مبلغ الاسترداد',
                outstandingCharge: 'المبلغ المستحق',
                exceedWarning: 'إجمالي الخصومات يتجاوز التأمين. يرجى التحقق قبل المتابعة.',
                save: 'حفظ الخصومات',
                proceedToSettlement: 'الانتقال للتسوية',
            },
            show: {
                noMoveOut: 'لا يوجد إخلاء نشط قيد التنفيذ.',
            },
            flash: {
                initiated: 'تم بدء عملية الإخلاء.',
                inspection_saved: 'تم حفظ المعاينة.',
                deductions_saved: 'تم حفظ الخصومات.',
            },
            errors: {
                already_initiated: 'يوجد إخلاء نشط بالفعل لهذا العقد.',
            },
        },
        quotes: {
            pageTitle: 'عروض الإيجار',
            heading: 'عروض الإيجار',
            description: 'إنشاء وإدارة عروض الإيجار للعملاء المحتملين.',
            newQuote: 'عرض جديد',
            search: 'بحث',
            searchPlaceholder: 'رقم العرض أو اسم المقيم',
            statusLabel: 'الحالة',
            allStatuses: 'كل الحالات',
            noQuotesFound: 'لا توجد عروض.',
            showingSummary: 'عرض {{from}} إلى {{to}} من {{total}} عرض',
            table: {
                quoteNumber: 'رقم العرض',
                contact: 'المقيم',
                unit: 'الوحدة',
                status: 'الحالة',
                rentAmount: 'قيمة الإيجار',
                validUntil: 'صالح حتى',
            },
            status: {
                draft: 'مسودة',
                sent: 'مرسل',
                viewed: 'تم الاطلاع',
                accepted: 'مقبول',
                rejected: 'مرفوض',
                expired: 'منتهي',
            },
            create: {
                pageTitle: 'اقتباس جديد',
                heading: 'إنشاء عرض إيجار',
                description: 'أدخل التفاصيل لإنشاء عرض إيجار للعميل المحتمل.',
                unitLabel: 'الوحدة',
                selectUnit: 'اختر الوحدة',
                residentLabel: 'المقيم',
                selectContact: 'اختر المقيم',
                contractType: 'نوع العقد',
                selectContractType: 'اختر نوع العقد',
                duration: 'المدة (بالأشهر)',
                durationPlaceholder: 'مثال: 12',
                financialTerms: 'الشروط المالية',
                rentAmount: 'قيمة الإيجار (ر.س)',
                paymentFrequency: 'وتيرة الدفع',
                selectFrequency: 'اختر وتيرة الدفع',
                securityDeposit: 'التأمين (ر.س)',
                validUntil: 'صالح حتى',
                startDate: 'تاريخ البدء',
                specialConditionsEn: 'الشروط الخاصة (بالإنجليزية)',
                specialConditionsAr: 'الشروط الخاصة (بالعربية)',
                additionalCharges: 'رسوم إضافية',
                addCharge: 'إضافة رسوم',
                chargeLabelEn: 'التسمية (EN)',
                chargeLabelAr: 'التسمية (AR)',
                chargeLabelPlaceholder: 'مثال: رسوم الموقف',
                chargeAmount: 'المبلغ (ر.س)',
                saveDraft: 'حفظ كمسودة',
                send: 'إرسال العرض',
            },
            show: {
                pageTitle: 'العرض {{number}}',
                breadcrumb: 'تفاصيل العرض',
                quoteDetails: 'تفاصيل العرض',
                confirmSend: 'هل أنت متأكد من إرسال هذا العرض للعميل؟',
                confirmReject: 'هل أنت متأكد من تعليم هذا العرض كمرفوض؟',
                confirmExpire: 'هل أنت متأكد من إنهاء صلاحية هذا العرض؟',
                previewLink: 'معاينة',
                months: 'شهر',
                specialConditions: 'الشروط الخاصة',
                revisionHistory: 'سجل التعديلات',
                latestVisible: 'يرى العميل النسخة {{version}}',
                acceptedReadonly: 'تم قبول هذا الاقتباس ولا يمكن تعديله.',
                expiredReadonly: 'انتهت صلاحية هذا الاقتباس ولا يمكن تعديله.',
                rejectedReadonly: 'تم رفض هذا الاقتباس ولا يمكن تعديله.',
            },
            revise: {
                pageTitle: 'تعديل الاقتباس',
                heading: 'تعديل الاقتباس {{number}}',
                description: 'حدّث حقول العرض. سيتم إنشاء نسخة جديدة وإرسالها إلى العميل المحتمل.',
                changedBadge: 'تم التغيير',
                unchangedBadge: 'بدون تغيير',
                wasValue: 'كان {{value}}',
                revisionNote: 'ملاحظة التعديل',
                revisionNotePlaceholder: 'صف ما تغيّر ولماذا...',
                subjectPrefixLabel: 'بادئة موضوع البريد الإلكتروني',
                subjectPrefixUpdatedQuote: 'اقتباس محدث',
                subjectPrefixRevisedOffer: 'عرض معدل',
                subjectPrefixCustom: 'مخصص',
                saveRevision: 'حفظ التعديل وإرسال',
                cancel: 'إلغاء',
            },
            action: {
                revise: 'تعديل الاقتباس',
                markRejected: 'تعليم كمرفوض',
                expireQuote: 'إنهاء صلاحية العرض',
            },
            preview: {
                pageTitle: 'العرض {{number}}',
                heading: 'عرض إيجار',
                quoteNumber: 'العرض رقم {{number}}',
                footer: 'تم مشاركة هذا العرض معك بشكل آمن.',
            },
            convert: {
                pageTitle: 'تحويل الاقتباس إلى طلب إيجار',
                title: 'تحويل الاقتباس إلى طلب إيجار',
                source: 'المصدر: الاقتباس #{id} (مقبول)',
                changesDetected: 'تم اكتشاف تغييرات من الاقتباس',
                saveDraft: 'حفظ كمسودة',
                saveGoToKyc: 'حفظ والانتقال للتوثيق',
                contractInfo: 'معلومات العقد',
                financialTerms: 'الشروط المالية (مملوءة مسبقاً من الاقتباس)',
                autoGenerateNumber: 'توليد رقم العقد تلقائياً',
            },
        },
        kyc: {
            title: 'مستندات التوثيق',
            pageTitle: 'مستندات التوثيق للعقد {{number}}',
            completion: '{{n}}/{{total}} مطلوب',
            requiredSection: 'مستندات مطلوبة',
            optionalSection: 'مستندات اختيارية',
            upload: 'رفع',
            view: 'عرض',
            remove: 'إزالة',
            submitBlocked: '{{n}} مستند مطلوب مفقود: {{list}}',
            submitForApproval: 'تقديم للموافقة',
            noDocumentTypes: 'لا توجد أنواع مستندات محددة.',
            uploadSuccess: 'تم رفع المستند.',
            removeSuccess: 'تم إزالة المستند.',
        },
        transactions: {
            pageTitle: 'المعاملات',
            heading: 'المعاملات',
            description: 'عرض وإدارة المعاملات المالية.',
            newTransaction: 'معاملة جديدة',
            search: 'بحث',
            searchPlaceholder: 'الرقم أو العقد أو الوحدة أو الملاحظات',
            status: 'الحالة',
            allStatuses: 'كل الحالات',
            category: 'الفئة',
            allCategories: 'كل الفئات',
            payment: 'السداد',
            allPayments: 'الكل',
            paid: 'مدفوع',
            unpaid: 'غير مدفوع',
            rowsPerPage: 'عدد الصفوف',
            noTransactionsFound: 'لا توجد معاملات.',
            showingSummary: 'عرض {{from}} إلى {{to}} من {{total}} معاملة',
            table: {
                id: 'المعرّف',
                lease: 'العقد',
                unit: 'الوحدة',
                category: 'الفئة',
                type: 'النوع',
                status: 'الحالة',
                amount: 'المبلغ',
                dueDate: 'تاريخ الاستحقاق',
                paid: 'السداد',
            },
            create: {
                breadcrumb: 'معاملة جديدة',
                pageTitle: 'تسجيل دفعة',
                heading: 'تسجيل دفعة',
                description: 'سجّل يدويًا دفعةً مستلمة من مستأجر أو مالك.',
                directionLabel: 'وارد',
                payer: 'الدافع',
                selectPayer: 'اختر الدافع…',
                residentsGroup: 'المستأجرون',
                ownersGroup: 'الملاك',
                paymentMethod: 'طريقة الدفع',
                selectPaymentMethod: 'اختر طريقة الدفع…',
                cash: 'نقدًا',
                bankTransfer: 'تحويل بنكي',
                cheque: 'شيك',
                paymentDate: 'تاريخ الدفع',
                referenceNumber: 'رقم المرجع (اختياري)',
                leaseOptional: 'عقد الإيجار (اختياري)',
                selectLease: 'اختر عقد الإيجار (اختياري)',
                selectUnit: 'اختر الوحدة…',
                selectCategory: 'اختر الفئة…',
                saveWithReceipt: 'حفظ وإنشاء الإيصال',
                saveWithoutReceipt: 'حفظ بدون إيصال',
                cancel: 'إلغاء',
                invoiceIncompleteTitle: 'إعدادات الفواتير غير مكتملة',
                invoiceIncompleteMessage: 'لن يتمكن النظام من إنشاء الإيصالات حتى يتم تكوين العلامة التجارية.',
                configureSettings: 'ضبط الإعدادات ←',
            },
            edit: {
                breadcrumb: 'تعديل',
                pageTitle: 'تعديل المعاملة',
                heading: 'تعديل المعاملة',
                description: 'تحديث تفاصيل المعاملة.',
                selectStatus: 'اختر الحالة',
                notes: 'ملاحظات',
                notesPlaceholder: 'ملاحظات إضافية...',
                updateButton: 'تحديث المعاملة',
            },
            show: {
                breadcrumb: 'التفاصيل',
                pageTitle: 'المعاملة #{{id}}',
                confirmDeletePrompt: 'هل أنت متأكد؟',
                amount: 'المبلغ',
                tax: 'الضريبة',
                status: 'الحالة',
                due: 'الاستحقاق',
                related: 'مرتبط بـ',
                lease: 'العقد',
                unit: 'الوحدة',
                payer: 'الدافع',
                payerCard: 'الدافع',
                payerName: 'الاسم',
                paymentMethod: 'طريقة الدفع',
                referenceNumber: 'المرجع',
                dates: 'التواريخ',
                paid: 'تم الدفع',
                created: 'تاريخ الإنشاء',
                receipt: 'الإيصال',
                receiptGenerated: 'تم إنشاء الإيصال',
                settingsIncomplete: 'الإعدادات غير مكتملة',
                downloadPdf: 'تنزيل PDF',
                sendReceipt: 'إرسال الإيصال',
                resendReceipt: 'إعادة إرسال الإيصال',
                lastSent: 'آخر إرسال: {{date}}',
                invoiceIncompleteMessage: 'إعدادات الفواتير غير مكتملة — يرجى ضبط العلامة التجارية قبل إرسال الإيصالات.',
                configureSettings: 'ضبط الإعدادات ←',
                sendReceiptTitle: 'إرسال الإيصال',
                sendReceiptBody: 'هل تريد إرسال إيصال المعاملة #{{id}} إلى {{name}} على البريد {{email}}؟',
                sendReceiptConfirm: 'إرسال الإيصال',
                sendReceiptCancel: 'إلغاء',
                receiptSentSuccess: 'تم إرسال الإيصال إلى {{email}}.',
                receiptSendFailed: 'فشل إرسال الإيصال. يرجى المحاولة مرة أخرى.',
            },
        },
        accountingSettings: {
            categories: {
                pageTitle: 'فئات المعاملات',
                heading: 'فئات المعاملات',
                description: 'ضبط فئات الدخل والمصروفات للمعاملات.',
                addCategory: 'إضافة فئة',
                tabIncome: 'فئات الدخل',
                tabExpense: 'فئات المصروفات',
                colNameEn: 'الاسم (بالإنجليزية)',
                colNameAr: 'الاسم (بالعربية)',
                colStatus: 'الحالة',
                colActions: 'الإجراءات',
                statusActive: 'نشط',
                statusInactive: 'غير نشط',
                defaultBadge: 'افتراضي',
                editAction: 'تعديل',
                deactivateAction: 'إلغاء تفعيل',
                reactivateAction: 'إعادة تفعيل',
                deleteAction: 'حذف',
                noCategories: 'لا توجد فئات بعد. أضف فئتك الأولى.',
                sheetAddTitle: 'إضافة فئة',
                sheetEditTitle: 'تعديل الفئة',
                labelCategoryType: 'نوع الفئة',
                labelNameEn: 'الاسم (بالإنجليزية)',
                labelNameAr: 'الاسم (بالعربية)',
                typeIncome: 'دخل',
                typeExpense: 'مصروف',
                saveButton: 'حفظ الفئة',
                saving: 'جارٍ الحفظ...',
                cancelButton: 'إلغاء',
                deactivateConfirmTitle: 'إلغاء تفعيل الفئة؟',
                deactivateConfirmBody: 'ستحتفظ المعاملات الحالية بهذه الفئة.',
                deactivateConfirmButton: 'إلغاء تفعيل',
                keepActiveButton: 'إبقاء نشطة',
                deleteDefaultError: 'لا يمكن حذف الفئات الافتراضية؛ يمكنك إلغاء تفعيلها بدلاً من ذلك.',
            },
        },
        serviceRequests: {
            pageTitle: 'طلباتي',
            heading: 'طلباتي',
            description: 'تتبع حالة طلبات الخدمة التي قدمتها.',
            newRequest: 'طلب جديد',
            noRequestsYet: 'لم تقدم أي طلبات حتى الآن.',
            statusNotConfigured: 'حالة طلب الخدمة غير مكوَّنة. يرجى التواصل مع الدعم.',
            createPageTitle: 'طلب خدمة جديد',
            createHeading: 'تقديم طلب خدمة',
            createDescription: 'صف مشكلتك حتى نتمكن من مساعدتك بسرعة.',
            whatNeedsFixing: 'ما الذي يحتاج إلى إصلاح؟',
            whereIsTheProblem: 'أين المشكلة؟',
            howUrgent: 'ما مدى الإلحاح؟',
            describeTheIssue: 'صف المشكلة',
            attachPhotos: 'إرفاق صور (اختياري)',
            categoryLabel: 'الفئة',
            subcategoryLabel: 'الفئة الفرعية',
            communityLabel: 'المجتمع',
            unitLabel: 'الوحدة',
            roomLabel: 'الغرفة / الموقع',
            urgencyLabel: 'درجة الإستعجال',
            urgencyNormal: 'عادي',
            urgencyUrgent: 'عاجل',
            descriptionLabel: 'وصف المشكلة',
            descriptionPlaceholder: 'صف المشكلة بالتفصيل...',
            photoHint: 'JPG, PNG. الحد الأقصى 10 ميغابايت لكل صورة',
            addPhoto: '+ إضافة صورة',
            removePhoto: 'إزالة',
            submitButton: 'تقديم الطلب',
            submitting: 'جارٍ الإرسال...',
            cancelButton: 'إلغاء',
            selectCategory: 'اختر فئة',
            selectSubcategory: 'اختر فئة فرعية',
            subcategoryDisabled: 'اختر فئة أولاً',
            selectCommunity: 'اختر مجتمعاً',
            selectUnit: 'اختر وحدة',
            selectRoom: 'اختر الغرفة / الموقع',
            confirmedHeading: 'تم تقديم الطلب بنجاح',
            confirmedDescription: 'ستتلقى إشعاراً عند تعيين فني وعند اكتمال العمل.',
            referenceLabel: 'رقم الطلب الخاص بك',
            copyButton: 'نسخ',
            expectedResponseLabel: 'الرد المتوقع بحلول',
            expectedResolutionLabel: 'الحل المتوقع بحلول',
            viewMyRequests: 'عرض طلباتي',
            submitAnother: 'تقديم طلب آخر',
            urgencyNormalHint: 'الإصلاح في إطار مستوى الخدمة',
            urgencyUrgentHint: 'أمان / طارئ',
            roomKitchen: 'مطبخ',
            roomBathroom: 'حمام',
            roomLivingRoom: 'غرفة معيشة',
            roomBedroom: 'غرفة نوم',
            roomBalcony: 'بلكونة',
            roomOther: 'أخرى',
            // Admin triage
            triagePageTitle: 'طلبات الخدمة',
            triageHeading: 'طلبات الخدمة',
            triageDescription: 'مراجعة الطلبات الواردة، تعيين الفنيين وتحديد الأولوية.',
            tabAll: 'جميع الطلبات',
            tabUnassigned: 'غير معيّنة',
            tabOverdue: 'متأخرة',
            tabSlaBreach: 'تجاوز زمن الخدمة',
            searchPlaceholder: 'بحث برقم الطلب أو اسم الساكن',
            filterStatus: 'الحالة',
            filterCategory: 'الفئة',
            filterCommunity: 'المجتمع',
            filterUrgency: 'درجة الإستعجال',
            applyFilters: 'تطبيق',
            resetFilters: 'مسح',
            colRef: 'الرقم المرجعي',
            colResident: 'الساكن',
            colUnit: 'الوحدة',
            colCategory: 'الفئة',
            colUrgency: 'درجة الإستعجال',
            colStatus: 'الحالة',
            colSla: 'زمن الاستجابة',
            colAssigned: 'المعيّن له',
            slaOverdue: 'متأخر',
            slaDueSoon: 'يقترب موعده',
            slaOnTime: 'في الوقت',
            noRequests: 'لا توجد طلبات خدمة حتى الآن.',
            noFilteredRequests: 'لا توجد طلبات تطابق الفلاتر.',
            clearFilters: 'مسح الفلاتر',
            allAssigned: 'جميع الطلبات معيّنة. عمل رائع!',
            assignTechnicianLabel: 'تعيين فني',
            assignPlaceholder: 'اختر فنياً...',
            priorityLabel: 'الأولوية',
            priorityLow: 'منخفض',
            priorityMedium: 'متوسط',
            priorityHigh: 'عالي',
            priorityUrgent: 'عاجل',
            internalNotesLabel: 'ملاحظات داخلية',
            notePlaceholder: 'أضف ملاحظة داخلية (غير مرئية للساكن)...',
            addNoteButton: 'إضافة ملاحظة',
            notesHistoryLabel: 'سجل الملاحظات',
            assignButton: 'تعيين وحفظ',
            assignSuccess: 'تم تعيين الطلب بنجاح.',
            noteSuccess: 'تمت إضافة الملاحظة بنجاح.',
            assignError: 'فشل التعيين. حاول مرة أخرى.',
            viewButton: 'عرض',
            backToList: 'العودة إلى القائمة',
            detailResident: 'الساكن',
            detailLocation: 'الموقع',
            detailCategory: 'الفئة',
            detailUrgency: 'درجة الإستعجال',
            detailStatus: 'الحالة',
            detailSubmitted: 'تاريخ التقديم',
            detailDescription: 'الوصف',
            detailPhotos: 'الصور المرفقة',
        },
        serviceCategories: {
            pageTitle: 'فئات الخدمة',
            heading: 'فئات الخدمة',
            description: 'تكوين الفئات والفئات الفرعية وأهداف مستوى الخدمة لطلبات الخدمة.',
            newCategory: '+ فئة جديدة',
            newSubcategory: '+ إضافة فئة فرعية',
            labelNameEn: 'الاسم (إنجليزي)',
            labelNameAr: 'الاسم (عربي)',
            labelIcon: 'اختر أيقونة',
            labelResponseSla: 'وقت الاستجابة المستهدف (ساعات)',
            labelResolutionSla: 'وقت الحل المستهدف (ساعات)',
            labelDefaultAssignee: 'المسؤول الافتراضي',
            labelCommunities: 'متاح في المجتمعات',
            labelRequirePhoto: 'يتطلب صورة إنجاز',
            labelStatus: 'الحالة',
            sheetNewTitle: 'فئة جديدة',
            sheetEditTitle: 'تعديل الفئة',
            sheetSubNewTitle: 'فئة فرعية جديدة',
            sheetSubEditTitle: 'تعديل الفئة الفرعية',
            saveButton: 'حفظ الفئة',
            saveSubButton: 'حفظ الفئة الفرعية',
            saving: 'جار الحفظ...',
            cancelButton: 'إلغاء',
            inheritedSla: 'موروث من الفئة الرئيسية',
            statusActive: 'نشط',
            statusInactive: 'غير نشط',
            colSubName: 'الاسم',
            colSubSla: 'مستوى الخدمة',
            colSubActions: 'الإجراءات',
            editAction: 'تعديل',
            disableAction: 'تعطيل',
            enableAction: 'تفعيل',
            deleteAction: 'حذف',
            noCategories: 'لا توجد فئات مكونة. أنشئ أول فئة خدمة.',
            responseSlaLabel: 'استجابة {{hours}} ساعة',
            resolutionSlaLabel: 'حل {{hours}} ساعة',
            placeholderAssignee: 'ابحث عن مستخدم...',
            placeholderResponseHours: '4',
            placeholderResolutionHours: '24',
            slaConfigHeading: 'إعداد مستوى الخدمة',
        },
        leads: {
            pageTitle: 'العملاء المحتملون',
            heading: 'العملاء المحتملون',
            description: 'إدارة ومتابعة قائمة العملاء المحتملين',
            addLead: '+ إضافة عميل محتمل',
            import: 'استيراد',
            importComingSoon: 'استيراد العملاء قريباً',
            noLeadsYet: 'لا يوجد عملاء محتملون بعد',
            noLeadsDescription: 'ابدأ بتتبع قائمتك بإضافة أول عميل محتمل أو استيراد قائمة موجودة من Excel.',
            importFromExcel: 'استيراد من Excel',
            downloadTemplate: 'تنزيل القالب',
            importLeads: 'استيراد العملاء',
            showingSummary: 'عرض {{from}} إلى {{to}} من أصل {{total}} عميل محتمل',
            importSheet: {
                heading: 'استيراد العملاء من Excel',
                fileHint: 'الصيغ المقبولة: ‏.xlsx‏، ‏.xls',
                maxSize: 'الحجم الأقصى للملف: 5 ميجابايت',
                maxRows: 'الحد الأقصى للصفوف: 500',
                templateLink: 'تنزيل قالب الاستيراد',
                uploadCta: 'رفع ومعاينة',
                uploading: 'جارٍ الرفع…',
                formatError: 'تعذّر قراءة هذا الملف. يرجى رفع ملف Excel بصيغة ‏.xlsx أو ‏.xls باستخدام القالب المُرفَق.',
                cancel: 'إلغاء',
            },
            review: {
                pageTitle: 'مراجعة الاستيراد: {{filename}}',
                heading: 'نتائج التحقق',
                description: 'راجع الأخطاء أدناه. لا يزال بإمكانك استيراد الصفوف الصحيحة.',
                totalRows: 'إجمالي الصفوف',
                validRows: 'الصفوف الصحيحة',
                errorRows: 'الصفوف الخاطئة',
                allValidAlert: 'اجتاز جميع {{n}} صف عملية التحقق. جاهز للاستيراد.',
                allInvalidAlert: 'لا توجد صفوف صحيحة. أصلح الأخطاء وأعد الرفع.',
                downloadErrorReport: 'تنزيل تقرير الأخطاء',
                tableRowNum: 'الصف',
                tableField: 'الحقل',
                tableError: 'الخطأ',
                importCta: 'استيراد {{n}} صف صحيح',
                importAllCta: 'استيراد {{n}} عميل',
                importDisabledTooltip: 'لا توجد صفوف صحيحة للاستيراد',
                cancelLink: 'إلغاء — العودة إلى العملاء المحتملين',
                confirmTitle: 'هل تريد استيراد الصفوف الصحيحة فقط؟',
                confirmBody: 'أنت على وشك استيراد {{n}} عميل محتمل صحيح. سيتم تخطي {{m}} صفًا تحتوي على أخطاء دون حفظها. يمكنك إعادة استيرادها بعد تصحيح الأخطاء.',
                confirmCta: 'استيراد {{n}} عميل',
                successToast: 'تم استيراد {{n}} عميل محتمل بنجاح.',
            },
            filter: {
                search: 'بحث',
                searchPlaceholder: 'الاسم أو رقم الهاتف…',
                status: 'الحالة',
                allStatuses: 'جميع الحالات',
                source: 'المصدر',
                allSources: 'جميع المصادر',
                rows: 'الصفوف',
                apply: 'تطبيق',
                reset: 'إعادة تعيين',
            },
            table: {
                name: 'اسم العميل',
                phone: 'الهاتف',
                email: 'البريد الإلكتروني',
                source: 'المصدر',
                status: 'الحالة',
                assignedTo: 'مسؤول عنه',
                createdAt: 'تاريخ الإضافة',
                unassigned: '—',
            },
            status: {
                new: 'جديد',
                contacted: 'تم التواصل',
                qualified: 'مؤهل',
                converted: 'تم التحويل',
                lost: 'خسر',
            },
            drawer: {
                title: 'إضافة عميل محتمل',
                nameEn: 'اسم العميل (إنجليزي)',
                nameAr: 'اسم العميل (عربي)',
                phone: 'رقم الهاتف',
                email: 'البريد الإلكتروني',
                source: 'المصدر',
                selectSource: 'اختر المصدر',
                notes: 'ملاحظات (اختياري)',
                save: 'حفظ العميل',
                cancel: 'إلغاء',
            },
            toast: {
                added: 'تم إضافة العميل',
                assigned: 'تم تعيين العميل المحتمل إلى {{name}}',
                statusUpdated: 'تم تحديث الحالة إلى {{status}}',
                noteAdded: 'تمت إضافة الملاحظة',
                deleted: 'تم حذف العميل المحتمل',
                error: 'فشل الحفظ — يرجى المحاولة مرة أخرى',
            },
            detail: {
                pageTitle: '{{name}} — تفاصيل العميل المحتمل',
                tabDetails: 'التفاصيل',
                tabActivity: 'النشاط',
                tabActivityCount: 'النشاط ({{count}})',
                statusLabel: 'الحالة',
                statusPlaceholder: 'اختر الحالة…',
                lostReasonLabel: 'سبب الخسارة (اختياري)',
                lostReasonPlaceholder: 'مثال: قيود الميزانية، غير مهتم…',
                lostReasonCounter: '{{n}} / 500',
                unsavedChanges: 'تغييرات غير محفوظة',
                discard: 'تجاهل',
                saveChanges: 'حفظ التغييرات',
                assignmentTitle: 'التعيين',
                assignedToLabel: 'المعيَّن له',
                unassigned: 'غير مُعيَّن',
                assignBtn: 'تعيين',
                changeBtn: 'تغيير',
                unassignBtn: 'إلغاء التعيين',
                sheetTitle: 'تعيين العميل المحتمل',
                sheetSearch: 'البحث عن أعضاء الفريق…',
                sheetEmptyDefault: 'ابدأ الكتابة للبحث عن أعضاء الفريق',
                sheetEmptyNoResults: 'لا يوجد أعضاء فريق يتطابقون مع "{{query}}"',
                sheetCancel: 'إلغاء',
                sheetConfirm: 'تعيين',
                currentlyAssigned: 'معيَّن حالياً',
                notesTitle: 'الملاحظات',
                notesPlaceholder: 'لم تُضَف ملاحظات',
                contactInfoTitle: 'بيانات التواصل',
                phoneLabel: 'الهاتف',
                emailLabel: 'البريد الإلكتروني',
                sourceLabel: 'المصدر',
                createdLabel: 'تاريخ الإضافة',
                addNote: 'إضافة ملاحظة',
                composePlaceholder: 'اكتب ملاحظة…',
                composeCancel: 'إلغاء',
                composeSave: 'حفظ الملاحظة',
                emptyActivityHeading: 'لا يوجد نشاط بعد',
                emptyActivityBody: 'ستظهر هنا الأحداث مثل تغييرات الحالة وتحديثات التعيين والملاحظات.',
                emptyActivityCta: 'أضف أول ملاحظة',
                activityAssigned: 'تم التعيين إلى {{name}}',
                activityUnassigned: 'تم إلغاء التعيين من {{name}}',
                activityStatusChange: 'تغيير الحالة: {{from}} ← {{to}}',
                activityNote: 'ملاحظة',
                activityActor: 'بقلم: {{name}} · {{time}}',
                deleteTitle: 'حذف العميل المحتمل',
                deleteBody: 'هل أنت متأكد من حذف هذا العميل المحتمل؟ لا يمكن التراجع عن هذا الإجراء.',
                deleteConfirm: 'حذف العميل المحتمل',
                moreActions: 'المزيد من الإجراءات',
                editLead: 'تعديل العميل المحتمل',
                deleteLead: 'حذف العميل المحتمل',
                activityConverted: 'تم تحويل العميل إلى جهة اتصال {{type}}',
            },
            conversion: {
                convertBtn: 'تحويل إلى جهة اتصال',
                rowMenuConvert: 'تحويل إلى جهة اتصال',
                drawerTitle: 'تحويل إلى جهة اتصال',
                drawerSubtitle: 'حدد نوع جهة الاتصال وأكد لإنشاء السجل.',
                typeLabel: 'نوع جهة الاتصال',
                typeOwner: 'مالك',
                typeOwnerHint: 'سيُضاف إلى الملاك في جهات الاتصال',
                typeResident: 'مستأجر',
                typeResidentHint: 'سيُضاف إلى المستأجرين في جهات الاتصال',
                dataPreviewTitle: 'البيانات التي سيتم نقلها',
                fieldFirstName: 'الاسم الأول (EN)',
                fieldLastName: 'اسم العائلة (EN)',
                fieldFirstNameAr: 'الاسم الأول (AR)',
                fieldLastNameAr: 'اسم العائلة (AR)',
                fieldEmail: 'البريد الإلكتروني',
                fieldPhone: 'الهاتف',
                convertCta: 'تحويل',
                convertCtaLoading: 'جارٍ التحويل…',
                cancel: 'إلغاء',
                dedupTitle: 'تحذير: احتمال وجود تكرار',
                dedupBody: 'يوجد بالفعل جهة اتصال بنفس الهاتف أو البريد الإلكتروني.',
                dedupLinkOption: 'ربط العميل بجهة الاتصال الحالية (موصى به)',
                dedupCreateOption: 'إنشاء سجل جهة اتصال جديد على أي حال',
                dedupCta: 'متابعة',
                linkConfirmTitle: 'ربط العميل بجهة اتصال موجودة',
                linkConfirmBody: 'لن يتم إنشاء سجل جهة اتصال جديد.',
                linkConfirmCta: 'تأكيد الربط',
                createAnywayTitle: 'إنشاء جهة اتصال جديدة؟',
                createAnywayWarning: 'قد يؤدي إنشاء سجل جديد إلى تكرار جهات الاتصال في النظام.',
                createAnywayCta: 'إنشاء على أي حال',
                successCreated: 'تم التحويل — تم إنشاء جهة الاتصال.',
                successLinked: 'تم ربط العميل بجهة الاتصال الموجودة.',
                errorFailed: 'فشل التحويل. يرجى المحاولة مرة أخرى.',
                errorDedupCheck: 'تعذّر التحقق من التكرار. يرجى المحاولة مرة أخرى.',
                convertedCardTitle: 'جهة الاتصال المحولة',
                convertedDateLabel: 'تاريخ التحويل',
                convertedViewLink: 'عرض جهة الاتصال: {{name}} ({{type}})',
            },
        },
        pipeline: {
            title: 'سير عقود الإيجار',
            expiryWindow: 'نافذة الانتهاء',
            expiryFilter30: '٣٠ يوم',
            expiryFilter60: '٦٠ يوم',
            expiryFilter90: '٩٠ يوم',
            allStatuses: 'كل الحالات',
            allCommunities: 'كل المجمعات',
            search: 'بحث',
            searchPlaceholder: 'رقم العقد أو اسم المستأجر…',
            apply: 'تطبيق',
            reset: 'إعادة تعيين',
            groupExpiringSoon: 'تنتهي قريباً ({{n}})',
            groupActive: 'نشطة ({{n}})',
            groupExpired: 'منتهية ({{n}})',
            groupTerminated: 'منهية ({{n}})',
            groupPending: 'معلقة ({{n}})',
            columnLease: 'العقد#',
            columnUnit: 'الوحدة',
            columnBuilding: 'المبنى',
            columnCommunity: 'المجتمع',
            columnTenant: 'المستأجر',
            columnStartDate: 'البداية',
            columnEndDate: 'النهاية',
            columnRent: 'الإيجار',
            columnDays: 'أيام',
            daysRemaining: '{{n}}ي',
            daysExpired: 'منتهي',
            countTotal: 'عرض {{shown}} من {{total}} عقد',
            empty: 'لا توجد عقود تطابق الفلاتر الحالية.',
            clearFilters: 'مسح الفلاتر',
            viewLease: 'عرض العقد',
            export: 'تصدير Excel',
            exportTitle: 'تصدير سير العقود',
            exportMatching: 'تصدير {{n}} عقد مطابق للفلتر الحالي',
            exportColumns: 'الأعمدة المضمنة',
            exportFormat: 'الصيغة',
            exportDownload: 'تحميل',
            exportCancel: 'إلغاء',
            settingsLink: 'الإعدادات',
            settings: {
                title: 'إعدادات تنبيهات العقود',
                description: 'تكوين موعد استلام الإشعارات داخل التطبيق ورسائل البريد الإلكتروني حول انتهاء العقود القادمة.',
                alertThresholds: 'حدود التنبيه',
                dayBefore: '{{n}} يوم قبل الانتهاء',
                inApp: 'إشعار داخل التطبيق',
                email: 'بريد إلكتروني',
                addCustom: 'إضافة حد مخصص',
                removeThreshold: 'إزالة',
                customDays: 'أيام',
                resetDefaults: 'إعادة للافتراضي',
                save: 'حفظ الإعدادات',
                saved: 'تم حفظ إعدادات التنبيه.',
                backToPipeline: '← العودة للسير',
            },
        },
        contacts: {
            shared: {
                name: 'الاسم',
                firstName: 'الاسم الأول',
                lastName: 'اسم العائلة',
                email: 'البريد الإلكتروني',
                phone: 'الهاتف',
                phoneNumber: 'رقم الهاتف',
                phoneNumberPlaceholder: '5XXXXXXXX',
                countryCode: 'رمز الدولة',
                countryCodePlaceholder: 'SA',
                nationalId: 'الهوية الوطنية',
                gender: 'الجنس',
                dateOfBirth: 'تاريخ الميلاد',
                role: 'الدور',
                status: 'الحالة',
                units: 'الوحدات',
                leases: 'العقود',
                contactInfo: 'بيانات التواصل',
                personalInfo: 'البيانات الشخصية',
                info: 'المعلومات',
                lastLogin: 'آخر تسجيل دخول',
                select: 'اختر',
                selectGender: 'اختر الجنس',
                male: 'ذكر',
                female: 'أنثى',
                confirmDeletePrompt: 'هل أنت متأكد؟',
            },
            tenants: {
                pageTitle: 'المستأجرون',
                heading: 'المستأجرون',
                description: 'إدارة المستأجرين عبر عقاراتك.',
                newTenant: 'مستأجر جديد',
                createTitle: 'إنشاء مستأجر',
                createDescription: 'إضافة مستأجر جديد.',
                editTitle: 'تعديل {{name}}',
                editDescription: 'تحديث بيانات المستأجر.',
                createButton: 'إنشاء مستأجر',
                updateButton: 'تحديث المستأجر',
                emptyMessage: 'لا يوجد مستأجرون.',
            },
            owners: {
                pageTitle: 'الملاك',
                heading: 'الملاك',
                description: 'إدارة ملاك العقارات.',
                newOwner: 'مالك جديد',
                createTitle: 'إنشاء مالك',
                createDescription: 'إضافة مالك عقار جديد.',
                editTitle: 'تعديل {{name}}',
                editDescription: 'تحديث بيانات المالك.',
                createButton: 'إنشاء مالك',
                updateButton: 'تحديث المالك',
                emptyMessage: 'لا يوجد ملاك.',
            },
            admins: {
                pageTitle: 'المديرون',
                heading: 'المديرون',
                description: 'إدارة المدراء ومسؤولي النظام.',
                newAdmin: 'مدير جديد',
                createTitle: 'إنشاء مدير',
                createDescription: 'إضافة مدير جديد.',
                editTitle: 'تعديل {{name}}',
                editDescription: 'تحديث بيانات المدير.',
                createButton: 'إنشاء مدير',
                updateButton: 'تحديث المدير',
                emptyMessage: 'لا يوجد مديرون.',
                roles: {
                    admin: 'مدير',
                    accountingManager: 'مدير مالي',
                    serviceManager: 'مدير الخدمات',
                    marketingManager: 'مدير التسويق',
                    salesLeasingManager: 'مدير المبيعات والتأجير',
                },
            },
            professionals: {
                pageTitle: 'المختصون',
                heading: 'المختصون',
                description: 'إدارة المختصين والفنيين.',
                newProfessional: 'مختص جديد',
                createTitle: 'إنشاء مختص',
                createDescription: 'إضافة مختص خدمة جديد.',
                editTitle: 'تعديل {{name}}',
                editDescription: 'تحديث بيانات المختص.',
                createButton: 'إنشاء مختص',
                updateButton: 'تحديث المختص',
                emptyMessage: 'لا يوجد مختصون.',
            },
            residents: {
                pageTitle: 'المقيمون',
                heading: 'المقيمون',
                description: 'جميع جهات الاتصال للمقيمين في مجتمعك.',
                newResident: 'مقيم جديد',
                createTitle: 'مقيم جديد',
                createDescription: 'أدخل التفاصيل أدناه لإنشاء جهة اتصال مقيم جديدة.',
                createButton: 'إنشاء مقيم',
                cancel: 'إلغاء',
                emptyMessage: 'لا يوجد مقيمون.',
                searchPlaceholder: 'ابحث بالاسم أو رقم الجوال…',
                firstNameEn: 'الاسم الأول (إنجليزي)',
                lastNameEn: 'الاسم الأخير (إنجليزي)',
                firstNameAr: 'الاسم الأول (عربي)',
                lastNameAr: 'الاسم الأخير (عربي)',
                idType: 'نوع الهوية',
                selectIdType: 'اختر نوع الهوية',
                idNumber: 'رقم الهوية',
                duplicateHeading: 'يوجد مقيم مسجل بهذا الرقم بالفعل.',
                duplicateGoToExisting: 'الانتقال إلى السجل الموجود',
                duplicateConfirmCheckbox: 'أؤكد أن هذا شخص مختلف وأريد إنشاء سجل جديد.',
                createdToast: 'تم إنشاء المقيم بنجاح.',
                dependents: 'المستخدمون الإضافيون',
                idTypes: {
                    national_id: 'الهوية الوطنية',
                    passport: 'جواز السفر',
                    iqama: 'الإقامة',
                    emirates_id: 'هوية إماراتية',
                    other: 'أخرى',
                },
            },
        },
        appSettings: {
            common: {
                appSettings: 'إعدادات التطبيق',
                actions: 'الإجراءات',
                status: 'الحالة',
                active: 'نشط',
                inactive: 'غير نشط',
                save: 'حفظ',
                cancel: 'إلغاء',
                edit: 'تعديل',
                delete: 'حذف',
            },
            shell: {
                pageTitle: 'الإعدادات - {{title}}',
                heading: '{{title}}',
                description: 'إدارة وتحديث إعدادات مساحة العمل.',
                cardDescription: 'راجع وعدّل قسم الإعدادات المحدد.',
                tabs: {
                    invoice: 'الفاتورة',
                    serviceRequest: 'طلب الخدمة',
                    visitorRequest: 'طلب الزوار',
                    bankDetails: 'البيانات البنكية',
                    visitsDetails: 'تفاصيل الزيارات',
                    salesDetails: 'تفاصيل المبيعات',
                },
                requestTypes: {
                    homeService: 'خدمة منزلية',
                    neighbourhoodService: 'خدمة الحي',
                },
                invoiceDescription: 'إدارة بيانات الفاتورة ومعلومات التحصيل.',
                companyName: 'اسم الشركة',
                companyNamePlaceholder: 'أدخل اسم الشركة',
                address: 'العنوان',
                addressPlaceholder: 'أدخل العنوان',
                vat: 'ضريبة القيمة المضافة %',
                vatNumber: 'الرقم الضريبي',
                vatNumberPlaceholder: 'أدخل الرقم الضريبي',
                crNumber: 'رقم السجل التجاري',
                crNumberPlaceholder: 'أدخل رقم السجل التجاري',
                paymentInstructions: 'تعليمات الدفع',
                paymentInstructionsPlaceholder: 'أدخل تعليمات الدفع',
                notes: 'ملاحظات',
                invoiceNotesPlaceholder: 'أدخل ملاحظات الفاتورة',
                saveInvoiceSettings: 'حفظ إعدادات الفاتورة',
                serviceRequestTitle: 'طلب الخدمة',
                serviceRequestTypes: 'أنواع طلبات الخدمة',
                category: 'الفئة',
                status: 'الحالة',
                subcategories: 'الفئات الفرعية',
                navigation: 'التنقل',
                details: 'التفاصيل',
                openCategory: 'فتح الفئة',
                noServiceRequestCategories: 'لا توجد فئات لطلبات الخدمة.',
                enabled: 'مفعل',
                requirePreApproval: 'يتطلب موافقة مسبقة',
                maxVisitorsPerRequest: 'الحد الأقصى للزوار لكل طلب',
                allowedVisitDurationMinutes: 'المدة المسموحة للزيارة (بالدقائق)',
                saveVisitorRequestSettings: 'حفظ إعدادات طلب الزوار',
                allDayVisits: 'زيارات طوال اليوم',
                availableDays: 'الأيام المتاحة',
                startTime: 'وقت البداية',
                endTime: 'وقت النهاية',
                maxDailyVisits: 'الحد الأقصى للزيارات اليومية',
                saveVisitsDetails: 'حفظ تفاصيل الزيارات',
                beneficiaryName: 'اسم المستفيد',
                bankName: 'اسم البنك',
                accountNumber: 'رقم الحساب',
                iban: 'رقم الآيبان',
                saveBankDetails: 'حفظ البيانات البنكية',
                depositTimeLimitDays: 'مهلة السداد (بالأيام)',
                cashContractSigningDays: 'مدة توقيع عقد الكاش (بالأيام)',
                bankContractSigningDays: 'مدة توقيع عقد البنك (بالأيام)',
                saveSalesDetails: 'حفظ تفاصيل المبيعات',
                unsupportedSettingsTab: 'تبويب الإعدادات هذا غير مُعد بعد.',
            },
            requestCategories: {
                pageTitle: 'فئات الطلبات',
                heading: 'فئات الطلبات',
                description: 'إدارة فئات الطلبات والفئات الفرعية.',
                newCategory: 'فئة جديدة',
                createTitle: 'إنشاء فئة طلب',
                createDescription: 'إضافة فئة طلب جديدة.',
                editTitle: 'تعديل {{name}}',
                editHeading: 'تعديل: {{name}}',
                editDescription: 'تحديث بيانات الفئة وإدارة الفئات الفرعية.',
                nameEn: 'الاسم (بالإنجليزية)',
                nameAr: 'الاسم (بالعربية)',
                status: 'الحالة',
                hasSubcategories: 'يتضمن فئات فرعية',
                subcategories: 'الفئات الفرعية',
                subcategoriesCount: 'الفئات الفرعية',
                createButton: 'إنشاء الفئة',
                updateButton: 'تحديث الفئة',
                addSubcategory: 'إضافة فئة فرعية',
                addSubcategoryButton: 'إضافة فئة فرعية',
                cancelSubcategory: 'إلغاء',
                allDay: 'طوال اليوم',
                startTime: 'وقت البداية',
                endTime: 'وقت النهاية',
                addSubcategorySubmit: 'إضافة الفئة الفرعية',
                serviceSettingsTitle: 'صلاحيات إعدادات الخدمة',
                serviceSettingsDescription: 'إعداد صلاحيات معالجة الطلبات لهذه الفئة.',
                managerCloseRequest: 'يمكن للمدير إغلاق الطلب',
                attachmentsRequired: 'المرفقات مطلوبة',
                allowProfessionalReschedule: 'السماح للمختص بإعادة الجدولة',
                skipRequestCodeForProfessional: 'تخطي رمز الطلب للمختص',
                skipRequestPhotoForProfessional: 'تخطي رفع صورة الطلب للمختص',
                saveServiceSettings: 'حفظ إعدادات الخدمة',
                noSubcategories: 'لا توجد فئات فرعية.',
                noCategories: 'لا توجد فئات بعد.',
                deleteCategoryConfirm: 'هل أنت متأكد من حذف هذه الفئة؟',
                deleteSubcategoryConfirm: 'حذف هذه الفئة الفرعية؟',
                subcategoryNamePlaceholder: 'اسم الفئة الفرعية',
                subcategoryNameArPlaceholder: 'اسم الفئة الفرعية بالعربية',
            },
            facilityCategories: {
                pageTitle: 'فئات المرافق',
                heading: 'فئات المرافق',
                description: 'إدارة أنواع فئات المرافق.',
                newCategory: 'فئة جديدة',
                createTitle: 'إنشاء فئة مرفق',
                createDescription: 'إضافة فئة مرفق جديدة.',
                editTitle: 'تعديل {{name}}',
                editHeading: 'تعديل: {{name}}',
                editDescription: 'تحديث فئة المرفق.',
                nameEn: 'الاسم (بالإنجليزية)',
                nameAr: 'الاسم (بالعربية)',
                facilities: 'المرافق',
                createButton: 'إنشاء الفئة',
                updateButton: 'تحديث الفئة',
                noCategories: 'لا توجد فئات بعد.',
                deleteCategoryConfirm: 'هل أنت متأكد من حذف هذه الفئة؟',
            },
            facilities: {
                pageTitle: 'إعدادات المرافق',
                heading: 'إعدادات المرافق',
                description: 'إدارة المرافق وجهوزيتها للحجز.',
                addFacility: 'إضافة مرفق',
                editFacility: 'تعديل مرفق',
                formDescription: 'ضبط تفاصيل المرفق وقواعد الحجز.',
                createFacility: 'إنشاء مرفق',
                updateFacility: 'تحديث المرفق',
                name: 'الاسم',
                nameEn: 'الاسم (بالإنجليزية)',
                nameAr: 'الاسم (بالعربية)',
                category: 'الفئة',
                selectCategory: 'اختر الفئة',
                community: 'المشروع',
                selectCommunity: 'اختر المشروع',
                descriptionLabel: 'الوصف',
                capacity: 'السعة',
                open: 'وقت الفتح',
                close: 'وقت الإغلاق',
                bookingFee: 'رسوم الحجز',
                requiresApproval: 'يتطلب موافقة',
                status: 'الحالة',
                bookings: 'الحجوزات',
                actions: 'الإجراءات',
                view: 'عرض',
                empty: 'لا توجد مرافق.',
                facilityTitle: 'المرفق - {{name}}',
                detailsDescription: 'راجع بيانات المرفق وآخر الحجوزات.',
                details: 'تفاصيل المرفق',
                approval: 'الموافقة',
                required: 'مطلوبة',
                notRequired: 'غير مطلوبة',
                recentBookings: 'آخر الحجوزات',
                bookingTitle: 'حجز #{{id}}',
                to: 'إلى',
                guests: 'الضيوف',
                notes: 'ملاحظات',
                noBookings: 'لا توجد حجوزات حديثة.',
            },
            serviceRequestDetails: {
                pageTitle: 'طلب الخدمة - {{name}}',
                heading: 'طلب الخدمة - {{name}}',
                description: 'راجع الفئات الفرعية وإعدادات الصلاحيات.',
                details: 'التفاصيل',
                type: 'النوع',
                categoryCode: 'رمز الفئة',
                backToSettings: 'العودة للإعدادات',
                openCategoryEditor: 'فتح محرر الفئة',
                subcategories: 'الفئات الفرعية',
                name: 'الاسم',
                status: 'الحالة',
                noSubcategories: 'لا توجد فئات فرعية.',
                permissionsTitle: 'الصلاحيات',
                enabled: 'مفعل',
                disabled: 'غير مفعل',
                noPermissions: 'لا توجد صلاحيات مُعدة.',
            },
            general: {
                pageTitle: 'الإعدادات العامة',
                heading: 'الإعدادات العامة',
                description: 'إدارة جداول القيم لأنواع العقود وجداول السداد وفئات المعاملات.',
                addSetting: 'إضافة إعداد',
                cancelAdd: 'إلغاء',
                nameEn: 'الاسم (بالإنجليزية)',
                nameAr: 'الاسم (بالعربية)',
                type: 'النوع',
                selectType: 'اختر النوع',
                settingNamePlaceholder: 'اسم الإعداد',
                settingNameArPlaceholder: 'اسم الإعداد بالعربية',
                addSettingSubmit: 'إضافة إعداد',
                noSettings: 'لا توجد إعدادات حتى الآن. أضف أول إعداد للبدء.',
                rentalContractTypes: 'أنواع عقود الإيجار',
                paymentSchedules: 'جداول السداد',
                transactionCategories: 'فئات المعاملات',
                transactionTypes: 'أنواع المعاملات',
                calculationBasis: 'أساس الاحتساب',
                fitOutStatus: 'حالة التشطيب',
                paymentFrequency: 'تكرار السداد',
                unknownType: 'أخرى ({{type}})',
                deleteSettingConfirm: 'حذف هذا الإعداد؟',
            },
        },
        settingsForms: {
            pageTitle: 'إعدادات النماذج',
            formTemplates: 'قوالب النماذج',
            description: 'أنشئ وأدر نماذج طلبات بسيطة لفريقك.',
            createTemplate: 'إنشاء قالب',
            templates: 'القوالب',
            name: 'الاسم',
            category: 'الفئة',
            communityBuilding: 'المشروع / المبنى',
            status: 'الحالة',
            actions: 'الإجراءات',
            preview: 'معاينة',
            empty: 'لا توجد قوالب نماذج.',
            editTemplate: 'تعديل القالب',
            createDescription: 'أعد النموذج في ثلاث خطوات سريعة: التفاصيل، النطاق، والأسئلة.',
            simpleFlowHint: 'نصيحة: اجعل النموذج مختصرًا وأضف فقط الأسئلة الضرورية.',
            selectCommunity: 'اختر المشروع',
            selectBuilding: 'اختر المبنى',
            updateTemplate: 'تحديث القالب',
            newTemplate: 'قالب جديد',
            basicDetails: '1. التفاصيل الأساسية',
            basicDetailsDescription: 'أدخل اسمًا واضحًا للنموذج ووصفًا مختصرًا.',
            whereUsed: '2. مكان استخدام النموذج (اختياري)',
            whereUsedDescription: 'اختر الفئة والمشروع والمبنى فقط إذا أردت تقييد مكان ظهور النموذج.',
            templateName: 'اسم النموذج',
            descriptionLabel: 'وصف مختصر',
            requestCategory: 'فئة الطلب',
            selectCategory: 'اختر الفئة (اختياري)',
            community: 'المشروع',
            building: 'المبنى',
            selectCommunityFirst: 'اختر مشروعًا أولًا',
            schemaJson: 'المخطط (JSON)',
            formBuilder: '3. الأسئلة',
            formBuilderDescription: 'أضف الأسئلة التي سيقوم المستخدم بتعبئتها.',
            addField: 'إضافة سؤال',
            addFirstField: 'أضف أول سؤال',
            fieldPosition: 'السؤال {{number}}',
            fieldLabel: 'نص السؤال',
            fieldKey: 'مفتاح الحقل',
            fieldType: 'نوع الإجابة',
            fieldPlaceholder: 'مثال للإجابة (اختياري)',
            fieldKeyAutoGenerated: 'لا تحتاج لأي إعداد تقني. يتم إنشاء معرفات الحقول تلقائيًا.',
            moveFieldUp: 'نقل لأعلى',
            moveFieldDown: 'نقل لأسفل',
            removeField: 'حذف السؤال',
            fieldTypeText: 'نص',
            fieldTypeTextarea: 'نص طويل',
            fieldTypeNumber: 'رقم',
            fieldTypeEmail: 'بريد إلكتروني',
            fieldTypeTel: 'هاتف',
            fieldTypeDate: 'تاريخ',
            activeTemplate: 'تفعيل هذا النموذج',
            schemaMustBeValidJson: 'يجب أن يكون مخطط JSON صالحًا.',
            templatePreviewTitle: 'معاينة القالب - {{name}}',
            previewTitle: 'معاينة القالب - {{name}}',
            previewDescription: 'راجع إعدادات القالب والحقول.',
            templateDetails: 'تفاصيل القالب',
            fieldsPreview: 'معاينة الحقول',
            type: 'النوع',
            required: 'إجابة إلزامية',
            fieldFallback: 'حقل {{number}}',
            noFieldsConfigured: 'لا توجد أسئلة بعد.',
            requiredFields: 'الحقول الإلزامية',
            noRequiredFields: 'لا توجد حقول إلزامية.',
            selectCommunityDescription: 'اختر مشروعًا للبدء في إنشاء نموذج له.',
            communities: 'المشاريع',
            select: 'اختيار',
            noCommunities: 'لا توجد مشاريع.',
            selectBuildingDescription: 'اختر مشروعًا ومبنى ثم أكمل إعداد النموذج.',
            buildingScope: 'نطاق المبنى',
            useInTemplate: 'استخدم هذا المبنى',
            noBuildingsForCommunity: 'لا توجد مبانٍ لهذا المشروع.',
        },
        companyProfile: {
            pageTitle: 'ملف الشركة',
            heading: 'ملف الشركة',
            description: 'اضبط هوية شركتك وشعارها وإعدادات علامتها التجارية التي تظهر على جميع العقود والفواتير وصفحة تسجيل الدخول.',
            sectionIdentity: 'الهوية',
            sectionLogos: 'الشعار والعلامة التجارية',
            sectionRegional: 'الإعدادات الإقليمية',
            sectionBrandColors: 'ألوان العلامة التجارية',
            nameEn: 'اسم الشركة (بالإنجليزية)',
            nameEnPlaceholder: 'أدخل اسم الشركة بالإنجليزية',
            nameAr: 'اسم الشركة (بالعربية)',
            nameArPlaceholder: 'أدخل اسم الشركة بالعربية',
            vatNumber: 'رقم تسجيل ضريبة القيمة المضافة',
            vatNumberPlaceholder: 'أدخل رقم ضريبة القيمة المضافة المكون من 15 رقمًا',
            vatHelper: 'يظهر على جميع الفواتير الصادرة.',
            crNumber: 'رقم السجل التجاري (اختياري)',
            crNumberPlaceholder: 'أدخل رقم السجل التجاري',
            primaryLogo: 'الشعار الأساسي',
            logoHint: 'يظهر هذا الشعار على جميع عقود الإيجار والفواتير.',
            uploadLogo: 'رفع شعار',
            changeLogo: 'تغيير',
            removeLogo: 'إزالة',
            uploadHelper: 'صيغة PNG أو SVG، الحجم الأقصى 2 ميغابايت',
            logoArVariant: 'نسخة الشعار العربية (اختياري)',
            logoArHint: 'يُستخدم في المستندات العربية. إذا لم يُرفع، يُستخدم الشعار الأساسي.',
            timezone: 'المنطقة الزمنية',
            timezoneHelper: 'يؤثر على جميع الطوابع الزمنية: تواريخ العقود ومواعيد استحقاق الفواتير وفتحات الحجز. الافتراضي هو UTC.',
            timezoneSearch: 'ابحث عن المنطقة الزمنية…',
            timezoneGroupGulf: 'الخليج',
            primaryColor: 'لون العلامة التجارية الأساسي',
            colorHint: 'يُطبَّق على رأس رسائل الإشعارات البريدية وزخارف قوالب المستندات. لا يؤثر على لون الشريط الجانبي للمنصة.',
            hexLabel: 'رمز اللون',
            emptyState: 'أعدّ ملف شركتك للبدء. سيظهر اسم شركتك وشعارها ورقم ضريبة القيمة المضافة على جميع المستندات الصادرة.',
            noLogo: 'لم يتم رفع شعار',
            unsavedChanges: 'تغييرات غير محفوظة',
            discard: 'تجاهل',
            saveChanges: 'حفظ التغييرات',
            saving: 'جارٍ الحفظ…',
            toastSuccess: 'تم حفظ ملف الشركة بنجاح.',
            toastError: 'حدث خطأ. تعذّر حفظ التغييرات.',
            retryMessage: 'حاول مرة أخرى، أو تواصل مع الدعم إذا استمر الخطأ.',
            validationNameEnRequired: 'اسم الشركة (بالإنجليزية) مطلوب.',
            validationNameArRequired: 'اسم الشركة (بالعربية) مطلوب.',
            validationVatFormat: 'رقم ضريبة القيمة المضافة يجب أن يتكوّن من 15 رقمًا.',
            validationLogoType: 'الملف يجب أن يكون بصيغة PNG أو SVG.',
            validationLogoSize: 'يجب ألا يتجاوز حجم الملف 2 ميغابايت.',
        },
    },
};

export const messages = {
    en: deepMerge(deepMerge(enBaseMessages as MessageTree, appEn), appEnFallback),
    ar: deepMerge(deepMerge(arBaseMessages as MessageTree, appAr), appArFallback),
};
