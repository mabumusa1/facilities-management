...
- PR #402 — 5 ACs / 20 gap tests added — 32 total, all pass. Notes: Carbon::setTestNow() essential for expiry-window boundary tests (always reset with setTestNow() at end); auth()->logout() required in unauthenticated tests when setUp() calls actingAs(); AppSetting is single-row-per-tenant via BelongsToAccountTenant; invalid expiry_window silently falls back to 30 (no 422); export-preview scopes count to current session tenant.
- PR #403 — 6 ACs / 24 gap tests added — 51 total, all pass. Notes: 'viewers' role does not exist (renamed to 'dependents') — fixed 2 existing tests that used it; Inertia JSON serializes whole-number floats as int (8500.0 → 8500) so Inertia ->where() assertions must use int not float; Double-finalize bug: MoveOutPolicy::finalize() does not check move-out is already settled — re-finalizing succeeds (302) and creates duplicate transactions; flagged for reviewer.
