...
- PR #375 (review fix) — service-requests/admin-triage-211 — `whereColumn('col','col')` is always true (tautology); always scope assignee dropdowns via JOIN+where. Scope `Rule::exists` to `account_memberships` table with `where('account_tenant_id', ...)` for cross-tenant injection defence. Cross-tenant assignment test: create user in OTHER tenant, assert PATCH returns 422 and `assigned_to_user_id` stays null.
- PR #376 (review fix) — facilities/admin-calendar-view-249 — Wayfinder regen in worktree: `ln -sf /var/www/html/vendor <worktree>/vendor` then `php artisan wayfinder:generate` from worktree dir. Dead FormRequest (authorize=false): set `return true` and wire into controller replacing inline validate(). Race lock: `Facility::where('id',$id)->lockForUpdate()->firstOrFail()` inside DB::transaction BEFORE conflict check — lock the always-existing parent row, not booking rows. Cross-tenant scope: `Rule::exists('rf_facilities','id')->where('account_tenant_id', Tenant::current()?->id)`. i18n common section only had delete/edit — add `'app.common.optional'` EN:'Optional' AR:'اختياري'.
- PR #376 round-2 fix — Route ordering: calendar routes MUST be registered BEFORE `Route::resource('facilities')` — resource registers `GET facilities/{facility}` which swallows `/facilities/calendar` in real HTTP (named-route tests pass regardless, masking the bug). Add a literal-URL test (`$this->get('/facilities/calendar')->assertOk()`) as regression guard — named-route tests alone do not catch ordering bugs. Same pattern applies to any explicit sub-path under a resourceful route.
- PR #377 — leasing/approval-workflow-173 — `leases.APPROVE` is generated by RbacSeeder cartesian (PermissionSubject::Leases × PermissionAction::Approve); managers+admins+salesAndLeasingManagers get it. Backend (controller, policy, migration, routes, Wayfinder) was already merged on 1.x from a prior engineer run — only UI (Show.vue approval panel + reject dialog) and tests remained. WorkflowNotifier sends WorkflowStatusChangedNotification to all tenant users — use Notification::fake() + assertSentTo() in tests.
- PR #377 (review fix) — i18n keys for approval panel must go in BOTH appEnFallback.ts AND appArFallback.ts (14 keys). Magic-number status ID (76) belongs in the backend as isPendingApplication prop, not computed on the frontend. Cross-tenant isolation tests need actingAs(adminB)+withSession(['tenant_id'=>tenantB->id]) to prove belongsToCurrentTenant gate fires. Pint auto-hoists FQCN inline use to the import block — write FQCNs inline, Pint fixes it.
- PR #389 (review fix) — admin/user-management-#242: (1) i18n keys for this feature live in messages.ts as nested EN+AR objects (not appEnFallback.ts/appArFallback.ts — those are for features added after the initial messages.ts was written). (2) Wayfinder routes dir is gitignored — the reviewer's concern about missing functions is a deploy-time concern, not a commit concern; running `php artisan wayfinder:generate` locally doesn't produce committable files. (3) Cross-tenant guard pattern: `abort_unless(AccountMembership::where('user_id',$user->id)->where('account_tenant_id', Tenant::current()?->id)->exists(), 403)` — add at top of each method before state checks. (4) Destructive confirmation dialog: use `<Dialog role="alertdialog">` (not a separate AlertDialog component — none exists); pattern from admin/roles/Index.vue. Vue 3 allows multiple root elements in templates (fragments); existing page components use this.
- PR #394 — leasing/lease-amendments-178 — Amendment history JSON endpoint lives under `rf.leases.amendments` (api group), not `leases.amendments` (web group). When a tech design says `GET rf/leases/{lease}/amendments → rf.leases.amendments`, the route is in routes/api.php under the `rf` prefix group, not web.php. Feature tests for this endpoint must use `route('rf.leases.amendments', $lease)` + `getJson()`. Stash-pop workflow: if an `agent` session leaves changes unstashed and you need to pull, do `git stash && git pull && git stash pop && git checkout -b <branch>`.
- PR #395 — leasing/tenant-notices-179 — Cross-tenant route model binding: `BelongsToAccountTenant` global scope does NOT filter route model binding (Lease resolves globally); `ChecksTenantOwnership` in policy enforces cross-tenant = 403 not 404. `LazilyRefreshDatabase` + PostgreSQL: new migrations not in schema dump cause `pg_type_typname_nsp_index` unique violation when two test runs overlap. Fix: run `php artisan schema:dump` after new migrations to update the snapshot so LazilyRefreshDatabase uses the dump instead of re-running. The `tenants` role (RbacSeeder) has no `leases.UPDATE` — use it for 403 tests. Only `admins` role has `leases.UPDATE`.
- PR #395 (review fix) — leasing/tenant-notices-179 — `Inertia::flash('toast', [...])` stores under session key `inertia.flash_data` (not `toast`); test must assert `assertSessionHas('inertia.flash_data')` and check `$data['toast']['type']`/`$data['toast']['message']`. Dead `v-if="form.wasSuccessful"` in a useForm component is always false after a redirect — remove it. To expose a Notices nav button in Show.vue: declare prop in defineProps, import Wayfinder action, add Button+Link+Badge in header alongside canAmend button.
- PR #398 — feat/admin-leads-list-296 — Backend files (controller/policy/request/factory/migration) may already exist as uncommitted changes from a prior agent session; check before recreating. When PR labels don't match (e.g. 'type:feature' doesn't exist), use 'type:story' + 'area:leasing' instead. `statusSeeder` adds lead statuses with IDs 80-84 — always seed it in lead-domain tests. `deferred` props appear as `undefined` on the first server render; type them as `T | undefined` and show skeleton when `=== undefined`. PageHeader slot `#actions` is the correct way to add a non-link button (like a drawer trigger) alongside the header title.
- PR #399 — feat/admin-lead-detail-297 — (1) `users` table has no `first_name`/`last_name` columns — only select `id,name,email` in eager loads. (2) When a dependent story (297) is based on an unmerged PR (398), branch from that PR's head and set `--base` to that branch's name. (3) Wayfinder action URLs for non-trivial routes take `{ lead: id }` object, not bare `id`. (4) Always run `php artisan schema:dump` after adding new migrations so LazilyRefreshDatabase uses the dump. (5) `addNote` policy can reuse `leads.UPDATE` permission — no need for a separate `leads.ADD_NOTE` in the RBAC seeder.
- PR #400 (review fix) — feat/admin-lead-import-298 — Bulk insert: `Model::insert($rows)` bypasses Eloquent events including BelongsToAccountTenant; manually add `account_tenant_id`, `created_at`, `updated_at` to every row array. CSV injection: tab-prefix any field value starting with =, +, -, @ before writing to CSV (OWASP pattern). Dead-branch fix: option (a) remove dead if — safest when UX flow spec doesn't require the alternate path. `gh pr checkout N` does NOT switch worktree; must `git checkout <branch>` manually in `/var/www/html`. LazilyRefreshDatabase parallel-seeder race (pg_type_typname_nsp_index) resolves by re-running test with `--no-coverage` flag (serialises test runner).
